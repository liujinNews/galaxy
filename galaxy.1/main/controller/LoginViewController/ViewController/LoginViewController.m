...
- (void)createProtocolBtn{
    ...
    float protocolBtnW = 130*w_Ratio;
    float userOriginX = ((Main_Screen_Width/2.0) - protocolBtnW)*9/10.0;
    
    UIButton *userPBtn = [UIButton buttonWithType:UIButtonTypeSystem];
    [userPBtn setTitle:Custing(@"《喜报用户协议》", nil) forState:UIControlStateNormal];
    userPBtn.clipsToBounds = YES;
    userPBtn.titleLabel.font = [UIFont systemFontOfSize:13*w_Ratio];
    [userPBtn setTitleColor:Color_Blue_Important_20 forState:UIControlStateNormal];
    [userPBtn setFrame:CGRectMake(userOriginX, Main_Screen_Height - 50*w_Ratio, protocolBtnW, 15*w_Ratio)];
    [userPBtn addTarget:self action:@selector(userProtocol:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:userPBtn];
    
    float protocolOriginX = (Main_Screen_Width/2.0)+((Main_Screen_Width/2.0) - protocolBtnW)*1/10.0;
    ...
    protocolBtn.clipsToBounds = YES;
    protocolBtn.titleLabel.font = [UIFont systemFontOfSize:13*w_Ratio];
    [protocolBtn setTitleColor:Color_Blue_Important_20 forState:UIControlStateNormal];
    [protocolBtn setFrame:CGRectMake(protocolOriginX, Main_Screen_Height - 50*w_Ratio, protocolBtnW, 15*w_Ratio)];
    [protocolBtn addTarget:self action:@selector(logOutProtocol:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:protocolBtn];
}
- (void)userProtocol:(UIButton *)sender{
    RootWebViewController *vc = [[RootWebViewController alloc]initWithUrl:@"http://139.196.104.114:82/useragreement.html"];
    [self.navigationController pushViewController:vc animated:YES];
}
//喜报隐私协议
- (void)logOutProtocol:(UIButton *)sender{
