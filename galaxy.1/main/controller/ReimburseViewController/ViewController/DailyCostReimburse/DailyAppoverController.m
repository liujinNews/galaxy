...
@interface DailyAppoverController ()

@end

@implementation DailyAppoverController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[dailyReimFormData alloc]initWithStatus:3];
    }
    return self;
}
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        if ([model.fieldName isEqualToString:@"TotalAmount"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
        ...
        }else if ([model.fieldName isEqualToString:@"SupplierBankName"]&&[[model.isShow stringValue]isEqualToString:@"1"]){
            [self updateSupplierBankNameViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"SupplierBankAccount"]&&[[model.isShow stringValue]isEqualToString:@"1"]){
...
-(void)updateReimShareViewWithType:(NSInteger)type{
    if (type == 1) {
        [_ReimShareMainView updateReimShareMainViewWith:self.FormDatas.arr_ShareForm WithData:self.FormDatas.arr_ShareData WithEditType:3 WithComePlace:2];
        __weak typeof(self) weakSelf = self;
...
-(void)updateSupplierBankNameViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_SupplierBankName WithContent:_txf_SupplierBankName WithFormType:formViewShowText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
...
-(void)updateSupplierBankAccountViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_SupplierBankAccount WithContent:_txf_SupplierBankAccount WithFormType:formViewShowText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView==_View_DetailsTable) {
        ...
        [cell configViewWithArray:self.FormDatas.arr_sonItem withIndex:indexPath.row withNeedSure:self.FormDatas.bool_needSure withComePlace:@"daily"];
        if (cell.btn_Sure) {
