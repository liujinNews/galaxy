...
@interface dailyReimViewController ()

@end

@implementation dailyReimViewController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[dailyReimFormData alloc]initWithStatus:1];
    }
    return self;
}
...
-(void)createMainView{
    ...
    _View_SupplierBankName = [[UIView alloc]init];
    _View_SupplierBankName.backgroundColor = Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_SupplierBankName];
    [_View_SupplierBankName mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_Supplier.bottom);
        make.left.right.equalTo(self.contentView);
    }];

    _View_SupplierBankAccount = [[UIView alloc]init];
    _View_SupplierBankAccount.backgroundColor = Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_SupplierBankAccount];
    [_View_SupplierBankAccount mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_SupplierBankName.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_EntertainApp mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_SupplierBankAccount.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        ...
        }else if ([model.fieldName isEqualToString:@"SupplierBankName"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateSupplierBankNameViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
        }else if ([model.fieldName isEqualToString:@"SupplierBankAccount"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateSupplierBankAccountViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
...
-(void)updateReimShareView{
    [_ReimShareMainView updateReimShareMainViewWith:self.FormDatas.arr_ShareForm WithData:self.FormDatas.arr_ShareData WithEditType:1 WithComePlace:2];
    __weak typeof(self) weakSelf = self;
...
//MARK:更新供应商银行名称视图
-(void)updateSupplierBankNameViewWithModel:(MyProcurementModel *)model{
    _txf_SupplierBankName = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_SupplierBankName WithContent:_txf_SupplierBankName WithFormType:formViewEnterText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_SupplierBankName addSubview:view];
}
//MARK:更新供应商银行帐号视图
-(void)updateSupplierBankAccountViewWithModel:(MyProcurementModel *)model{
    model.enterLimit = 100;
    _txf_SupplierBankAccount = [[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_SupplierBankAccount WithContent:_txf_SupplierBankAccount WithFormType:formViewEnterText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_SupplierBankAccount addSubview:view];
    _txf_SupplierBankAccount.keyboardType = UIKeyboardTypeEmailAddress;
}
//MARK:更新业务招待视图
...
-(void)clearPersonalData{
    ...
    self.FormDatas.str_InvLoanAmount = @"0";
...
-(void)SupplierClick{
    ...
    vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
        ...
        weakSelf.txf_SupplierBankName.text = model.depositBank;
        weakSelf.txf_SupplierBankAccount.text = model.bankAccount;
    };
    [self.navigationController pushViewController:vc animated:YES];
}
//MARK:修改业务招待单
...
-(void)BorrowingFormClick{
    ...
    choose.ChooseBorrowFormBlock = ^(NSMutableArray *array, NSString *reversalType) {
        ...
        weakSelf.FormDatas.str_InvLoanAmount = @"0";
        for (ChooseCateFreModel *model in array) {
            [name addObject:[GPUtils getSelectResultWithArray:@[[NSString stringWithFormat:@"%@",model.serialNo],[NSString stringWithIdOnNO:model.reason]] WithCompare:@"/"]];
            [Id addObject:[NSString stringWithIdOnNO:model.taskId]];
            weakSelf.FormDatas.str_LoanTotalAmount = [GPUtils decimalNumberAddWithString:model.amount with:weakSelf.FormDatas.str_LoanTotalAmount];
            weakSelf.FormDatas.str_InvLoanAmount = [GPUtils decimalNumberAddWithString:model.originalCurrencyAmt with:weakSelf.FormDatas.str_InvLoanAmount];
        }
        ...
        if ([[GPUtils decimalNumberSubWithString:weakSelf.FormDatas.str_amountTotal with:weakSelf.FormDatas.str_LoanTotalAmount]floatValue]<0) {
...
-(void)configModelOtherData{
    ...
    self.FormDatas.SubmitData.SupplierBankName = _txf_SupplierBankName.text;
    self.FormDatas.SubmitData.SupplierBankAccount = _txf_SupplierBankAccount.text;
