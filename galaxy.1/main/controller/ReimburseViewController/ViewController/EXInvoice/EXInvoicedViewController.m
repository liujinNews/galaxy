...
@interface EXInvoicedViewController ()<UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,GPClientDelegate>
...
@implementation EXInvoicedViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    if (_int_Type == 1) {
         [self setTitle:Custing(@"扫描结果", nil) backButton:YES];
    }else{
...
//创建失败视图
-(void)createErrorView:(NSInteger)type{
    ...
    UILabel *labs = [GPUtils createLable:CGRectMake(0, 110, Main_Screen_Width, 40) text:type==2?Custing(@"无法获取超过一年的发票信息", nil):Custing(@"发票信息会延迟一天,请开票次日扫描", nil) font:Font_Important_15_20 textColor:Color_form_TextField_20 textAlignment:NSTextAlignmentCenter];
...
-(void)updateMainView{
    if (_int_Type == 1) {
        ...
        if ([_dic_baiwang[@"isExpense"] integerValue]==1) {
            ...
            if ([[NSString stringWithIdOnNO:_dic_baiwang[@"taxNoValid"]] isEqualToString:@"0"]||[[NSString stringWithIdOnNO:_dic_baiwang[@"taxNoValid"]] isEqualToString:@"2"]) {
        ...
        _exv_view.lab_purchaserName.text = [NSString stringWithIdOnNO:_dic_requestBwInvoice[@"gmF_MC"]];
        _exv_view.lab_purchaserTaxNo.text = [NSString stringWithIdOnNO:_dic_requestBwInvoice[@"gmF_NSRSBH"]];
        _exv_view.lab_salesName.text = [NSString stringWithIdOnNO:_dic_requestBwInvoice[@"xsF_MC"]];
        _exv_view.lab_totalAmount.text = [GPUtils transformNsNumber:_dic_requestBwInvoice[@"jshj"]];
        _exv_view.lab_shuier.text =  [GPUtils transformNsNumber:_dic_requestBwInvoice[@"hjse"]];
        _exv_view.lab_billingDate.text = [NSString stringWithIdOnNO:_dic_requestBwInvoice[@"kprq"]];
        _exv_view.lab_invoiceCode.text = [NSString stringWithIdOnNO:_dic_requestBwInvoice[@"fP_DM"]];
        _exv_view.lab_invoiceNumber.text = [NSString stringWithIdOnNO:_dic_requestBwInvoice[@"fP_HM"]];
        _exv_view.lab_invoice_type.text= Custing(@"增值税专用发票", nil);
        _exv_view.img_state_img.image=nil;
        _exv_view.img_State.image=nil;
        _exv_view.lab_title_State.text=nil;
        
        _img_category.image = [UIImage imageNamed:[NSString isEqualToNull:_dic_requestBwInvoice[@"expenseIcon"]]?_dic_requestBwInvoice[@"expenseIcon"]:@"15"];
        _str_category = [NSString isEqualToNull:_dic_requestBwInvoice[@"expenseType"]]?_dic_requestBwInvoice[@"expenseType"]:@"";
        _str_expenseCode = _dic_requestBwInvoice[@"expenseCode"];
        _str_expenseIcon = _dic_requestBwInvoice[@"expenseIcon"];
        _str_expenseCat = _dic_requestBwInvoice[@"expenseCat"];
        _str_expenseCatCode = _dic_requestBwInvoice[@"expenseCatCode"];
        _str_expenseTag = @"";
...
-(void)updateBottonView{
    ...
    if (_int_Type == 1) {
        UIButton *addAgain =[GPUtils createButton:CGRectMake(0, 0, ScreenRect.size.width*3/7, 50)action:@selector(btn_Click:) delegate:self];
        addAgain.tag = 1;
        addAgain.backgroundColor =Color_Orange_Weak_20;
        [addAgain setTitle:Custing(@"保存并再扫描", nil)  forState:UIControlStateNormal];
        addAgain.titleLabel.font=Font_filterTitle_17 ;
        [addAgain setTitleColor:Color_form_TextFieldBackgroundColor forState:UIControlStateNormal];
        [_view_botton addSubview:addAgain];
        
        UIButton *addSave=[GPUtils createButton:CGRectMake(ScreenRect.size.width*3/7, 0, ScreenRect.size.width*4/7, 50)action:@selector(btn_submit:) delegate:self];
        addSave.tag = 0;
        ...
        if ([[NSString stringWithIdOnNO:_dic_baiwang[@"isExpense"]] isEqualToString:@"1"]||[[NSString stringWithFormat:@"%@",_dic_baiwang[@"hasSensitiveWord"]] isEqualToString:@"1"]||[[NSString stringWithFormat:@"%@",_dic_baiwang[@"salesNameHasSensitiveWord"]] isEqualToString:@"1"]||[[NSString stringWithIdOnNO:_dic_baiwang[@"taxNoValid"]] isEqualToString:@"0"]||[[NSString stringWithIdOnNO:_dic_baiwang[@"taxNoValid"]] isEqualToString:@"2"]) {
            addAgain.userInteractionEnabled = NO;
            addAgain.backgroundColor = Color_GrayLight_Same_20;
            [addAgain setTitleColor:Color_GrayDark_Same_20 forState:UIControlStateNormal];
        ...
        addSave.tag = 0;
        addSave.backgroundColor = Color_Blue_Important_20;
        [addSave setTitle:Custing(@"导入", nil)  forState:UIControlStateNormal];
        addSave.titleLabel.font=Font_filterTitle_17 ;
        [addSave setTitleColor:Color_form_TextFieldBackgroundColor forState:UIControlStateNormal];
        [_view_botton addSubview:addSave];
    }
...
-(void)requestBaiWCloudCheckInv{
    NSArray *arr = [_str_result componentsSeparatedByString:@","];
    if (arr.count>0&&arr.count>=6) {
        NSString *date1 = [NSDate dateWithstringByLine:arr[5]];
        NSDate *pickerDate = [NSDate date];
        NSDateFormatter * pickerFormatter = [[NSDateFormatter alloc]init];
        [pickerFormatter setDateFormat:@"yyyy-MM-dd"];
        NSString *date2 = [pickerFormatter stringFromDate:pickerDate];
        
        if ([[NSDate DateDuringFromTime:date1 to:date2 WithFormat:@"yyyy-MM-dd" WithDuringType:2] floatValue]>365) {

            [self createErrorView:2];


        }else{
            [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
            NSString *checkCode = [NSString stringWithId:arr[6]];
            NSDictionary *parameters=@{@"invoiceCode":arr[2],
                                       @"invoiceNumber":arr[3],
                                       @"billingDate":[NSDate dateWithstringByLine:arr[5]],
                                       @"checkCode":[NSString isEqualToNull:checkCode]?checkCode.length>=6?[checkCode substringFromIndex:checkCode.length-6]:@"":@"",
                                       @"totalAmount":[NSString isEqualToNull:arr[4]]?[NSString stringWithFormat:@"%.2f",[arr[4] floatValue]]:@"0.00"};
            NSString *url=[NSString stringWithFormat:@"%@",BaiWCloudCheckInv];
            [[GPClient shareGPClient]REquestByPostWithPath:url Parameters:parameters Delegate:self SerialNum:0 IfUserCache:NO];

        }
    }else{
        
        [self createErrorView:1];
    }
}

-(void)requestBaiWangGetInvoiceDetail{
    [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
    NSDictionary *parameters=@{@"AccountNo":_str_AccountNo,
                               @"AccountType":_str_AccountType,
                               @"fplx":_str_fplx,
                               @"FP_DM":[NSString stringWithIdOnNO:_dic_bwInvoice[@"fP_DM"]],
                               @"FP_HM":[NSString stringWithIdOnNO:_dic_bwInvoice[@"fP_HM"]]};
    NSString *url=[NSString stringWithFormat:@"%@",BaiWangGetInvoiceDetail];
...
-(void)requestaddRequestAddCostList{
    if (_int_Type == 1) {
        ...
        NSMutableDictionary *cateDic = [NSMutableDictionary dictionaryWithDictionary:@{@"fP_DM":_dic_requestBwInvoice[@"fP_DM"],@"fP_HM":_dic_requestBwInvoice[@"fP_HM"],@"fplx":_str_fplx,@"gmF_MC":_dic_requestBwInvoice[@"gmF_MC"],@"jshj":_dic_requestBwInvoice[@"jshj"]}];
        
        NSDictionary * dict = @{@"AccountType":_str_AccountType,@"AccountNo":_str_AccountNo,@"Type":_str_type,@"Attachments":_str_imageDataString,@"ExpenseCode":_str_expenseCode,@"ExpenseType":_str_category,@"ExpenseIcon":_str_expenseIcon,@"InvItems":@[cateDic],@"ExpenseCat":_str_expenseCat,@"ExpenseCatCode":_str_expenseCatCode};
        [[GPClient shareGPClient]REquestByPostWithPath:url Parameters:@{@"Invoices":[NSString transformToJson:dict]} Delegate:self SerialNum:3 IfUserCache:NO];
    }
    ...
    switch (serialNum) {
        ...
        case 4:{
            if ([responceDic[@"result"] isKindOfClass:[NSDictionary class]]) {
                _dic_requestBwInvoice = responceDic[@"result"];
            }else{
                _dic_requestBwInvoice = [NSDictionary dictionary];
            }
            [self createSureView];
        ...
        case 5:
        {
            ...
            [self createMainView];
            if (_int_Type == 1) {
                [self requestBaiWCloudCheckInv];
            }else if (_int_Type == 2){
                [self requestBaiWangGetInvoiceDetail];
            }
        }
            break;
        case 6:
...
-(void)saveBaiwangInfoWithDict:(NSDictionary *)dict{
    ...
    if ([dict[@"result"]isKindOfClass:[NSArray class]]) {
        for (NSDictionary *dicts in dict[@"result"]) {
            if ([dicts[@"fieldName"]isEqualToString:@"TaxRate"]&&[dicts[@"isShow"]floatValue]==1) {
                _model_NewAddCost.TaxRate = _dic_baiwang[@"taxRate"];
            }else if ([dicts[@"fieldName"]isEqualToString:@"Tax"]&&[dicts[@"isShow"]floatValue]==1){
