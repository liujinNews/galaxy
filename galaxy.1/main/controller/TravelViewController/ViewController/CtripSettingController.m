...
- (void)requestSuccess:(NSDictionary *)responceDic SerialNum:(int)serialNum {
    ...
    switch (serialNum) {
        ...
        case 4:
        {
            ...
            break;
        case 5:
            [self requestPowerMemberAfterAdd];
            break;
            case 6:
            [self deletePowerMember];
            break;
        default:
            break;
    }
...
-(void)dealWithTravelBook{
    ...
    if (![result isKindOfClass:[NSNull class]]) {
        ...
        //如果差旅负责人的数量超过1个，那么会删除数组里面除了第一个的所有数据
        if (_resultArray.count > 1) {
            NSMutableArray *roleNames = [NSMutableArray arrayWithCapacity:42];
            NSMutableArray *userNames = [NSMutableArray arrayWithCapacity:42];
            for (int i = 1; i < _resultArray.count; i ++) {
                NSDictionary *dict = _resultArray[i];
                if ([NSString isEqualToNull:dict[@"userId"]]) {
                [roleNames addObject:dict[@"userId"]];
                }
                if ([NSString isEqualToNull:dict[@"userName"]]) {
...
- (void)deletePowerMember{
    ...
    BOOL isDelete = NO;
    NSString *requestorUserId = @"";
    if (_choosePeopleArray.count >0) {
        NSDictionary *dic = _choosePeopleArray[0];
        requestorUserId = [NSString isEqualToNull:dic[@"requestorUserId"]]?[NSString stringWithFormat:@"%@",dic[@"requestorUserId"]]:@"";
    }
    NSString *newAddUserId = @"";
    if (_resultArray.count >= 2) {
        for (int i = 0 ; i < _resultArray.count; i ++) {
            NSDictionary *dic = _resultArray[i];
            if ([NSString isEqualToNull:dic[@"userId"]]) {
                newAddUserId = [NSString stringWithFormat:@"%@",dic[@"userId"]];
                if ([NSString isEqualToNull:newAddUserId]&&[NSString isEqualToNull:requestorUserId]&&[newAddUserId isEqualToString:requestorUserId]) {
                    isDelete = YES;
                }
            }
        }
    ...
    //如果有多余一个人就删除原来的差旅负责人
    if (isDelete) {
        NSDictionary *dict = _choosePeopleArray[0];
        [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
        NSString *url=[NSString stringWithFormat:@"%@",PowerMemberDelete];
        NSDictionary *parameters =@{@"RoleId":@"8",@"RoleName":dict[@"requestorUserId"],@"UserName":dict[@"requestor"],@"Description":Custing(@"差旅代订人", nil)};
        [[GPClient shareGPClient]REquestByPostWithPath:url Parameters:parameters Delegate:self SerialNum:3 IfUserCache:NO];
    }else{
        [self requestPowerMemberAgain];
    }
