//
//  CalendarMainController.m
//  galaxy
//
//  Created by hfk on 2018/1/15.
...
@interface CalendarMainController ()<GPClientDelegate,FSCalendarDataSource,FSCalendarDelegate,UIGestureRecognizerDelegate,UITableViewDelegate,UITableViewDataSource>
{
    void * _KVOContext;
}
/**
...
@implementation CalendarMainController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTitle:Custing(@"日程", nil) backButton:YES];
...
//    [nextView addSubview:_btn_Today];
    ...
    [AddButton setBackgroundImage:[UIImage imageNamed:@"Reimburse_Add"] forState:UIControlStateNormal];
    [AddButton setBackgroundImage:[UIImage imageNamed:@"Reimburse_Add"] forState:UIControlStateSelected];
...
-(void)MoreClick:(UIButton *)btn{
    if ([KxMenu isShowingInView:self.view]) {
        ...
        [menuItems addObject:[KxMenuItem menuItem:Custing(@"同事日程", nil) image:nil target:self action:@selector(colleagueNotify)]];
        [menuItems addObject:[KxMenuItem menuItem:Custing(@"知会我的日程", nil) image:nil target:self action:@selector(notifyMe)]];
...
//MARK:同事日程
-(void)colleagueNotify{
    ColleagueListController *vc=[[ColleagueListController alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
}
//MARK:知会我的日程
-(void)notifyMe{
    NotifyMeCalendarController *vc=[[NotifyMeCalendarController alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
}
//MARK:增加日程
-(void)AddCalendar:(id)obj{
...
-(void)getMonthData{
    ...
    NSDictionary *Parameters=@{@"ScheduleDate":_str_SelectMonth,@"UserId":self.userdatas.userId};
    [[GPClient shareGPClient]REquestByPostWithPath:url Parameters:Parameters Delegate:self SerialNum:0 IfUserCache:NO];
}
-(void)getDayData{
    [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
    NSString *url=[NSString stringWithFormat:@"%@",GETDAYCaldaner];
    NSDictionary *Parameters=@{@"ScheduleDate":_str_SelectDate,@"UserId":self.userdatas.userId};
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.arr_DayEventInfo&&self.arr_DayEventInfo.count>0) {
        ...
        return [CalendarShowCell cellHeightWithObj:model WithType:1];
    }else if(_bool_NoData){
        return 210;
    }
    return 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.arr_DayEventInfo&&self.arr_DayEventInfo.count>0) {
        ...
        CalendarShowModel * model = (CalendarShowModel *)self.arr_DayEventInfo[indexPath.row];
        [cell configCellWithObj:model WithType:1];
        return cell;
    }else if(_bool_NoData){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.userId floatValue]!=[self.userdatas.userId floatValue]) {
        vc.int_status=3;
    }else{
        vc.int_status=1;
    }
