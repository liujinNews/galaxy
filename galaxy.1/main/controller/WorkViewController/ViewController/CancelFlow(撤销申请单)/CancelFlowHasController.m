...
@interface CancelFlowHasController ()

@end

@implementation CancelFlowHasController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas = [[CancelFlowFormData alloc]initWithStatus:2];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        ...
        }else if ([model.fieldName isEqualToString:@"FormSerialNo"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1 = 1;
            [self updateCancelFormViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"FlowName"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1 = 1;
...
-(void)updateCancelFormViewWithModel:(MyProcurementModel *)model{
    model.fieldValue = [NSString stringWithIdOnNO:self.FormDatas.str_FormReason];
    [_View_CancelForm addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_CancelForm updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateFlowNameViewWithModel:(MyProcurementModel *)model{
    [_View_FlowName addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_FlowName updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
