//
//  GeneralAppHasSubmitController.m
//  galaxy
//
...
@interface GeneralAppHasSubmitController ()

@end

@implementation GeneralAppHasSubmitController
-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[MyGeneralFormData alloc]initWithStatus:2];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)createMainView{
    ...
    _View_Title=[[UIView alloc]init];
    _View_Title.backgroundColor=Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_Title];
    ...
    [_View_Detail makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_Title.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    _View_DisposalProceeds = [[UIView alloc]init];
    _View_DisposalProceeds.backgroundColor = Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_DisposalProceeds];
    [_View_DisposalProceeds mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_View_Approve makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_DisposalProceeds.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        ...
        }else if ([model.fieldName isEqualToString:@"Title"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateTitleViewWithModel:model];
        ...
        }else if ([model.fieldName isEqualToString:@"DisposalProceeds"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line2 = 1;
            [self updateDisposalProceedsViewWithModel:model];
...
//MARK:更新通用审批标题
-(void)updateTitleViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_Title addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_Title updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateRemarkViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_Detail addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_Detail updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
//MARK:更新处置收益
-(void)updateDisposalProceedsViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_DisposalProceeds addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 IsAmount:1 block:^(NSInteger height) {
        [weakSelf.View_DisposalProceeds updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
