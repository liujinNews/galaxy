...
//    单一采购来源清单
    ...
    [_View_PayWay mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_SoAddDetails.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
- (void)updateAmAddDetailsView{
    SubmitFormView *view = [[SubmitFormView alloc] initAddBtbWithBaseView:_View_AmAddDetails withTitle:Custing(@"增加采购金额明细", nil) withTitleAlignment:1 withImageArray:@[@"commom_addDetails_Icon"] withBtnLocation:1 withlineStyle:0];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model) {
        [weakSelf keyClose];
...
- (void)updateBuDetailsTableView{
    ...
    [_View_purBusinessTable updateConstraints:^(MASConstraintMaker *make) {
        if (weakSelf.FormDatas.bool_ThirdHaveAttchs) {
            make.height.equalTo(@((self.FormDatas.arr_FouDetailsArray.count*42+76+27)*self.FormDatas.arr_ThirDetailsDataArray.count));
        }else{
            make.height.equalTo(@((self.FormDatas.arr_FouDetailsArray.count*42+27)*self.FormDatas.arr_ThirDetailsDataArray.count));
        }
    }];
...
- (void)updateBuAddDetailsView{
    SubmitFormView *view = [[SubmitFormView alloc] initAddBtbWithBaseView:_View_BuAddDetails withTitle:Custing(@"增加采购内容明细", nil) withTitleAlignment:1 withImageArray:@[@"commom_addDetails_Icon"] withBtnLocation:1 withlineStyle:0];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model) {
        [weakSelf keyClose];
...
- (void)updateSoDetailsTableView{
    if (self.FormDatas.arr_FouDetailsDataArray.count == 0) {
        DeatilsModel *model = [[DeatilsModel alloc] init];
        model.No = @"1";
        [self.FormDatas.arr_FouDetailsDataArray addObject:model];
    }
    __weak typeof(self) weakSelf = self;
    [_View_purSourceTable updateConstraints:^(MASConstraintMaker *make) {
        if (weakSelf.FormDatas.bool_FourthHaveAttchs) {
            make.height.equalTo(@((self.FormDatas.arr_FouDetailsArray.count*42+76+27)*self.FormDatas.arr_FouDetailsDataArray.count));
        }else{
            make.height.equalTo(@((self.FormDatas.arr_FouDetailsArray.count*42+27)*self.FormDatas.arr_FouDetailsDataArray.count));
        }
    }];
    [_View_purSourceTable reloadData];
}
//MARK:更新单一采购来源清单增加按钮
- (void)updateSoAddDetailsView{
    SubmitFormView *view = [[SubmitFormView alloc] initAddBtbWithBaseView:_View_SoAddDetails withTitle:Custing(@"增加单一采购来源清单", nil) withTitleAlignment:1 withImageArray:@[@"commom_addDetails_Icon"] withBtnLocation:1 withlineStyle:0];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model) {
        [weakSelf keyClose];
        DeatilsModel *model4 = [[DeatilsModel alloc] init];
        [weakSelf.FormDatas.arr_FouDetailsDataArray addObject:model4];
        model4.No = [NSString stringWithFormat:@"%lu",(unsigned long)weakSelf.FormDatas.arr_FouDetailsDataArray.count];
        [weakSelf updateSoDetailsTableView];
    }];
    [_View_SoAddDetails addSubview:view];
}
...
//                    tf.text = Model.Type;
        ...
        return cell;
    }else if (tableView == _View_purBusinessTable){//采购内容明细
...
- (void)CreateAmHeadViewWithSection:(NSInteger)section{
    ...
    if (self.FormDatas.arr_SecDetailsDataArray.count == 1) {
        titleLabel.text = Custing(@"采购金额明细", nil);
    }else{
        titleLabel.text = [NSString stringWithFormat:@"%@(%ld)",Custing(@"采购金额明细", nil),(long)section+1];
        if (section != 0) {
...
- (void)AmDeleteDetails:(UIButton *)btn{
    [self keyClose];
    NSLog(@"删除金额明细");
    UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"" message:Custing(@"删除金额明细", nil) preferredStyle:UIAlertControllerStyleAlert];
    __weak typeof(self) weakSelf = self;
...
- (void)CreateBuHeadViewWithSection:(NSInteger)section{
    ...
    if (self.FormDatas.arr_ThirDetailsDataArray.count == 1) {
        titleLabel.text = Custing(@"采购内容明细", nil);
    }else{
        titleLabel.text = [NSString stringWithFormat:@"%@(%ld)",Custing(@"采购内容明细", nil),(long)section+1];
        if (section != 0) {
...
- (void)BuDeleteDetails:(UIButton *)btn{
    [self keyClose];
    NSLog(@"删除内容明细");
    UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"" message:Custing(@"删除内容明细", nil) preferredStyle:UIAlertControllerStyleAlert];
    __weak typeof(self) weakSelf = self;
    UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:Custing(@"删除", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)SoDeleteDetails:(UIButton *)btn{
    [self keyClose];
    NSLog(@"删除单一采购来源清单");
    UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"" message:Custing(@"删除单一采购来源清单", nil) preferredStyle:UIAlertControllerStyleAlert];
    __weak typeof(self) weakSelf = self;
    UIAlertAction *deleteAction = [UIAlertAction actionWithTitle:Custing(@"删除", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)dealWithBuImagesArray:(NSMutableArray *)imgTotalArray withIndexThird:(NSInteger)indexThird {
    ...
    [[VoiceDataManger sharedManager]uploadImageDataWithImgSoure:imgTotalArray WithUrl:ProcurementLoadImage WithBlock:^(id data, BOOL hasError) {
        NSLog(@"indexThird==%ld,data==%@",indexThird,data);
        if (hasError) {
            weakSelf.dockView.userInteractionEnabled=YES;
            [[GPAlertView sharedAlertView]showAlertText:weakSelf WithText:data duration:2.0];
        }else{
...
- (void)dealWithSoImagesArray:(NSMutableArray *)imgTotalArray withIndexFourth:(NSInteger)indexFourth{
    ...
    __weak typeof(self) weakSelf = self;
    [[VoiceDataManger sharedManager]uploadImageDataWithImgSoure:imgTotalArray WithUrl:ProcurementLoadImage WithBlock:^(id data, BOOL hasError) {
        NSLog(@"indexFouth==%ld,data==%@",indexFourth,data);
        if (hasError) {
            weakSelf.dockView.userInteractionEnabled=YES;
            [[GPAlertView sharedAlertView]showAlertText:weakSelf WithText:data duration:2.0];
        }else{
