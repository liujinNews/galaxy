//
//  TravelCarChooseAddressController.m
//  galaxy
//
...
@interface TravelCarChooseAddressController ()<PlaceAroundTableViewDeleagate,MAMapViewDelegate,UISearchBarDelegate>
...
@implementation TravelCarChooseAddressController

- (void)viewDidLoad {
    [super viewDidLoad];
    NSString *title = @"";
    if (self.type == 1) {
        title = Custing(@"出发地", nil);
    }else{
        title = Custing(@"目的地", nil);
    }
...
- (void)initTableview{
    self.tableview = [[PlaceAroundTableView alloc] initWithFrame:CGRectMake(0, 49, Main_Screen_Width, Main_Screen_Height - 49 - NavigationbarHeight)];
...
- (void)initMapView{
    self.mapView = [[MAMapView alloc] initWithFrame:CGRectMake(0, 49, 0, 0)];
