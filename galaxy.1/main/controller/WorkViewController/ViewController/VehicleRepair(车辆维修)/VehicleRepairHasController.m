...
@interface VehicleRepairHasController ()

@end

@implementation VehicleRepairHasController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[VehicleRepairFormData alloc]initWithStatus:2];
    }
    return self;
}
...
-(void)createMainView{
    ...
    [_View_CarSize makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_View_LoadCapacity makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_CarSize.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_Mileage makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_LoadCapacity.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        ...
        }else if ([model.fieldName isEqualToString:@"CarSize"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateCarSizeViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"LoadCapacity"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
...
-(void)updateCarSizeViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_CarSize addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_CarSize updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateLoadCapacityViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_LoadCapacity addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_LoadCapacity updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateDetailsTableView{
    if (self.FormDatas.bool_isOpenDetail) {
        NSInteger height=10;
        for (VehicleRepairDeatil *model in self.FormDatas.arr_DetailsDataArray) {
            height=height+[ProcureDetailsCell VehicleRepairCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
        }
        [_View_DetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }else{
        VehicleRepairDeatil *model=self.FormDatas.arr_DetailsDataArray[0];
        NSInteger height=10+[ProcureDetailsCell VehicleRepairCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
        [_View_DetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView==_View_DetailsTable) {
        VehicleRepairDeatil *model=self.FormDatas.arr_DetailsDataArray[indexPath.row];
        return [ProcureDetailsCell VehicleRepairCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
    }else{
        return 0;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView==_View_DetailsTable) {
        ...
        [cell configVehicleRepairCellWithArray:self.FormDatas.arr_DetailsArray withDetailsModel:self.FormDatas.arr_DetailsDataArray[indexPath.row] withindex:indexPath.row withCount:self.FormDatas.arr_DetailsDataArray.count] ;
        if (cell.LookMore) {
