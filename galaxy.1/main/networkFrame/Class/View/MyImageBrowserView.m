...
@interface MyImageBrowserView() <UIScrollViewDelegate>
...
@property (nonatomic, strong) MyMsgPicModel* model;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
- (id)initWithPicModel:(MyMsgPicModel *)picModel thumbnail:(UIImage*)thumbnail fromRect:(CGRect)rect
{
...
- (void)showImageViewAnimation
{
    ...
    if (self.thumbnail) {
        ...
        if (finalFrame.size.height > self.ttheight) {
            self.scrollView.contentSize = CGSizeMake(self.ttwidth, finalFrame.size.height);
        }
        ...
        [UIView animateWithDuration:PREVIEW_ANIMATION_DURATION animations:^{
            ...
            if (self.model) {
                TIMImageElem* elem = (TIMImageElem *)self.model.elem;
...
//    self.progressIndicator.center = CGPointMake(self.size.width / 2, self.size.height / 2);
    self.saveBtn.ttleft = BUTTON_SIDE_MARGIN;
    self.saveBtn.ttbottom = self.ttheight - BUTTON_SIDE_MARGIN;
...
- (CGRect)calculateScaledFinalFrame
{
    ...
    CGFloat finalHeight = self.ttwidth * (thumbSize.height / thumbSize.width);
    CGFloat top = 0.f;
    if (finalHeight < self.ttheight) {
        top = (self.ttheight - finalHeight) / 2.f;
    }
    return CGRectMake(0.f, top, self.ttwidth, finalHeight);
}
