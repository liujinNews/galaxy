...
#import <pop/POP.h>
#define MenuButtonHeight 115
#define MenuButtonVerticalPadding 10
#define MenuButtonHorizontalMargin 0
...
@property (nonatomic, strong) PopMenuItem *selectedItem;
...
- (void)setup {
    ...
    _footerView=[[UIView alloc] initWithFrame:CGRectMake(0, Main_Screen_Height-48, Main_Screen_Width, 48)];
}
...
- (void)showButtons {
    ...
    /**
     获得按钮的宽
     @param self.bounds 当前屏幕宽
     @param (perRowItemCount + 1) * MenuButtonHorizontalMargin 按钮直接间隙
     @return 按钮的宽
     */
    CGFloat menuButtonWidth =Main_Screen_Width>(50+65)*3?50+65:105;
    
    typeof(self) __weak weakSelf = self;
    for (int index = 0; index < items.count; index ++) {
        PopMenuItem *menuItem = items[index];
        // 如果没有自定义index，就按照正常流程，从0开始
        if (menuItem.index < 0) {
            menuItem.index = index;
        }
        PopMenuButton *menuButton = (PopMenuButton *)[self viewWithTag:kMenuButtonBaseTag + index];
        ...
        if (!menuButton) {
            menuButton = [[PopMenuButton alloc] initWithFrame:fromRect menuItem:menuItem];
            menuButton.tag = kMenuButtonBaseTag + index;
            menuButton.didSelctedItemCompleted = ^(PopMenuItem *menuItem) {
...
- (CGRect)getFrameWithItemCount:(NSInteger)itemCount
    ...
    CGFloat insetY = (Main_Screen_Height-(itemHeight+paddingY)*2)/2;
    NSLog(@"%f",Main_Screen_Width);
    CGFloat originX =(Main_Screen_Width-perRowItemCount*itemWidth)/2+(index % perRowItemCount) * (itemWidth + paddingX) + paddingX + (page * CGRectGetWidth(self.bounds));
