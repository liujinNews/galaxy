...
- (id)init{
    if ((self = [super init])) {
        ...
        _tapView.backgroundColor = Color_Unsel_TitleColor;
        ...
        _photoImageView.backgroundColor = Color_Unsel_TitleColor;
    ...
    if (buttonIndex == 0){
        if([UIImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypeSavedPhotosAlbum]) {
            ...
            if (_photoImageView.image) {
                [self showMessageWithText:Custing(@"保存成功",nil)];
            }
        }else{
            if (_photoImageView.image) {
...
//        NSLog(@"scroller contentsize %@",NSStringFromCGSize(self.contentSize));
...
//        self.minimumZoomScale = 1.0;
        ...
        // Initial zoom
        self.maximumZoomScale = 1.0*2;
        self.minimumZoomScale = minScale;
    }else {
        self.maximumZoomScale = yScale*2;
...
- (void)layoutSubviews {
    ...
    CGSize boundsSize = self.bounds.size;
//    CGSize boundsSize = CGSizeMake(Main_Screen_Width*2, Main_Screen_Height*2);
...
//    NSLog(@"max %f  min %f , bouncesZoom %f",self.maximumZoomScale,self.minimumZoomScale,self.bouncesZoom);
}
...
//    NSLog(@"end max %f  min %f bouncesZoom %f,imageview freme %@,scrollview frame %@ and contentsize %@",self.maximumZoomScale,self.minimumZoomScale,self.zoomScale,NSStringFromCGRect(_photoImageView.frame),NSStringFromCGRect(scrollView.frame),NSStringFromCGSize(scrollView.contentSize));
    [self setNeedsLayout];
    [self layoutIfNeeded];
}
...
- (void)handleDoubleTap:(CGPoint)touchPoint {
    ...
    if (self.zoomScale != self.minimumZoomScale && self.zoomScale != [self initialZoomScaleWithMinScale]) {
        ...
        if (self.isLoadingDone) {
            ...
            CGFloat newZoomScale = ((self.maximumZoomScale/2 + self.minimumZoomScale) / 1.5);
