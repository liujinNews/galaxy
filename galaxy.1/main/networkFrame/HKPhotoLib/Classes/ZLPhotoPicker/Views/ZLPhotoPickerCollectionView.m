...
- (BOOL)validatePhotoCount:(NSInteger)maxCount{
    if (self.selectAssets.count >= maxCount) {
        ...
        NSString *format = [NSString stringWithFormat:@"%@%zd%@",Custing(@"最多只能选择", nil),maxCount,Custing(@"张图片",nil)];
        if (maxCount == 0) {
            format = [NSString stringWithFormat:@"%@",Custing(@"您已经选满了图片了.", nil)];
        }
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:Custing(@"提醒", nil) message:format delegate:self cancelButtonTitle:nil otherButtonTitles:Custing(@"好的", nil), nil];
        [alertView show];
        return NO;
    }
    return YES;
