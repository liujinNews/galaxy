...
//    [_pageControl addTarget:self action:@selector(changePage:) forControlEvents:UIControlEventValueChanged];
    ...
    _sentView = [[UIView alloc]init];
    _sentView.backgroundColor = [GPUtils colorHString:@"#ededed"];
    [self addSubview:_sentView];
    
    _sentBtn = [[UIButton alloc]init];
    [_sentBtn setTitle:Custing(@"发送", nil) forState:UIControlStateNormal];
    [_sentBtn addTarget:self action:@selector(btn_Click)  forControlEvents:UIControlEventTouchUpInside];
    _sentBtn.titleLabel.font = Font_Important_15_20;
    [_sentBtn setTitleColor:Color_form_TextFieldBackgroundColor forState:UIControlStateNormal];
    [_sentBtn setBackgroundColor:Color_Blue_Important_20];
    [self addSubview:_sentBtn];
}

- (void)btn_Click
{
    [self.PageClick_Delegate sentBtn_Click:1];
//    NSNotificationCenter *center = [NSNotificationCenter defaultCenter];
    //添加通知
//    [center addObserver:[[RichChatInputToolBar alloc]init] selector:@selector(emoj_Click:) name:@"emoj" object:nil];
//    [center addObserver:[[RichChatInputToolBar alloc]init] selector:nil name:@"emoj" object:nil];
//    //发送通知
//    [center postNotificationName:@"emoj"
//                          object:self
//                        userInfo:nil];
}
