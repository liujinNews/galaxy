...
//扫码区域各种参数
@property (nonatomic, strong) JQScanViewStyle* viewStyle;
...
//线条扫码动画封装
@property (nonatomic,strong)JQScanLineAnimation *scanLineAnimation;
//网格扫码动画封装
@property (nonatomic,strong)JQScanNetAnimation *scanNetAnimation;
...
- (void)startDeviceReadyingWithText:(NSString*)text
{
    ...
    if (!_activityView)
    {
        ...
        _labelReadying.textColor  = Color_form_TextFieldBackgroundColor;
    ...
    switch (_viewStyle.anmiationStyle)
    {
        case LBXScanViewAnimationStyle_LineMove:
        {
            ...
            if (!_scanLineAnimation)
                self.scanLineAnimation = [[JQScanLineAnimation alloc]init];
        ...
        case LBXScanViewAnimationStyle_NetGrid:
        {
            ...
            if (!_scanNetAnimation)
                self.scanNetAnimation = [[JQScanNetAnimation alloc]init];
