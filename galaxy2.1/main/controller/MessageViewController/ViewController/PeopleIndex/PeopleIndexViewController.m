...
@property (weak, nonatomic) IBOutlet UILabel *lab_xibao;
...
//    _arr_people_request = dic_people_request[@"dic"];
    ...
    _lab_Phone.text = Custing(@"手机通讯录", nil);
    _lab_xibao.text = Custing(@"喜报好友", nil);
...
//    [self.organiBtn setBackgroundColor:Color_form_TextFieldBackgroundColor];
    ...
    [self.lab_Phone setTextColor:Color_form_TextField_20];
    [self.lab_ConpanyName setTextColor:Color_form_TextField_20];
    [self.lab_xibao setTextColor:Color_form_TextField_20];
...
- (IBAction)btn_contacts_click:(id)sender {
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 9.0) {
        ...
        switch (ABstatus) {
            case kABAuthorizationStatusAuthorized:
            {
                ...
                RDVTabBarController *nav = (RDVTabBarController *)window.rootViewController;
                UINavigationController *navi = nav.viewControllers[0];
                [navi pushViewController:contacts animated:YES];
...
- (IBAction)btn_xibao_click:(id)sender {
    
    XibaoFriendViewController *xibao = [[XibaoFriendViewController alloc]init];
    UIWindow *window=[[[UIApplication sharedApplication] delegate]window];
    RDVTabBarController *nav = (RDVTabBarController *)window.rootViewController;
    UINavigationController *navi = nav.viewControllers[0];
    [navi pushViewController:xibao animated:YES];
