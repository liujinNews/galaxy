...
@property(nonatomic, strong)NSString * purSendMailOfApproval;
...
@property(nonatomic,strong)UISwitch * sw_Approval;
...
-(void)createMainView{
    ...
    NSArray * setArr = @[
                         ...
                         @{@"image":[NSString stringWithFormat:@"%@",self.rejected],@"name":Custing(@"我参与的流程审批完成", nil),@"tag":@"8"}
    ...
    for (int j = 0 ; j < [setArr count] ; j ++ ) {
        ...
        self.chooseBtn = [GPUtils createLable:CGRectMake(15, j*self.mainHeight/8, WIDTH(self.headerView)-30, self.mainHeight/8) text:[[setArr objectAtIndex:j]  objectForKey:@"name"] font:Font_Important_15_20 textColor:Color_GrayDark_Same_20 textAlignment:NSTextAlignmentLeft];
        [self.mainView  addSubview:self.chooseBtn];
        if (j!=setArr.count-1) {
            UIView * line = [[UIView alloc]initWithFrame:CGRectMake(15, j*self.mainHeight/8+self.mainHeight/8, WIDTH(self.headerView)-30, 0.5)];
            line.backgroundColor = Color_GrayLight_Same_20;
    ...
    self.newtaskImage = [[UISwitch alloc]initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16-HEIGHT(nicai)/2, 0,0)];
    ...
    self.finishImage = [[UISwitch alloc]initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16*3-HEIGHT(nicai)/2, 0, 0)];
    ...
    self.returnImage = [[UISwitch alloc]initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16*5-HEIGHT(nicai)/2, 0, 0)];
    ...
    self.rejectImage = [[UISwitch alloc]initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16*7-HEIGHT(nicai)/2, 0, 0)];
    ...
    self.payImage = [[UISwitch alloc]initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16*9-HEIGHT(nicai)/2, 0, 0)];
    ...
    self.sw_RedirectMe = [[UISwitch alloc]initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16*11-HEIGHT(nicai)/2, 0, 0)];
    ...
    self.sw_CCMe = [[UISwitch alloc]initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16*13-HEIGHT(nicai)/2, 0, 0)];
    ...
    self.sw_Approval = [[UISwitch alloc] initWithFrame:CGRectMake(WIDTH(self.mainView)-15-WIDTH(nicai), self.mainHeight/16*15-HEIGHT(nicai)/2, 0, 0)];
    self.sw_Approval.backgroundColor = [UIColor clearColor];
    if ([self.purSendMailOfApproval isEqualToString:release]) {
        [self.sw_Approval setOn:[self.purSendMailOfApproval boolValue] animated:NO];
    }else{
        [self.sw_Approval setOn:[self.purSendMailOfApproval boolValue] animated:NO];
    }
    self.sw_Approval.tag = 110;
    [self.sw_Approval addTarget:self action:@selector(chooseSettingBtn:) forControlEvents:UIControlEventValueChanged];
    self.sw_Approval.onTintColor = Color_Blue_Important_20;
    [self.mainView addSubview:self.sw_Approval];
}
...
-(void)chooseSettingBtn:(UISwitch *)btn{
    ...
    switch (btn.tag) {
        ...
        {
            if ([self.purSendMailOfApproval isEqualToString:release]) {
                self.purSendMailOfApproval = @"1";
                [self requestSaveMessage];
                [self.sw_Approval setOn:[self.purSendMailOfApproval boolValue] animated:NO];
            }else{
                self.purSendMailOfApproval = @"0";
                [self requestSaveMessage];
                [self.sw_Approval setOn:[self.purSendMailOfApproval boolValue] animated:NO];
            }
        }
...
//                                  @"operationType":[NSString stringWithFormat:@"%@",self.operationType],
                                  ...
                                  @"purSendMailOfApproval":[NSString stringWithFormat:@"%@",self.purSendMailOfApproval]
                                  };
...
//    }
    ...
    if (serialNum ==0) {
        ...
        if (![result isKindOfClass:[NSNull class]] && result != nil && result.count != 0){
            ...
            self.purSendMailOfApproval = [NSString stringWithFormat:@"%@",[result objectForKey:@"purSendMailOfApproval"]];
        }
    }
