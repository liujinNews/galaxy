...
@interface BusiImportViewController ()
...
@implementation BusiImportViewController
...
- (void)requestSuccess:(NSDictionary *)responceDic SerialNum:(int)serialNum {
    ...
    switch (serialNum) {
        ...
        case 2:
            ...
            break;
        case 3:
            if ([responceDic[@"success"] intValue] == 1) {
//                [self.navigationController popViewControllerAnimated:YES];
                [[GPAlertView sharedAlertView]showAlertText:self WithText:Custing(@"上传成功", nil) duration:2.0];
                [self performSelector:@selector(goNavBack) withObject:self afterDelay:2.0];
            }
            break;
        default:
            break;
...
//    NSDictionary *parameter;
    NSDictionary *parameter = [self dicWithModel];
//    parameter = @{@"invs":[self transformToJson:dic]};
    NSString *url=[NSString stringWithFormat:@"%@",ImportAddNote];
    [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
    [[GPClient shareGPClient]REquestByPostWithPath:url Parameters:parameter Delegate:self SerialNum:3 IfUserCache:NO];
}
...
- (NSDictionary *)dicWithModel{
    if (self.isMultiple == NO) {
        ...
        [self.resultArray addObject:self.importModel];
    }
    ...
    for (int i=0; i<self.chooseArray.count; i++) {
        ...
        if ([str isEqualToString:@"1"]) {
            ...
            [dict setValue:[NSString stringWithFormat:@"%@",self.str_type] forKey:@"type"];
            [dict setValue:@"29" forKey:@"dataSource"];
            [dict setValue:[NSString stringWithIdOnNO:model.tmcOrderID] forKey:@"orderId"];
            [dict setValue:[NSString stringWithIdOnNO:_expenseCat] forKey:@"expenseCat"];
            [dict setValue:[NSString stringWithIdOnNO:_expenseCatCode] forKey:@"expenseCatCode"];
            [dict setValue:[NSString stringWithIdOnNO:_expenseCode] forKey:@"expenseCode"];
            [dict setValue:[NSString stringWithIdOnNO:_expenseIcon] forKey:@"expenseIcon"];
            [dict setValue:[NSString stringWithIdOnNO:_categoryString] forKey:@"expenseType"];
...
//            [dict setValue:[NSString stringWithIdOnNO:model.tag] forKey:@"tag"];
            [dict setValue:Custing(@"0", nil) forKey:@"tax"];
            [dict setValue:Custing(@"人民币", nil) forKey:@"currency"];
            [dict setValue:Custing(@"CNY", nil) forKey:@"currencyCode"];
            [dict setValue:Custing(@"1.0000", nil) forKey:@"exchangeRate"];
            [dict setValue:[NSString stringWithIdOnNO:model.userId] forKey:@"ownerUserId"];
            [dict setValue:[NSString stringWithIdOnNO:model.userId] forKey:@"requestUserId"];
            [dict setValue:[NSString stringWithIdOnNO:model.userId] forKey:@"userId"];
            [dict setValue:[NSString stringWithIdOnNO:model.amount] forKey:@"exclTax"];
            [dict setValue:[NSString stringWithIdOnNO:model.amount] forKey:@"localCyAmount"];

            if ([[NSString stringWithFormat:@"%@",model.payType]isEqualToString:@"0"]) {//0: 个人支付 1: 企业支付 2: 个人企业支付
                [dict setValue:Custing(@"个人支付", nil) forKey:@"payType"];
                [dict setValue: @"1" forKey:@"payTypeId"];
                [dict setValue:[NSString isEqualToNull:model.personalPay]?[NSString stringWithFormat:@"%@",model.personalPay]:@"0" forKey:@"amount"];
            }else{
                [dict setValue:Custing(@"企业支付", nil) forKey:@"payType"];
                [dict setValue: @"2" forKey:@"payTypeId"];
                [dict setValue:[NSString isEqualToNull:model.companyPay]?[NSString stringWithFormat:@"%@",model.companyPay]:@"0" forKey:@"amount"];
            }
            
            NSString *departureTime = [[NSString stringWithIdOnNO:model.startDate] stringByReplacingOccurrencesOfString:@"-" withString:@"/"];
...
//            NSString *departureDay = departureTime.length >10 ? [departureTime substringToIndex:10]:@"";
            ...
            NSString *finishTime = [[NSString stringWithIdOnNO:model.endDate] stringByReplacingOccurrencesOfString:@"-" withString:@"/"];
...
//            NSString *finishDay = finishTime.length >10 ? [finishTime substringToIndex:10]:@"";
            ...
            if ([[NSString stringWithFormat:@"%@",model.tag]isEqualToString:@"Trans"]) {
                [dict setValue:[NSString stringWithIdOnNO:model.departure] forKey:@"transDCityName"];
                [dict setValue:[NSString stringWithIdOnNO:model.destination] forKey:@"transACityName"];
                ...
                [dict setValue:[NSString stringWithIdOnNO:model.departure] forKey:@"taxiDCityName"];
                [dict setValue:[NSString stringWithIdOnNO:model.destination] forKey:@"taxiACityName"];
                [dict setValue:departureHour forKey:@"taxiFromDate"];
                [dict setValue:finishHour forKey:@"taxiToDate"];
            }
            [dict setValue:departureTime forKey:@"expenseDate"];
