//
//  ReimImportCustomViewController.m
//  galaxy
//
...
@interface ReimImportCustomViewController ()<UISearchBarDelegate,GPClientDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation ReimImportCustomViewController
...
-(void)requestData{
    ...
    NSDictionary *parameter = @{@"FlowCode":self.dict_parameter[@"FlowCode"],
                                @"UserId":self.dict_parameter[@"UserId"],
                                @"OwnerUserId":self.dict_parameter[@"OwnerUserId"],
                                @"CompanyId":self.userdatas.companyId,
                                @"LanguageCode":self.userdatas.language,
                                @"ExpenseType":_searchAim,
                                @"ExpUsers":self.dict_parameter[@"ExpUsers"],
                                @"ExpenseCode":self.dict_parameter[@"ExpenseCode"] ? self.dict_parameter[@"ExpenseCode"]:@"",
                                };
    [[GPClient shareGPClient]REquestByPostWithPath:GETUSERUNSUBMITEXP Parameters:parameter Delegate:self SerialNum:0 IfUserCache:NO];
}
-(void)requestImport{
    ...
    for (NSMutableDictionary *dict in self.resultArray) {
        ...
        for (NSMutableDictionary *data in arr) {
            if ([data[@"checked"]floatValue] == 1) {
                NSDictionary *dict1 = @{@"Id":[NSString stringIsExist:data[@"id"]],
                                        @"Type":[NSString stringIsExist:data[@"type"]],
    ...
    [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
    NSDictionary *Input = @{@"FlowCode":self.dict_parameter[@"FlowCode"],
                            @"UserId":self.dict_parameter[@"UserId"],
                            @"CompanyId":self.userdatas.companyId,
                            @"LanguageCode":self.userdatas.language,
                            @"ExpenseCode":self.dict_parameter[@"ExpenseCode"] ? self.dict_parameter[@"ExpenseCode"]:@"",
    ...
    [[GPClient shareGPClient]REquestByPostWithPath:IMPORTUSERUNSUBMITEXP Parameters:@{@"Input":[NSString transformToJson:Input]} Delegate:self SerialNum:1 IfUserCache:NO];
}
...
- (void)requestSuccess:(NSDictionary *)responceDic SerialNum:(int)serialNum {
    ...
    switch (serialNum) {
        ...
        case 1:
        {
            if ([responceDic[@"result"]isKindOfClass:[NSArray class]]&&self.importCustomBackBlock) {
                self.importCustomBackBlock(responceDic[@"result"]);
            }
...
-(void)createNOdataView{
    [_tableView configBlankPage:EaseBlankNormalView hasTips:Custing(@"您还没有费用明细哦", nil) hasData:(self.resultArray.count!=0) hasError:NO reloadButtonBlock:nil];
}
/*
