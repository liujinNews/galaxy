...
-(void)createMainView{
    ...
    [_View_FormScope mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_BusinessOwner.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        ...
        }else if([model.fieldName isEqualToString:@"FormScope"]){//单据可见范围
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
...
- (NSMutableArray *)arr_FormScope{
    if (!_arr_FormScope) {
        ...
        NSArray *idArray = [NSArray arrayWithObjects:@"1",@"2", nil];
        NSArray *typeArray = [NSArray arrayWithObjects:Custing(@"相同成本中心内均可见", nil),Custing(@"全公司可见", nil), nil];
        for (int i = 0 ; i < 2; i ++) {
            STOnePickModel *model = [[STOnePickModel alloc] init];
            model.Id = idArray[i];
