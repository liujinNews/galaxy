                ...
                if ([formData[@"paymentExpDetailDaTa"] isKindOfClass:[NSDictionary class]]) {
                    ...
                    if ([paymentExpDetailDaTa[@"sa_PaymentExpDetail"] isKindOfClass:[NSArray class]]) {
                        for (NSDictionary *dict in paymentExpDetailDaTa[@"sa_PaymentExpDetail"]) {
                            ...
                            [model setValuesForKeysWithDictionary:dict];
                            if (![dict[@"accruedList"] isKindOfClass:[NSNull class]]) {
                                NSArray *accruedArr = (NSArray *)[NSString transformToObj:[NSString stringWithFormat:@"%@",dict[@"accruedList"]]];
                                self.arr_SecDetailsDataArray = [self createAccruedArrWithArr:accruedArr];
                            }
...
//            if ([formData[@"paymentExpDetailDaTa"] isKindOfClass:[NSDictionary class]]) {
//                NSDictionary *paymentExpDetailDaTa = formData[@"paymentExpDetailDaTa"];
//                if ([paymentExpDetailDaTa[@"Sa_PaymentExpDetailRelevanceAccruedExpDetail"] isKindOfClass:[NSArray class]]) {
//                    for (NSDictionary *dict in paymentExpDetailDaTa[@"Sa_PaymentExpDetailRelevanceAccruedExpDetail"]) {
    ...
    if (![NSString isEqualToNull:returnTips]) {
        ...
        for (PaymentExpDetail *model in self.arr_DetailsDataArray) {
            if (self.bool_isDataIntegrity && [model.isIntegrity integerValue] == 1) {
                returnTips = [NSString stringWithFormat:@"%@%ld%@",Custing(@"第", nil),(long)count,Custing(@"条费用明细不完整,请补充信息后再提交", nil)];
                goto when_failed;
            }
            //报销期间控制
            if (self.arr_overDueList.count > 0 && [[NSString stringIsExist:model.ExpenseDate] length] == 10) {
                ...
                for (NSDictionary *overDue in self.arr_overDueList) {
                    if ([NSString isEqualToNullAndZero:expMth]
                        ...
                        NSString *overEnd = [NSString stringWithFormat:@"%ld/%@", (long)([[NSString stringWithFormat:@"%@",overDue[@"isNextYear"]] isEqualToString:@"1"] ? [year integerValue] + 1 : [year integerValue]), upToMth];
        ...
        //【预提明细的费用类别是否与付款单的费用类别一致】若不一致，则不能保存
//        if (self.arr_SecDetailsDataArray.count > 0) {
//            for (AccruedDetailModel *model in self.arr_SecDetailsDataArray) {
//                if (![self.str_ExpenseCode isEqualToString:model.expenseCode]||![self.str_ExpenseCat isEqualToString:model.expenseCat]) {
//                    returnTips = Custing(@"预提明细的费用类别必须与付款单的费用类别一致", nil);
//                    goto when_failed;
//                }
//            }
//        }
...
-(void)contectData{
    ...
    NSMutableDictionary *sa_PayReAccrued = [NSMutableDictionary dictionary];
    if (self.arr_SecDetailsDataArray.count > 0) {
//        NSMutableDictionary *modelsDic2 = [AccruedDetailModel initDicByModel:self.arr_SecDetailsDataArray[0]];
//        fieldNames2 = [modelsDic2 allKeys];
        fieldNames2 = @[@"paymentOrderNo",@"accruedTaskId",@"active",@"gridOrder",@"accruedGridOrder",@"accruedDetailInfo",@"createTime",@"upadteTime",@"updater",@"taskId"];
        ...
        [sa_PayReAccrued setObject:@"Sa_PaymentExpDetailRelevanceAccruedExpDetail" forKey:@"tableName"];
        [sa_PayReAccrued setObject:fieldNames2 forKey:@"fieldNames"];
        [sa_PayReAccrued setObject:values2 forKey:@"fieldBigValues"];
    }else{
        [sa_PayReAccrued setObject:@"Sa_PaymentExpDetailRelevanceAccruedExpDetail" forKey:@"tableName"];
        [sa_PayReAccrued setObject:fieldNames2 forKey:@"fieldNames"];
        [sa_PayReAccrued setObject:values2 forKey:@"fieldBigValues"];
    }
    [detailedArray addObject:sa_PayReAccrued];
...
- (NSMutableArray *)createAccruedArrWithArr:(NSArray *)dataArr{
    NSMutableArray *accruedArr = [NSMutableArray array];
    if (dataArr.count>0) {
        for (NSDictionary *dict in dataArr) {
            AccruedDetailModel *model=[[AccruedDetailModel alloc]init];
            if (![dict[@"taskId"] isKindOfClass:[NSNull class]]) {
                model.taskId=[NSString stringWithFormat:@"%@",dict[@"taskId"]];
            }
            if (![dict[@"gridOrder"] isKindOfClass:[NSNull class]]) {
                model.gridOrder=[NSString stringWithFormat:@"%@",dict[@"gridOrder"]];
            }
            if (![dict[@"paymentOrderNo"] isKindOfClass:[NSNull class]]) {
                model.paymentOrderNo=[NSString stringWithFormat:@"%@",dict[@"paymentOrderNo"]];
            }
            if (![dict[@"accruedTaskId"] isKindOfClass:[NSNull class]]) {
                model.accruedTaskId=[NSString stringWithFormat:@"%@",dict[@"accruedTaskId"]];
            }
            if (![dict[@"accruedGridOrder"] isKindOfClass:[NSNull class]]) {
                model.accruedGridOrder=[NSString stringWithFormat:@"%@",dict[@"accruedGridOrder"]];
            }
            if (![dict[@"accruedDetailInfo"] isKindOfClass:[NSNull class]]) {
                model.accruedDetailInfo=[NSString stringWithFormat:@"%@",dict[@"accruedDetailInfo"]];
            }
            if (![dict[@"companyId"] isKindOfClass:[NSNull class]]) {
                model.companyId=[NSString stringWithFormat:@"%@",dict[@"companyId"]];
            }
            if (![dict[@"active"] isKindOfClass:[NSNull class]]) {
                model.active=[NSString stringWithFormat:@"%@",dict[@"active"]];
            }
            if (![dict[@"createTime"] isKindOfClass:[NSNull class]]) {
                model.createTime=[NSString stringWithFormat:@"%@",dict[@"createTime"]];
            }
            if (![dict[@"creater"] isKindOfClass:[NSNull class]]) {
                model.creater=[NSString stringWithFormat:@"%@",dict[@"creater"]];
            }
            if (![dict[@"upadteTime"] isKindOfClass:[NSNull class]]) {
                model.upadteTime=[NSString stringWithFormat:@"%@",dict[@"upadteTime"]];
            }
            if (![dict[@"updater"] isKindOfClass:[NSNull class]]) {
                model.updater=[NSString stringWithFormat:@"%@",dict[@"updater"]];
            }
            [accruedArr addObject:model];
        }
    }
    return accruedArr;
}
