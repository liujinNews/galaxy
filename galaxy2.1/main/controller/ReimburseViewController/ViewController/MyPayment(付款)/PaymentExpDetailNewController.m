//
//  PaymentExpDetailNewController.m
//  galaxy
//
...
#import "PaymentExpDetailNewController.h"

@interface PaymentExpDetailNewController ()

/**
 是否数组
 */
@property (nonatomic, strong) NSMutableArray *arr_IsOrNot;
//@property (nonatomic, copy) NSString *str_AccruedFormId;
//@property (nonatomic, copy) NSString *str_AccruedFormInfo;
...
@implementation PaymentExpDetailNewController
...
//    if (![NSString isEqualToNull:self.PaymentExpDetail.CurrencyCode]) {
//        self.PaymentExpDetail.CurrencyCode = self.dict_CurrencyCode[@"CurrencyCode"];
//        self.PaymentExpDetail.ExchangeRate = self.dict_CurrencyCode[@"ExchangeRate"];
//        self.PaymentExpDetail.Currency = self.dict_CurrencyCode[@"Currency"];
//    }
...
-(void)updateMainView{
    for (MyProcurementModel *model in self.arr_show) {
        ...
        }else if ([model.fieldName isEqualToString:@"IsStorage"]){//是否已入库
            self.model_IsStorage = model;
        ...
        }else if ([model.fieldName isEqualToString:@"Government"]){//是否政府相关
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
...
-(void)updateExpenseDateViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ExpenseDate WithContent:_txf_ExpenseDate WithFormType:formViewSelectDate WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.ExpenseDate}];
    __weak typeof(self) weakSelf = self;
    [view setTimeClickedBlock:^(MyProcurementModel *model, NSString *selectTime) {
        weakSelf.PaymentExpDetail.ExpenseDate = selectTime;
...
-(void)updateInvoiceNoViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_InvoiceNo WithContent:_txf_InvoiceNo WithFormType:formViewEnterText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.InvoiceNo}];
    [_View_InvoiceNo addSubview:view];
}
//MARK:更新费用类别
-(void)updateCateViewWithModel:(MyProcurementModel *)model{
    _txf_Cate=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_Cate WithContent:_txf_Cate WithFormType:formViewSelectCate WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":[GPUtils getSelectResultWithArray:@[self.PaymentExpDetail.ExpenseCat,self.PaymentExpDetail.ExpenseType]]}];
    __weak typeof(self) weakSelf = self;
    ...
    if ([NSString isEqualToNull:[NSString stringWithFormat:@"%@",self.PaymentExpDetail.ExpenseCode]]&&![[NSString stringWithFormat:@"%@",self.PaymentExpDetail.ExpenseCode] isEqualToString:@"0"]) {
        [view setCateImg:self.PaymentExpDetail.ExpenseIcon];
    }
}
//MARK:更新借款金额
-(void)updateAmountViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_Amount WithContent:_txf_Amount WithFormType:formViewEnterAmout WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.Amount}];
    __weak typeof(self) weakSelf = self;
    [view setAmountChangedBlock:^(NSString *amount){
        NSString *local = [GPUtils decimalNumberMultipWithString:amount with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.ExchangeRate]?weakSelf.PaymentExpDetail.ExchangeRate:@"1.0000")];
        ...
        NSString *payLoc = [GPUtils getRoundingOffNumber:[GPUtils decimalNumberMultipWithString:amount with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.InvCyPmtExchangeRate]?weakSelf.PaymentExpDetail.InvCyPmtExchangeRate:@"1.0000")] afterPoint:2];
        NSString *airLoc1 = payLoc;
        if ([[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.InvoiceType] isEqualToString:@"1"] && [[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1004"] && [NSDate intervalSinceReferenceDate_double:[NSDate dateWithstring:[NSDate dateWithstringBySemicolon:weakSelf.PaymentExpDetail.ExpenseDate]] localeDate:[NSDate dateWithstring:@"2019-04-01"]] > 0) {
...
-(void)updateCurrencyCodeView:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_CurrencyCode WithContent:_txf_CurrencyCode WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.Currency}];
    __weak typeof(self) weakSelf = self;
...
-(void)updateExchangeRateView:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_ExchangeRate WithContent:_txf_ExchangeRate WithFormType:formViewEnterExchange WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.ExchangeRate}];
    __weak typeof(self) weakSelf = self;
    [view setExchangeChangedBlock:^(NSString *exchange){
        weakSelf.PaymentExpDetail.ExchangeRate = exchange;
        ...
        NSString *airLoc = [NSString stringWithFormat:@"%@",local];
        NSString *payLoc = [GPUtils getRoundingOffNumber:[GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.InvCyPmtExchangeRate]?weakSelf.PaymentExpDetail.InvCyPmtExchangeRate:@"1.0000")] afterPoint:2];
        NSString *airLoc1 = payLoc;
        if ([[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.InvoiceType] isEqualToString:@"1"] && [[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1004"] && [NSDate intervalSinceReferenceDate_double:[NSDate dateWithstring:[NSDate dateWithstringBySemicolon:weakSelf.PaymentExpDetail.ExpenseDate]] localeDate:[NSDate dateWithstring:@"2019-04-01"]] > 0) {
            airLoc = [GPUtils decimalNumberSubWithString:airLoc with:[GPUtils decimalNumberSubWithString:airLoc with:weakSelf.txf_AirlineFuelFee.text]];
            airLoc1 = [GPUtils decimalNumberSubWithString:airLoc1 with:[GPUtils decimalNumberSubWithString:airLoc1 with:weakSelf.txf_AirlineFuelFee.text]];
...
-(void)updateLocalCyAmountView:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_LocalCyAmount WithContent:_txf_LocalCyAmount WithFormType:formViewShowAmount WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.LocalCyAmount}];
...
-(void)updateInvCyPmtExchangeRateView:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_InvCyPmtExchangeRate WithContent:_txf_InvCyPmtExchangeRate WithFormType:formViewEnterExchange WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.InvCyPmtExchangeRate}];
    __weak typeof(self) weakSelf = self;
    [view setExchangeChangedBlock:^(NSString *exchange){
        weakSelf.PaymentExpDetail.InvCyPmtExchangeRate = exchange;
        NSString *local = [GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:exchange]?exchange:@"1.0000")];
        local = [GPUtils getRoundingOffNumber:local afterPoint:2];
...
-(void)updateInvPmtAmountView:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_InvPmtAmount WithContent:_txf_InvPmtAmount WithFormType:formViewShowAmount WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.InvPmtAmount}];
...
-(void)updateInvoiceTypeViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_InvoiceType WithContent:_txf_InvoiceType WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.InvoiceTypeName}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.InvoiceTypeCode] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.InvoiceType = Model.invoiceType;
                weakSelf.PaymentExpDetail.InvoiceTypeCode = Model.Id;
                weakSelf.PaymentExpDetail.InvoiceTypeName = Model.Type;
        ...
        model1.Id = [NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.InvoiceTypeCode];
        picker.Model = model1;
        [picker UpdatePickUI];
...
-(void)updateAirlineFuelFeeView{
    ...
    model.fieldValue = self.PaymentExpDetail.AirlineFuelFee;
    ...
    model1.fieldValue = self.PaymentExpDetail.AirTicketPrice;
    ...
    view1.AmountChangedBlock = ^(NSString *amount) {
        ...
        NSString *local = [GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.ExchangeRate]?weakSelf.PaymentExpDetail.ExchangeRate:@"1.0000")];
        local = [GPUtils getRoundingOffNumber:local afterPoint:2];
        NSString *airLoc = [NSString stringWithFormat:@"%@",local];
        NSString *payLoc = [GPUtils getRoundingOffNumber:[GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.InvCyPmtExchangeRate]?weakSelf.PaymentExpDetail.InvCyPmtExchangeRate:@"1.0000")] afterPoint:2];
        NSString *airLoc1 = payLoc;
    ...
    model2.fieldValue = self.PaymentExpDetail.DevelopmentFund;
    ...
    model3.fieldValue = self.PaymentExpDetail.FuelSurcharge;
    ...
    model4.fieldValue = self.PaymentExpDetail.OtherTaxes;
...
-(void)updateTaxRateViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_TaxRate WithContent:_txf_TaxRate WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.TaxRate}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            weakSelf.PaymentExpDetail.TaxRate = [NSString stringWithIdOnNO:Model.Type];
            weakSelf.txf_TaxRate.text = weakSelf.PaymentExpDetail.TaxRate;
            NSString *local = [GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.ExchangeRate] ? weakSelf.PaymentExpDetail.ExchangeRate:@"1.0000")];
            local = [GPUtils getRoundingOffNumber:local afterPoint:2];
    ...
    if (![[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceType] isEqualToString:@"1"]) {
...
-(void)updateTaxViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_Tax WithContent:_txf_Tax WithFormType:formViewEnterAmout WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.Tax}];
    __weak typeof(self) weakSelf = self;
    [view setAmountChangedBlock:^(NSString *amount) {
        NSString *local = [GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.ExchangeRate]?weakSelf.PaymentExpDetail.ExchangeRate:@"1.0000")];
...
-(void)updateExclTaxViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ExclTax WithContent:_txf_ExclTax WithFormType:formViewShowAmount WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.ExclTax}];
...
-(void)updateInvPmtTaxViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_InvPmtTax WithContent:_txf_InvPmtTax WithFormType:formViewEnterAmout WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.InvPmtTax}];
    __weak typeof(self) weakSelf = self;
    [view setAmountChangedBlock:^(NSString *amount) {
        NSString *local = [GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.InvCyPmtExchangeRate]?weakSelf.PaymentExpDetail.InvCyPmtExchangeRate:@"1.0000")];
...
-(void)updateInvPmtAmountExclTaxViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_InvPmtAmountExclTax WithContent:_txf_InvPmtAmountExclTax WithFormType:formViewShowAmount WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.InvPmtAmountExclTax}];
    ...
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceType] isEqualToString:@"1"]) {
        if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1003"]||[[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1004"]||[[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1005"]) {
            ...
            if ([NSDate intervalSinceReferenceDate_double:[NSDate dateWithstring:[NSDate dateWithstringBySemicolon:self.PaymentExpDetail.ExpenseDate]] localeDate:[NSDate dateWithstring:@"2019-04-01"]] < 0) {
                ...
                height = 60;
                if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1004"]) {
                    heightAir = 60;
                }else{
                ...
                if (type == 1) {
                    NSString *local = [GPUtils decimalNumberMultipWithString:self.txf_Amount.text with:([NSString isEqualToNull:self.PaymentExpDetail.ExchangeRate] ? self.PaymentExpDetail.ExchangeRate:@"1.0000")];
                    self.txf_ExclTax.text = [GPUtils transformNsNumber:[GPUtils decimalNumberSubWithString:local with:self.txf_Tax.text]];
                }else{
                    NSPredicate *pred1 = [NSPredicate predicateWithFormat:@"Id MATCHES %@", [NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] ];
                    ...
                    if (filterArray1.count > 0) {
                        ...
                        self.PaymentExpDetail.TaxRate = model.taxRate;
                        ...
                        NSString *airLoc = [NSString stringWithFormat:@"%@",local];
                        NSString *payLoc = [GPUtils getRoundingOffNumber:[GPUtils decimalNumberMultipWithString:self.txf_Amount.text with:([NSString isEqualToNull:self.PaymentExpDetail.InvCyPmtExchangeRate]?self.PaymentExpDetail.InvCyPmtExchangeRate:@"1.0000")] afterPoint:2];
                        NSString *airLoc1 = payLoc;
            ...
            NSString *local1 = [GPUtils decimalNumberMultipWithString:self.txf_Amount.text with:([NSString isEqualToNull:self.PaymentExpDetail.InvCyPmtExchangeRate]?self.PaymentExpDetail.InvCyPmtExchangeRate:@"1.0000")];
...
-(void)updateContractNameViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ContName WithContent:_txf_ContName WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.ContractName}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
        [weakSelf keyClose];
        if (weakSelf.isContractPaymentMethod == 1) {
            ...
            vc.ChooseCategoryId = weakSelf.PaymentExpDetail.ContractAppNumber;
            ...
            vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
                ...
                weakSelf.PaymentExpDetail.ContractAppNumber = model.taskId;
                weakSelf.PaymentExpDetail.ContractNo = model.contractNo;
                weakSelf.PaymentExpDetail.ContractName = [GPUtils getSelectResultWithArray:@[model.serialNo,model.contractName]];
                weakSelf.txf_ContName.text = weakSelf.PaymentExpDetail.ContractName;
            };
            ...
            vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
                ChooseCateFreModel *model = array[0];
                weakSelf.PaymentExpDetail.ContractAppNumber = model.taskId;
                weakSelf.PaymentExpDetail.ContractNo = model.serialNo;
                weakSelf.PaymentExpDetail.ContractName = [GPUtils getSelectResultWithArray:@[model.contractNo,model.contractName]];
                weakSelf.txf_ContName.text = weakSelf.PaymentExpDetail.ContractName;
            };
            [weakSelf.navigationController pushViewController:vc animated:YES];
        }
...
-(void)updateProjectViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_Project WithContent:_txf_Project WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.ProjName}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
...
-(void)updateCostTypeViewWithModel:(MyProcurementModel *)model{
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.CostType]isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"费用", nil);
        self.PaymentExpDetail.CostType = @"1";
    }else{
        model.fieldValue = Custing(@"资本性支出", nil);
        self.PaymentExpDetail.CostType = @"0";
    }
    _txf_CostType = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_CostType WithContent:_txf_CostType WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.CostType] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.CostType = Model.Id;
                weakSelf.txf_CostType.text = Model.Type;
                [weakSelf updateIsStorageViewWithModel:weakSelf.model_IsStorage];
            }
        }];
        picker.typeTitle = Custing(@"类型", nil);
        picker.DateSourceArray = weakSelf.arr_CostType;
        STOnePickModel *model1 = [[STOnePickModel alloc]init];
        model1.Id = weakSelf.PaymentExpDetail.CostType;
        picker.Model = model1;
        [picker UpdatePickUI];
        [picker setContentMode:STPickerContentModeBottom];
        [picker show];
    };
    [_View_CostType addSubview:view];
}
//MARK:是否已入库
-(void)updateIsStorageViewWithModel:(MyProcurementModel *)model{
    for (UIView *view in [_View_IsStorage subviews]) {
        [view removeFromSuperview];
    }
    if ([self.PaymentExpDetail.CostType isEqualToString:@"0"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
        if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.IsStorage]isEqualToString:@"1"]) {
            model.fieldValue = Custing(@"是", nil);
            self.PaymentExpDetail.IsStorage = @"1";
        }else{
            model.fieldValue = Custing(@"否", nil);
            self.PaymentExpDetail.IsStorage = @"0";
        }
        _txf_IsStorage = [[UITextField alloc]init];
        SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_IsStorage WithContent:_txf_IsStorage WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
        __weak typeof(self) weakSelf = self;
        view.FormClickedBlock = ^(MyProcurementModel *model) {
            STOnePickView *picker = [[STOnePickView alloc]init];
            [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
                if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.IsStorage] isEqualToString:Model.Id]) {
                    weakSelf.PaymentExpDetail.IsStorage = Model.Id;
                    weakSelf.txf_IsStorage.text = Model.Type;
                }
            }];
            picker.typeTitle = Custing(@"是否已入库", nil);
            picker.DateSourceArray = weakSelf.arr_IsOrNot;
            STOnePickModel *model1 = [[STOnePickModel alloc]init];
            model1.Id = weakSelf.PaymentExpDetail.IsStorage;
            picker.Model = model1;
            [picker UpdatePickUI];
            [picker setContentMode:STPickerContentModeBottom];
            [picker show];
        };
        [_View_IsStorage addSubview:view];
    }else{
        [_View_IsStorage updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@0);
        }];
    }
}
...
-(void)updateIsPrepayViewWithModel:(MyProcurementModel *)model{
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.IsPrepay]isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"是", nil);
        self.PaymentExpDetail.IsPrepay = @"1";
    }else{
        model.fieldValue = Custing(@"否", nil);
        self.PaymentExpDetail.IsPrepay = @"0";
    }
    _txf_IsPrepay = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_IsPrepay WithContent:_txf_IsPrepay WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.IsPrepay] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.IsPrepay = Model.Id;
                weakSelf.txf_IsPrepay.text = Model.Type;
                [weakSelf updatePrepayStartDateViewWithModel:weakSelf.model_PrepayStartDate];
                [weakSelf updatePrepayEndDateViewWithModel:weakSelf.model_PrepayEndDate];;
            }
        }];
        picker.typeTitle = Custing(@"是否预付", nil);
        picker.DateSourceArray = weakSelf.arr_IsOrNot;
        STOnePickModel *model1 = [[STOnePickModel alloc]init];
        model1.Id = weakSelf.PaymentExpDetail.IsPrepay;
        picker.Model = model1;
        [picker UpdatePickUI];
        [picker setContentMode:STPickerContentModeBottom];
        [picker show];
    };
    [_View_IsPrepay addSubview:view];
}
//MARK:更新预付开始期间
-(void)updatePrepayStartDateViewWithModel:(MyProcurementModel *)model{
    for (UIView *view in [_View_PrepayStartDate subviews]) {
        [view removeFromSuperview];
    }
    if ([self.PaymentExpDetail.IsPrepay isEqualToString:@"1"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
        _txf_PrepayStartDate = [[UITextField alloc]init];
        SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_PrepayStartDate WithContent:_txf_PrepayStartDate WithFormType:formViewSelectDate WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.PrepayStartDate}];
        __weak typeof(self) weakSelf = self;
        [view setTimeClickedBlock:^(MyProcurementModel *model, NSString *selectTime) {
            weakSelf.PaymentExpDetail.PrepayStartDate = selectTime;
        }];
        [_View_PrepayStartDate addSubview:view];
    }else{
        [_View_PrepayStartDate makeConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@0);
        }];
    }
}
...
-(void)updatePrepayEndDateViewWithModel:(MyProcurementModel *)model{
    for (UIView *view in [_View_PrepayEndDate subviews]) {
        [view removeFromSuperview];
    }
    if ([self.PaymentExpDetail.IsPrepay isEqualToString:@"1"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
        _txf_PrepayStartDate = [[UITextField alloc]init];
        SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_PrepayEndDate WithContent:_txf_PrepayEndDate WithFormType:formViewSelectDate WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.PrepayEndDate}];
        __weak typeof(self) weakSelf = self;
        [view setTimeClickedBlock:^(MyProcurementModel *model, NSString *selectTime) {
            weakSelf.PaymentExpDetail.PrepayEndDate = selectTime;
        }];
        [_View_PrepayEndDate addSubview:view];
    }else{
        [_View_PrepayEndDate makeConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@0);
        }];
    }
}
...
-(void)updateIsAccruedaccountViewWithModel:(MyProcurementModel *)model{
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.IsAccruedaccount]isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"是", nil);
        self.PaymentExpDetail.IsAccruedaccount = @"1";
    }else{
        model.fieldValue = Custing(@"否", nil);
        self.PaymentExpDetail.IsAccruedaccount = @"0";
    }
    _txf_IsAccruedaccount = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_IsAccruedaccount WithContent:_txf_IsAccruedaccount WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.IsAccruedaccount] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.IsAccruedaccount = Model.Id;
                weakSelf.txf_IsAccruedaccount.text = Model.Type;
                if ([Model.Id isEqualToString:@"0"]) {
                    [weakSelf.accruedArr removeAllObjects];
                }
                [weakSelf updateSa_PaymentExpDetailRelevanceAccruedExpDetailViewl];
            }
        }];
        picker.typeTitle = Custing(@"是否预提", nil);
        picker.DateSourceArray = weakSelf.arr_IsOrNot;
        STOnePickModel *model1 = [[STOnePickModel alloc]init];
        model1.Id = weakSelf.PaymentExpDetail.IsAccruedaccount;
        picker.Model = model1;
        [picker UpdatePickUI];
        [picker setContentMode:STPickerContentModeBottom];
        [picker show];
    };
    [_View_IsAccruedaccount addSubview:view];
    if ([self.PaymentExpDetail.IsAccruedaccount isEqualToString:@"1"]) {
        [self updateSa_PaymentExpDetailRelevanceAccruedExpDetailViewl];
    }
}
//MARK:更新预提明细表
-(void)updateSa_PaymentExpDetailRelevanceAccruedExpDetailViewl{
    if (self.accruedArr.count >= 0) {
        NSMutableArray *nameArr = [NSMutableArray array];
        NSMutableArray *idArr = [NSMutableArray array];
        for (AccruedDetailModel *model in self.accruedArr) {
            [nameArr addObject:[NSString stringWithFormat:@"%@/%@-%@",model.serialNo,model.reason,model.gridOrder]];
            [idArr addObject:model.taskId];
            model.paymentOrderNo = self.PaymentExpDetail.PaymentOrderNo;
        }
        self.str_accuredInfo = [GPUtils getSelectResultWithArray:nameArr WithCompare:@"⊕"];
        self.str_taskId = [GPUtils getSelectResultWithArray:idArr WithCompare:@","];
    }
    MyProcurementModel *model = [[MyProcurementModel alloc] init];
    model.Description = Custing(@"关联预提明细", nil);
    model.isShow = [NSNumber numberWithInt:1];
    if ([self.PaymentExpDetail.IsAccruedaccount isEqualToString:@"0"]) {
        model.isShow = [NSNumber numberWithInt:0];
...
-(void)updateGovernmentViewWithModel:(MyProcurementModel *)model{
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.Government]isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"是", nil);
        self.PaymentExpDetail.Government = @"1";
    }else{
        model.fieldValue = Custing(@"否", nil);
        self.PaymentExpDetail.Government = @"0";
    }
    _txf_Government = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_Government WithContent:_txf_Government WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.Government] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.Government = Model.Id;
                weakSelf.txf_Government.text = Model.Type;
            }
        }];
        picker.typeTitle = Custing(@"是否政府相关", nil);
        picker.DateSourceArray = weakSelf.arr_IsOrNot;
        STOnePickModel *model1 = [[STOnePickModel alloc]init];
        model1.Id = weakSelf.PaymentExpDetail.Government;
        picker.Model = model1;
        [picker UpdatePickUI];
        [picker setContentMode:STPickerContentModeBottom];
        [picker show];
    };
    [_View_Government addSubview:view];
}
//MARK:是否境外
-(void)updateOverseasViewWithModel:(MyProcurementModel *)model{
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.Overseas]isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"是", nil);
        self.PaymentExpDetail.Overseas = @"1";
    }else{
        model.fieldValue = Custing(@"否", nil);
        self.PaymentExpDetail.Overseas = @"0";
    }
    _txf_Overseas = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_Overseas WithContent:_txf_Overseas WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.Overseas] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.Overseas = Model.Id;
                weakSelf.txf_Overseas.text = Model.Type;
        ...
        picker.DateSourceArray = weakSelf.arr_IsOrNot;
        STOnePickModel *model1 = [[STOnePickModel alloc]init];
        model1.Id = weakSelf.PaymentExpDetail.Overseas;
        picker.Model = model1;
        [picker UpdatePickUI];
        [picker setContentMode:STPickerContentModeBottom];
        [picker show];
...
-(void)updateNationalityViewWithModel:(MyProcurementModel *)model{
    _txf_Nationality = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_Nationality WithContent:_txf_Nationality WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.Nationality}];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        ChooseCateFreshController *vc=[[ChooseCateFreshController alloc]initWithType:@"ConfigurationItem"];
        vc.ChooseCategoryId = [NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.NationalityId];
        vc.ChooseModel = model;
        vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
            ChooseCateFreModel *model = array[0];
            weakSelf.PaymentExpDetail.Nationality = model.name;
            weakSelf.txf_Nationality.text = model.name;
            weakSelf.PaymentExpDetail.NationalityId = model.Id;
        };
        [weakSelf.navigationController pushViewController:vc animated:YES];
    };
...
-(void)updateTransactionCodeViewWithModel:(MyProcurementModel *)model{
    _txf_TransactionCode = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_TransactionCode WithContent:_txf_TransactionCode WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.TransactionCode}];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        ChooseCateFreshController *vc=[[ChooseCateFreshController alloc]initWithType:@"ConfigurationItem"];
        vc.ChooseCategoryId = [NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.TransactionCodeId];
        vc.ChooseModel = model;
        vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
            ChooseCateFreModel *model = array[0];
            weakSelf.PaymentExpDetail.TransactionCode = model.name;
            weakSelf.txf_TransactionCode.text = model.name;
            weakSelf.PaymentExpDetail.TransactionCodeId = model.Id;
        };
        [weakSelf.navigationController pushViewController:vc animated:YES];
    };
...
-(void)updateHandmadePaperViewWithModel:(MyProcurementModel *)model{
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.HandmadePaper]isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"是", nil);
        self.PaymentExpDetail.HandmadePaper = @"1";
    }else{
        model.fieldValue = Custing(@"否", nil);
        self.PaymentExpDetail.HandmadePaper = @"0";
    }
    _txf_HandmadePaper = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_HandmadePaper WithContent:_txf_HandmadePaper WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.HandmadePaper] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.HandmadePaper = Model.Id;
                weakSelf.txf_HandmadePaper.text = Model.Type;
            }
        }];
        picker.typeTitle = Custing(@"是否手工票据", nil);
        picker.DateSourceArray = weakSelf.arr_IsOrNot;
        STOnePickModel *model1 = [[STOnePickModel alloc]init];
        model1.Id = weakSelf.PaymentExpDetail.HandmadePaper;
        picker.Model = model1;
        [picker UpdatePickUI];
        [picker setContentMode:STPickerContentModeBottom];
        [picker show];
...
-(void)updateCostCenterViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_CostCenter WithContent:_txf_CostCenter WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.CostCenter}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
    ...
    if ([NSString isEqualToNull:model.fieldValue]) {
        self.PaymentExpDetail.CostCenterId = model.fieldValue;
    }
}
//点击成本中心
- (void)clickCostCenter{
    ChooseCateFreshController *vc=[[ChooseCateFreshController alloc]initWithType:@"costCenter"];
    vc.ChooseCategoryId = self.PaymentExpDetail.CostCenterId;
    __weak typeof(self) weakSelf = self;
    vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
        ChooseCateFreModel *model = array[0];
        if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.CostCenterId] isEqualToString:model.Id]) {
            weakSelf.PaymentExpDetail.CostCenterId = model.Id;
...
//            weakSelf.str_CostCenterMgrUserId = model.costCenterMgrUserId;
            ...
            weakSelf.PaymentExpDetail.CostCenter = model.costCenter;
//            if (self.Type != 3) {
...
-(void)updateFeeAppNumberViewWithModel:(MyProcurementModel *)model{
    if (![NSString isEqualToNullAndZero:[NSString stringWithFormat:@"%@",self.PaymentExpDetail.FeeAppInfo]]) {
        self.PaymentExpDetail.FeeAppInfo=@"";
        self.PaymentExpDetail.FeeAppNumber=@"";
    }
    NSDictionary *dict = @{@"Id":[NSString stringWithIdOnNO:self.PaymentExpDetail.FeeAppNumber],
                           @"Value":[NSString stringWithIdOnNO:self.PaymentExpDetail.FeeAppInfo],
                           @"Model":model
                           };
...
//            weakSelf.FormDatas.str_EstimatedAmount = [GPUtils decimalNumberAddWithString:model.localCyAmount with:weakSelf.FormDatas.str_EstimatedAmount];
        ...
        weakSelf.PaymentExpDetail.FeeAppInfo = [GPUtils getSelectResultWithArray:name WithCompare:@"⊕"];
        weakSelf.PaymentExpDetail.FeeAppNumber = [GPUtils getSelectResultWithArray:Id WithCompare:@","];
...
-(void)updateIsSettlementViewWithModel:(MyProcurementModel *)model{
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.IsSettlement]isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"是", nil);
        self.PaymentExpDetail.IsSettlement = @"1";
    }else{
        model.fieldValue = Custing(@"否", nil);
        self.PaymentExpDetail.IsSettlement = @"0";
    }
    _txf_IsSettlement = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_IsSettlement WithContent:_txf_IsSettlement WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        STOnePickView *picker = [[STOnePickView alloc]init];
        [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
            if (![[NSString stringWithFormat:@"%@",weakSelf.PaymentExpDetail.IsSettlement] isEqualToString:Model.Id]) {
                weakSelf.PaymentExpDetail.IsSettlement = Model.Id;
                weakSelf.txf_IsSettlement.text = Model.Type;
            }
        }];
        picker.typeTitle = Custing(@"是否已结项", nil);
        picker.DateSourceArray = weakSelf.arr_IsOrNot;
        STOnePickModel *model1 = [[STOnePickModel alloc]init];
        model1.Id = weakSelf.PaymentExpDetail.IsSettlement;
        picker.Model = model1;
        [picker UpdatePickUI];
        [picker setContentMode:STPickerContentModeBottom];
        [picker show];
    };
    [_View_IsSettlement addSubview:view];
}
//MARK:辅助核算项
-(void)updateAccountItemViewWithModel:(MyProcurementModel *)model{
    _txf_AccountItem = [[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_AccountItem WithContent:_txf_AccountItem WithFormType:formViewEnterText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.AccountItem}];
...
-(void)updateAccountItemAmountViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_AccountItemAmount WithContent:_txf_AccountItemAmount WithFormType:formViewEnterAmout WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.AccountItemAmount}];
...
-(void)updateAccountItem2ViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_AccountItem2 WithContent:_txf_AccountItem2 WithFormType:formViewEnterText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.AccountItem2}];
...
-(void)updateAccountItemAmount2ViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_AccountItemAmount2 WithContent:_txf_AccountItemAmount2 WithFormType:formViewEnterAmout WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.AccountItemAmount2}];
    [_View_AccountItemAmount2 addSubview:view];
}
//MARK:辅助核算项3
-(void)updateAccountItem3ViewWithModel:(MyProcurementModel *)model{
    _txf_AccountItem3 = [[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_AccountItem3 WithContent:_txf_AccountItem3 WithFormType:formViewEnterText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.AccountItem3}];
...
-(void)updateAccountItemAmount3ViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_AccountItemAmount3 WithContent:_txf_AccountItemAmount3 WithFormType:formViewEnterAmout WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.AccountItemAmount3}];
...
-(void)updateOverseasSubViews{
    ...
    if (height == 0) {
        self.PaymentExpDetail.Nationality = @"";
        self.PaymentExpDetail.NationalityId = @"0";
        self.PaymentExpDetail.TransactionCode = @"";
        self.PaymentExpDetail.TransactionCodeId = @"";
        self.PaymentExpDetail.HandmadePaper = @"0";
    }
}
...
-(void)updateExpenseDescViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ExpenseDesc WithContent:_txv_ExpenseDesc WithFormType:formViewEnterTextView WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.ExpenseDesc}];
    [_View_ExpenseDesc addSubview:view];
}
//MARK:更新备注视图
-(void)updateRemarkViewWithModel:(MyProcurementModel *)model{
    _txv_Remark = [[UITextView alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_Remark WithContent:_txv_Remark WithFormType:formViewVoiceTextView WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.PaymentExpDetail.Remark}];
...
-(void)CateBtnClick:(UIButton *)btn{
    ...
    if ([_CateLevel isEqualToString:@"1"]) {
        ...
        model.expenseCode= self.PaymentExpDetail.ExpenseCode;
        ...
        [pickerArea setChooseCateBlock:^(CostCateNewModel *firstModel, CostCateNewSubModel *secondModel) {
            [weakSelf keyClose];
            if (![secondModel.expenseType isEqualToString:weakSelf.PaymentExpDetail.ExpenseType]) {
                ...
                weakSelf.PaymentExpDetail.ExpenseType = [NSString isEqualToNull:secondModel.expenseType]?secondModel.expenseType:@"";
                weakSelf.PaymentExpDetail.ExpenseCode = secondModel.expenseCode;
                weakSelf.PaymentExpDetail.ExpenseIcon = secondModel.expenseIcon;
                weakSelf.PaymentExpDetail.ExpenseCat = secondModel.expenseCat;
                weakSelf.PaymentExpDetail.ExpenseCatCode = secondModel.expenseCatCode;
        ...
        ex.CellClick = ^(CostCateNewSubModel *model) {
            if (![model.expenseType isEqualToString:weakSelf.PaymentExpDetail.ExpenseType]) {
                weakSelf.categoryImage.image = [UIImage imageNamed:[NSString isEqualToNull:model.expenseIcon]?model.expenseIcon:@"15"];
                weakSelf.PaymentExpDetail.ExpenseType = [NSString isEqualToNull:model.expenseType]?model.expenseType:@"";
                weakSelf.PaymentExpDetail.ExpenseCode = model.expenseCode;
                weakSelf.PaymentExpDetail.ExpenseIcon = model.expenseIcon;
                weakSelf.PaymentExpDetail.ExpenseCat = model.expenseCat;
                weakSelf.PaymentExpDetail.ExpenseCatCode = model.expenseCatCode;
...
-(void)changeCurrency{
    ...
    __weak typeof(self) weakSelf = self;
    [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
        weakSelf.PaymentExpDetail.CurrencyCode = Model.Id;
        weakSelf.PaymentExpDetail.Currency = Model.Type;
        weakSelf.txf_CurrencyCode.text = Model.Type;
        weakSelf.txf_ExchangeRate.text = Model.exchangeRate;
        weakSelf.PaymentExpDetail.ExchangeRate = Model.exchangeRate;
        NSString *local = [GPUtils decimalNumberMultipWithString:weakSelf.txf_Amount.text with:([NSString isEqualToNull:weakSelf.PaymentExpDetail.ExchangeRate]?weakSelf.PaymentExpDetail.ExchangeRate:@"1.0000")];
        local = [GPUtils getRoundingOffNumber:local afterPoint:2];
        weakSelf.txf_LocalCyAmount.text = [GPUtils transformNsNumber:local];
        weakSelf.PaymentExpDetail.InvCyPmtExchangeRate = Model.exchangeRate;
    ...
    STOnePickModel *model = [[STOnePickModel alloc]init];
    model.Id = [NSString isEqualToNull: self.PaymentExpDetail.CurrencyCode] ? self.PaymentExpDetail.CurrencyCode:@"";
    picker.Model = model;
    [picker UpdatePickUI];
    [picker setContentMode:STPickerContentModeBottom];
    [picker show];
...
-(void)ProjectClick{
    ...
    vc.ChooseCategoryId = self.PaymentExpDetail.ProjId;
    vc.dict_otherPars = @{@"CostCenterId":self.dict_parameter[@"CostCenterId"] ? self.dict_parameter[@"CostCenterId"]:@"0"};
    vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
        ChooseCateFreModel *model = array[0];
        weakSelf.PaymentExpDetail.ProjId = model.Id;
        weakSelf.PaymentExpDetail.ProjName = [GPUtils getSelectResultWithArray:@[model.no,model.projName]];
        weakSelf.PaymentExpDetail.ProjMgrUserId = model.projMgrUserId;
        weakSelf.PaymentExpDetail.ProjMgr = model.projMgr;
        weakSelf.txf_Project.text = weakSelf.PaymentExpDetail.ProjName;
    };
    [self.navigationController pushViewController:vc animated:YES];
}
//MARK:关联预提明细
...
//    vc.ChooseCategoryId = self.str_taskId;
    if (self.accruedArr.count > 0) {
        for (AccruedDetailModel *model in self.accruedArr) {
            ...
            [dic setValue:model.gridOrder forKey:@"gridOrder"];
            [vc.chooseCategoryArr addObject:dic];
        }
    }
    vc.multi_FieldJudgment = YES;
    vc.isMultiSelect = YES;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (![model.expenseType isEqualToString:@""]) {
        ...
        self.PaymentExpDetail.ExpenseType = [NSString isEqualToNull:model.expenseType]?model.expenseType:@"";
        self.PaymentExpDetail.ExpenseCode = model.expenseCode;
        self.PaymentExpDetail.ExpenseIcon = model.expenseIcon;
        self.PaymentExpDetail.ExpenseCat = model.expenseCat;
        self.PaymentExpDetail.ExpenseCatCode = model.expenseCatCode;
...
-(void)saveData{
    //liuj_add_S
    //类型
    self.PaymentExpDetail.CostType = [NSString isEqualToNull:self.txf_CostType.text]?self.txf_CostType.text:@"";
    if ([self.PaymentExpDetail.CostType isEqualToString:Custing(@"资本性支出", nil)]) {
        self.PaymentExpDetail.IsStorage = [NSString isEqualToNull:self.PaymentExpDetail.IsStorage]?self.PaymentExpDetail.IsStorage:@"0";
    }
    //是否预付
    self.PaymentExpDetail.IsPrepay = [NSString isEqualToNull:self.PaymentExpDetail.IsPrepay]?self.PaymentExpDetail.IsPrepay:@"0";
    if ([self.PaymentExpDetail.IsPrepay isEqualToString:@"1"]) {
        self.PaymentExpDetail.PrepayStartDate = [NSString isEqualToNull:self.txf_PrepayStartDate.text]?self.txf_PrepayStartDate.text:@"";
        self.PaymentExpDetail.PrepayEndDate = [NSString isEqualToNull:self.txf_PrepayEndDate.text]?self.txf_PrepayEndDate.text:@"";
    }
    //是否预提
    self.PaymentExpDetail.IsAccruedaccount = [NSString isEqualToNull:self.PaymentExpDetail.IsAccruedaccount]?self.PaymentExpDetail.IsAccruedaccount:@"0";
    //成本中心
    self.PaymentExpDetail.CostCenterId = [NSString isEqualToNull:self.PaymentExpDetail.CostCenterId]?self.PaymentExpDetail.CostCenterId:@"";
    self.PaymentExpDetail.CostCenter = [NSString isEqualToNull:self.txf_CostCenter.text]?self.txf_CostCenter.text:@"";
    //费用申请单
    self.PaymentExpDetail.FeeAppNumber = [NSString isEqualToNull:self.PaymentExpDetail.FeeAppNumber]?self.PaymentExpDetail.FeeAppNumber:@"";
    self.PaymentExpDetail.FeeAppInfo = [NSString isEqualToNull:self.PaymentExpDetail.FeeAppInfo]?self.PaymentExpDetail.FeeAppInfo:@"";
    //是否已结项
    self.PaymentExpDetail.IsSettlement = [NSString isEqualToNull:self.PaymentExpDetail.IsSettlement]?self.PaymentExpDetail.IsSettlement:@"0";
    //是否与政府相关
    self.PaymentExpDetail.Government = [NSString isEqualToNull:self.PaymentExpDetail.Government]?self.PaymentExpDetail.Government:@"0";
    //辅助核算项
    self.PaymentExpDetail.AccountItem = [NSString isEqualToNull:self.txf_AccountItem.text]?self.txf_AccountItem.text:@"";

    //辅助核算金额
    self.PaymentExpDetail.AccountItemAmount = [NSString isEqualToNull:self.txf_AccountItemAmount.text] ? self.txf_AccountItemAmount.text:@"";

    //辅助核算项2
    self.PaymentExpDetail.AccountItem2 = [NSString isEqualToNull:self.txf_AccountItem2.text]?self.txf_AccountItem2.text:@"";

    //辅助核算金额2
    self.PaymentExpDetail.AccountItemAmount2 = [NSString isEqualToNull:_txf_AccountItemAmount2.text] ? self.txf_AccountItemAmount2.text:@"";

    //辅助核算项3
    self.PaymentExpDetail.AccountItem3 = [NSString isEqualToNull:self.txf_AccountItem3.text]?self.txf_AccountItem3.text:@"";
    //辅助核算金额3
    self.PaymentExpDetail.AccountItemAmount3 = [NSString isEqualToNull:self.txf_AccountItemAmount3.text] ? self.txf_AccountItemAmount3.text:@"";
    self.PaymentExpDetail.ContractGridOrder = [NSString isEqualToNull:self.PaymentExpDetail.ContractGridOrder]?self.PaymentExpDetail.ContractGridOrder:@"";
    //liuj_add_E
    
    self.PaymentExpDetail.ExpenseDate = self.txf_ExpenseDate.text;
    self.PaymentExpDetail.InvoiceNo = self.txf_InvoiceNo.text;
    self.PaymentExpDetail.Amount = [NSString isEqualToNull:_txf_Amount.text] ? _txf_Amount.text:@"";
    NSString *LocalCyAmount = [GPUtils decimalNumberMultipWithString:_txf_Amount.text with:([NSString isEqualToNull:self.PaymentExpDetail.ExchangeRate] ? self.PaymentExpDetail.ExchangeRate:@"1.0000")];
    LocalCyAmount=[GPUtils getRoundingOffNumber:LocalCyAmount afterPoint:2];
    self.PaymentExpDetail.LocalCyAmount = [NSString isEqualToNull:LocalCyAmount] ? LocalCyAmount:@"0.00";
    CGFloat InvoiceTypeHight = self.View_InvoiceType.zl_height;
    if (InvoiceTypeHight == 0) {
        self.PaymentExpDetail.InvoiceType = @"0";
        self.PaymentExpDetail.InvoiceTypeName = @"";
        self.PaymentExpDetail.InvoiceTypeCode = @"0";
    }
    self.PaymentExpDetail.AirlineFuelFee = (_View_AirlineFuelFee.zl_height > 0 && [NSString isEqualToNull:_txf_AirlineFuelFee.text]) ?_txf_AirlineFuelFee.text:@"";
    self.PaymentExpDetail.AirTicketPrice = (_View_AirTicketPrice.zl_height > 0 && [NSString isEqualToNull:_txf_AirTicketPrice.text]) ?_txf_AirTicketPrice.text:@"";
    self.PaymentExpDetail.DevelopmentFund = (_View_DevelopmentFund.zl_height > 0 && [NSString isEqualToNull:_txf_DevelopmentFund.text]) ?_txf_DevelopmentFund.text:@"";
    self.PaymentExpDetail.FuelSurcharge = (_View_FuelSurcharge.zl_height > 0 && [NSString isEqualToNull:_txf_FuelSurcharge.text]) ?_txf_FuelSurcharge.text:@"";
    self.PaymentExpDetail.OtherTaxes = (_View_OtherTaxes.zl_height > 0 && [NSString isEqualToNull:_txf_OtherTaxes.text]) ?_txf_OtherTaxes.text:@"";


    self.PaymentExpDetail.TaxRate = (_View_TaxRate.zl_height > 0 && [NSString isEqualToNull:_txf_TaxRate.text])?_txf_TaxRate.text:@"";
    if (_View_Tax.zl_height > 0 && self.txf_Tax) {
        self.PaymentExpDetail.Tax = self.txf_Tax.text;
    }else{
        self.PaymentExpDetail.Tax = [NSString countTax:LocalCyAmount taxrate:self.PaymentExpDetail.TaxRate];
    }
    self.PaymentExpDetail.ExclTax = [GPUtils getRoundingOffNumber:[GPUtils decimalNumberSubWithString:LocalCyAmount with:self.PaymentExpDetail.Tax] afterPoint:2];
    
    
    NSString *InvPmtAmount = [GPUtils decimalNumberMultipWithString:_txf_Amount.text with:([NSString isEqualToNull:self.PaymentExpDetail.InvCyPmtExchangeRate] ? self.PaymentExpDetail.InvCyPmtExchangeRate:@"1.0000")];
    InvPmtAmount = [GPUtils getRoundingOffNumber:InvPmtAmount afterPoint:2];
    self.PaymentExpDetail.InvPmtAmount = [NSString isEqualToNull:InvPmtAmount] ? InvPmtAmount:@"0.00";
    if (_View_InvPmtTax.zl_height > 0 && self.txf_InvPmtTax) {
        self.PaymentExpDetail.InvPmtTax = self.txf_InvPmtTax.text;
    }else{
        self.PaymentExpDetail.InvPmtTax = [NSString countTax:InvPmtAmount taxrate:self.PaymentExpDetail.TaxRate];
    }
    self.PaymentExpDetail.InvPmtAmountExclTax = [GPUtils getRoundingOffNumber:[GPUtils decimalNumberSubWithString:InvPmtAmount with:self.PaymentExpDetail.InvPmtTax] afterPoint:2];
    
    self.PaymentExpDetail.Overseas = [NSString isEqualToNull:self.PaymentExpDetail.Overseas] ? [NSString stringWithFormat:@"%@",self.PaymentExpDetail.Overseas]:@"0";
    self.PaymentExpDetail.NationalityId = [NSString isEqualToNull:self.PaymentExpDetail.NationalityId] ? [NSString stringWithFormat:@"%@",self.PaymentExpDetail.NationalityId]:@"0";
    self.PaymentExpDetail.HandmadePaper = [NSString isEqualToNull:self.PaymentExpDetail.HandmadePaper] ? [NSString stringWithFormat:@"%@",self.PaymentExpDetail.HandmadePaper]:@"0";

    self.PaymentExpDetail.ExpenseDesc = _txv_ExpenseDesc.text;
    self.PaymentExpDetail.Attachments = (self.arr_totalFileArray.count!=0) ? @"1":@"";
    self.PaymentExpDetail.Remark = _txv_Remark.text;
    self.PaymentExpDetail.isIntegrity = @"0";
    ...
    for (MyProcurementModel *model in filterArray) {
        ...
        if (([fieldName isEqualToString:@"Nationality"]||[fieldName isEqualToString:@"TransactionCode"])&&![[NSString stringWithFormat:@"%@",self.PaymentExpDetail.Overseas] isEqualToString:@"1"]) {
            continue;
        }
//        if (([fieldName isEqualToString:@"TaxRate"]||[fieldName isEqualToString:@"Tax"])&&![[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceType] isEqualToString:@"1"]) {
//            continue;
//        }
        if ([fieldName isEqualToString:@"InvoiceType"] && ![NSString isEqualToNull:self.PaymentExpDetail.InvoiceTypeCode]) {
            [[GPAlertView sharedAlertView]showAlertText:self WithText:model.tips duration:2.0];
            return;
        }
        
        if (![fieldName isEqualToString:@"InvoiceType"] && ![NSString isEqualToNull:[self.PaymentExpDetail valueForKey:fieldName]]) {
    ...
    if ([NSString isEqualToNullAndZero:self.PaymentExpDetail.CurrencyCode] && ![[NSString stringWithFormat:@"%@",self.PaymentExpDetail.CurrencyCode]isEqualToString:@"CNY"] && ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1003"]||[[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1004"]||[[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1005"])) {
        [[GPAlertView sharedAlertView]showAlertText:self WithText:Custing(@"币种不是人民币，发票类型不能为铁路车票、航空行程单、公路、水路客票", nil) duration:2.0];
        return;
    }
    if ([[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceType] isEqualToString:@"1"] && [[NSString stringWithFormat:@"%@",self.PaymentExpDetail.InvoiceTypeCode] isEqualToString:@"1004"] && [NSDate intervalSinceReferenceDate_double:[NSDate dateWithstring:[NSDate dateWithstringBySemicolon:self.PaymentExpDetail.ExpenseDate]] localeDate:[NSDate dateWithstring:@"2019-04-01"]] > 0) {
        if (![NSString isEqualToNull:self.PaymentExpDetail.AirTicketPrice]) {
            [[GPAlertView sharedAlertView]showAlertText:self WithText:Custing(@"请输入票价", nil) duration:2.0];
            return;
        }else if (![NSString isEqualToNull:self.PaymentExpDetail.DevelopmentFund]){
            [[GPAlertView sharedAlertView]showAlertText:self WithText:Custing(@"请输入民航发展基金", nil) duration:2.0];
            return;
        }else if (![NSString isEqualToNull:self.PaymentExpDetail.FuelSurcharge]){
            [[GPAlertView sharedAlertView]showAlertText:self WithText:Custing(@"请输入燃油附加费", nil) duration:2.0];
            return;
        }else if (![NSString isEqualToNull:self.PaymentExpDetail.OtherTaxes]){
            [[GPAlertView sharedAlertView]showAlertText:self WithText:Custing(@"请输入其他税费", nil) duration:2.0];
            return;
        }else{
            NSString *amount = [GPUtils decimalNumberSubWithString:self.PaymentExpDetail.Amount with:self.PaymentExpDetail.AirTicketPrice];
            amount = [GPUtils decimalNumberSubWithString:amount with:self.PaymentExpDetail.DevelopmentFund];
            amount = [GPUtils decimalNumberSubWithString:amount with:self.PaymentExpDetail.FuelSurcharge];
            amount = [GPUtils decimalNumberSubWithString:amount with:self.PaymentExpDetail.OtherTaxes];
            if ([amount floatValue] != 0) {
                [[GPAlertView sharedAlertView]showAlertText:self WithText:Custing(@"票价、民航发展基金、燃油附加费、其他税费之和必须与金额一致", nil) duration:2.0];
                return;
            }
        }
    }
    
    if ([NSString isEqualToNull:self.PaymentExpDetail.ProjName]) {//有项目时
        if (![NSString isEqualToNull:self.PaymentExpDetail.ContractName]&&![NSString isEqualToNull:self.PaymentExpDetail.FeeAppInfo]) {
            NSString *message = Custing(@"请选择合同申请单或者费用申请单", nil);
            [[GPAlertView sharedAlertView]showAlertText:self WithText:message duration:2.0];
            return;
        }
    }else{
        if (![NSString isEqualToNull:self.PaymentExpDetail.CostCenter]) {//没有项目时
    ...
    [[VoiceDataManger sharedManager]uploadImageDataWithImgSoure:self.arr_totalFileArray WithUrl:travelImgLoad WithBlock:^(id data, BOOL hasError) {
        ...
        if (hasError) {
            ...
            weakSelf.PaymentExpDetail.Attachments = data;
            [weakSelf readySave];
        }
    }];
}
-(void)readySave{
    if (self.PaymentExpDetailAddEditBlock) {
        self.PaymentExpDetailAddEditBlock(self.PaymentExpDetail, self.type,self.accruedArr);
    }
