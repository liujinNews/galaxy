...
@implementation OtherReimFormData
...
-(void)initializeNewData{
    ...
    self.str_flowCode=@"F0010";

    self.arr_isShowmsArray=[[NSMutableArray alloc]initWithArray:@[@"ThirDetailList",@"ClaimType",@"Reason",@"Contact",@"RequestorDeptId",@"BranchId",@"CostCenterId",@"RequestorBusDeptId",@"AreaId",@"LocationId",@"BnfId",@"ProjId",@"ClientId",@"SupplierId",@"EntertainAppNumber",@"VehicleSvcAppNumber",@"FeeAppNumber",@"OverBudReason",@"StaffOutNumber",@"PurchaseNumber",@"SpecialRequirementsNumber",@"EmployeeTrainingNumber",@"AdvanceNumber",@"NoInvAmount",@"NumberOfDocuments",@"Payee",@"BankAccount",@"BankOutlets",@"Reserved1",@"Reserved2",@"Reserved3",@"Reserved4",@"Reserved5",@"Reserved6",@"Reserved7",@"Reserved8",@"Reserved9",@"Reserved10",@"Remark",@"FirstHandlerUserName",@"CcUsersName"]];
...
-(NSString *)OpenFormUrl{
    if (self.int_formStatus==1) {
        return [NSString stringWithFormat:@"%@",GETOtherExpData];
    }else{
        return [NSString stringWithFormat:@"%@",GETOtherHasExpData];
    }
}
-(NSDictionary *)OpenFormParameters{
    if (self.int_formStatus==1) {
        return @{@"TaskId":self.str_taskId,@"UserId":self.str_userId,@"ProcId":self.str_procId,@"ExpenseCode":self.str_expenseCode,@"FlowGuid":self.str_flowGuid};
    }else{
        return @{@"TaskId":self.str_taskId,@"UserId":self.str_userId,@"ProcId":self.str_procId};
...
-(void)DealWithFormBaseData{
    ...
    if (![result isKindOfClass:[NSNull class]]) {
        ...
        if ([NSString isEqualToNull:result[@"expenseTypeForTitle"]]) {
            self.str_OtherReimNavTitle = [NSString stringWithFormat:@"%@",result[@"expenseTypeForTitle"]];
        }
        ...
        //分摊表单规则消费记录等数据
        if (self.int_formStatus ==1) {
            [self getTravel_Daily_OtherReimData:result WithParameter:nil WithType:3];
        }else{
            [self getTravel_Daily_OtherHasReimData:result WithParameter:@{@"expensedetail":@"sa_dailyexpensedetail"} WithType:3];
        }
            ...
            if (mainFld.count!=0) {
                for (NSDictionary *dict in mainFld) {
                    ...
                    if ([dict[@"fieldName"] isEqualToString:@"PurchaseNumber"]) {
                        self.str_PurchaseNumber=[NSString stringWithIdOnNO:[NSString stringWithFormat:@"%@",[dict objectForKey:@"fieldValue"]]];
                    }
                    ...
                    if ([dict[@"fieldName"] isEqualToString:@"ActualAmount"]) {
                        self.str_ApprovalAmount=[dict objectForKey:@"fieldValue"];
                    }
                    if ([dict[@"fieldName"] isEqualToString:@"ExpenseCode"]) {
                        self.str_expenseCode=[NSString isEqualToNull:[dict objectForKey:@"fieldValue"]]?[dict objectForKey:@"fieldValue"]:self.str_expenseCode;
                    }
                    if ([dict[@"fieldName"] isEqualToString:@"ParentCode"]) {
                        self.str_parentCode=[NSString isEqualToNull:[dict objectForKey:@"fieldValue"]]?[dict objectForKey:@"fieldValue"]:self.str_parentCode;
                    }
                    if ([dict[@"fieldName"] isEqualToString:@"ExpenseType"]) {
                        self.str_expenseType=[NSString isEqualToNull:[dict objectForKey:@"fieldValue"]]?[dict objectForKey:@"fieldValue"]:self.str_expenseType;
                    }
                    if ([dict[@"fieldName"] isEqualToString:@"ExpenseIcon"]) {
                        self.str_expenseIcon=[NSString isEqualToNull:[dict objectForKey:@"fieldValue"]]?[dict objectForKey:@"fieldValue"]:self.str_expenseIcon;
                    }
...
-(void)inModelContent{
    ...
    self.SubmitData=[[OtherReimData alloc]init];
    ...
    self.SubmitData.Currency=@"";
    self.SubmitData.CurrencyCode=@"";
    ...
    self.SubmitData.ParentCode=self.str_parentCode;
    self.SubmitData.ExpenseType=self.str_expenseType;
    self.SubmitData.ExpenseIcon=self.str_expenseIcon;
    self.SubmitData.IsApprovalAuthority=self.str_IsApprovalAuthority;
    ...
    self.arr_SubmitExpenseCodes = [NSMutableArray arrayWithObject:[NSString stringWithIdOnNO:self.SubmitData.ExpenseCode]];
    self.arr_SubmitExpenseTypes = [NSMutableArray arrayWithObject:[NSString stringWithIdOnNO:self.SubmitData.ExpenseType]];
...
-(NSString *)testModel{
    ...
    OtherReimData *model=self.SubmitData;
    NSString *returnTips;
    NSMutableDictionary *modeldic = [OtherReimData initDicByModel:model];
...
//        self.SubmitData.CostCenterMgrUserIds = [GPUtils getSelectResultWithArray:arr_CostCenterMgrUserIds WithCompare:@","];
        ...
        if (self.bool_ThirDetailsShow && self.bool_isConsistentAmount) {
            ...
            if ([[GPUtils decimalNumberSubWithString:self.SubmitData.ActualAmount with:Amout] floatValue] != 0) {
                returnTips = Custing(@"收款人明细合计必须与应付金额相同", nil);
                goto when_failed;
            }
        }
...
-(void)contectData{
    ...
    NSMutableDictionary *modelDic=[OtherReimData initDicByModel:self.SubmitData];
...
-(NSString *)testApporveEditModel{
    if (self.int_comeEditType == 2 || self.int_comeEditType == 3 || self.int_comeEditType == 5 || self.int_comeEditType == 7) {
...
-(void)contectHasDataWithTableName:(NSString *)tableName{
    ...
    NSMutableArray *travelFieldName = [NSMutableArray arrayWithArray:@[@"FirstHandlerUserId",@"FirstHandlerUserName",@"IsApprovalAuthority",@"BudgetSubDate",@"IsReceiptOfInv",@"ShareDeptIds", @"PmtMethodId",@"PmtMethod"]];
    if (![NSString isEqualToNull:self.str_twoHandeId]) {
        self.str_twoHandeId=@"";
    }
    NSMutableArray *travelfieldValues = [NSMutableArray arrayWithArray:@[self.str_twoHandeId,self.str_twoApprovalName,self.str_IsApprovalAuthority,self.str_BudgetSubDate,self.str_ReceiptOfInv,self.str_ShareDeptIds,self.str_PayCode,self.str_PayMode]];
...
-(void)contectHasPayDataWithTableName:(NSString *)tableName{
    ...
    NSMutableArray *travelFieldName = [NSMutableArray arrayWithArray:@[@"twohandleruserid",@"twohandlerusername",@"IsApprovalAuthority",@"BudgetSubDate",@"IsReceiptOfInv",@"ShareDeptIds", @"PmtMethodId",@"PmtMethod"]];
    NSMutableArray *travelfieldValues = [NSMutableArray arrayWithArray:@[@"",@"",self.str_IsApprovalAuthority,self.str_BudgetSubDate,self.str_ReceiptOfInv,self.str_ShareDeptIds,self.str_PayCode,self.str_PayMode]];
    if (self.int_formStatus == 3) {
        [travelFieldName addObjectsFromArray:@[@"IsOverBud",@"IsOverStd",@"IsOverStd2"]];
        [travelfieldValues addObjectsFromArray:@[self.dict_JudgeAmount[@"IsOverBud"],self.dict_JudgeAmount[@"IsOverStd"],self.dict_JudgeAmount[@"IsOverStd2"]]];
    }
...
-(NSString *)getCommonField{
    ...
    if (self.int_formStatus==1) {
        ...
        dict=@{@"IsEdit":[[NSString stringWithFormat:@"%@",self.str_beforeBudgetSubDate] isEqualToString:[NSString stringWithFormat:@"%@",self.str_BudgetSubDate]]?@"0":@"1",@"EntertainAppNumber":[NSString isEqualToNull:self.str_EntertainNumber]?self.str_EntertainNumber:@"",@"VehicleSvcAppNumber":[NSString isEqualToNull:self.str_VehicleSvcNumber]?self.str_VehicleSvcNumber:@""};
    }
