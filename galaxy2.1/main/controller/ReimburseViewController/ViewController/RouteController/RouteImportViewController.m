//
//  RouteImportViewController.m
//  galaxy
//
//  Created by 贺一鸣 on 2017/8/21.
...
@interface RouteImportViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UIPickerViewDelegate,UIPickerViewDataSource,chooseTravelDateViewDelegate,GPClientDelegate>
...
@implementation RouteImportViewController
...
-(void)updateGenreView{
    [_GenreView updateConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@60);
    }];
    
    [_GenreView addSubview:[self createDownLineView]];
    ...
    [_GenreView addSubview:titleLbl];
    
    _genreLab=[GPUtils createLable:CGRectMake(0, 0,Main_Screen_Width-160, 18) text:Custing(@"日常Add", nil) font:Font_Important_15_20 textColor:Color_GrayDark_Same_20 textAlignment:NSTextAlignmentRight];
    _genreLab.center=CGPointMake(Main_Screen_Width/2+38, 35);
    [_GenreView addSubview:_genreLab];
    ...
    [_GenreView addSubview:genreImage];
    ...
    [_GenreView addSubview:btn];
    
}

//MARK:更新费用类别选择
-(void)updateCateView{
    [_CateView updateConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@60);
    }];
    ...
    [_CateView addSubview:lineUp];
    
    [_CateView addSubview:[self createLineView]];
    ...
    [_CateView addSubview:_categoryImage];
    ...
    [_CateView addSubview:_CateImage];
...
-(void)requestExpuserImportBatchExp{
    NSString *url=[NSString stringWithFormat:@"%@",ExpuserImportBatchDriveCar];
    NSMutableArray *arr = [NSMutableArray array];
    for (int i = 0; i<_Arr_Id.count; i++) {
        NSDictionary *dic = @{@"dataSource":@13,
                              @"driveCarId":_Arr_Id[i],
                              @"expenseCatCode":_expenseCatCode,
                              @"expenseCat":_expenseCat,
                              @"expenseCode":_expenseCode,
                              @"expenseType":_categoryString,
                              @"expenseDate":[NSString stringWithDate:[NSDate date]],
                              @"mileage":_Arr_Mileage[i],
                              @"sACityName":_Arr_sACityName[i],
                              @"sDCityName":_Arr_sDCityName[i],
                              @"type":_typeStr,
    ...
    [[GPClient shareGPClient]REquestByPostWithPath:url Parameters:@{@"Data":jsonString} Delegate:self SerialNum:1 IfUserCache:NO];
    [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
 }
