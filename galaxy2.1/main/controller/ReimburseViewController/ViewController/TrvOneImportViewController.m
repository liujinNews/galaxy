...
@implementation TrvOneImportViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    [self setTitle:Custing(@"差旅壹号", nil) backButton:YES];
    _segIndex = 0;
    [self createViews];
...
-(void)createViews{
    ...
    RouteTrvOneDateView *route = [[RouteTrvOneDateView alloc]initView:_txf_state_date endTxf:_txf_end_date];
    route.block = ^{
        self.currPage=1;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_cell==nil) {
        _cell=[[TrvOneImportCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"DiDiImportCell"];
    }
...
//            }
            ...
            TrvOneImportCell *cell = [self.tableView cellForRowAtIndexPath:indexPath];
