//
//  WeChatInvoiceController.m
//  galaxy
//
//  Created by hfk on 2017/10/15.
//  Copyright © 2017年 赵碚. All rights reserved.
//

#import "WeChatInvoiceController.h"

@interface WeChatInvoiceController ()
...
@implementation WeChatInvoiceController
            ...
            if (![_resultDict[@"result"]isKindOfClass:[NSNull class]]) {
                ...
                if (arr.count>0) {
                    ...
                    for (NSDictionary *dict in arr) {
                        WeChatInvoiceModel *model=[[WeChatInvoiceModel alloc]init];
                        [model setValuesForKeysWithDictionary:dict];
                        [resultarr addObject:model];
                    }
                    WeChatResultController *vc=[[WeChatResultController alloc]init];
                    vc.dataSoure=resultarr;
                    vc.type=self.str_typeName;
                    vc.category=[GPUtils getSelectResultWithArray:@[_expenseCat,_categoryString]];
                    vc.backIndex=@"0";
                    [self.navigationController pushViewController:vc animated:YES];
                }else if ([NSString isEqualToNull:_resultDict[@"msg"]]){
                    [[GPAlertView sharedAlertView]showAlertText:self WithText:[NSString stringWithFormat:@"%@",_resultDict[@"msg"]] duration:1.5];
                    return;
                }
            }
        }
...
-(void)selectedTypeOrCateary{
    ...
    [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
        if (![weakSelf.str_type isEqualToString:Model.Id]) {
            ...
            [weakSelf chooseAgainCostCategry];
        }
    }];
