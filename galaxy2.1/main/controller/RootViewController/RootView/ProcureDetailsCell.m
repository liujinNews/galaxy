...
//*************************************************费用类别明细***************************************************************//
-(void)configContractExpTypeCellWithArray:(NSMutableArray *)array withDetailsModel:(ExpTypeDetail *)deModel withindex:(NSInteger)index withCount:(NSInteger)count{
    [self.mainView removeFromSuperview];
    self.mainView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, Main_Screen_Width, [ProcureDetailsCell ContractExpTypeCellHeightWithArray:array WithModel:deModel])];
    [self.contentView addSubview:self.mainView];
    
    UILabel *titleLabel=[GPUtils createLable:CGRectMake(12, 10, Main_Screen_Width-55, 18) text:nil font:Font_Important_15_20 textColor:Color_CellDark_Same_28 textAlignment:NSTextAlignmentLeft];
    if (count>1) {
        titleLabel.text=[NSString stringWithFormat:@"%@(%ld)",Custing(@"费用类别", nil),(long)(index+1)];
    }else{
        titleLabel.text=[NSString stringWithFormat:@"%@",Custing(@"费用类别", nil)];
    }
    [self.mainView addSubview:titleLabel];
    ...
    for (MyProcurementModel *model in array) {
        ...
        NSString *content=[ProcureDetailsCell ContractExpTypeContentWithShowModel:model WithContentModel:deModel];
        CGSize size1 = [model.Description sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(XBHelper_Title_Width, 10000) lineBreakMode:NSLineBreakByCharWrapping];
        CGSize size = [content sizeCalculateWithFont:DetailLabel.font constrainedToSize:CGSizeMake(DetailLabel.frame.size.width, 10000) lineBreakMode:DetailLabel.lineBreakMode];
        NSInteger viewHeigh=(size.height>size1.height)?size.height:size1.height;
        nameLabel.frame=CGRectMake(12, height, XBHelper_Title_Width, viewHeigh);
...
+(CGFloat)ContractExpTypeCellHeightWithArray:(NSMutableArray *)arr WithModel:(ExpTypeDetail *)CellModel{
    NSInteger height=33;
    for (MyProcurementModel *models in arr) {
        NSString *content=[ProcureDetailsCell ContractExpTypeContentWithShowModel:models WithContentModel:CellModel];
        CGSize size1 = [models.Description sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(XBHelper_Title_Width, 10000) lineBreakMode:NSLineBreakByCharWrapping];
        if ([NSString isEqualToNull:content]) {
            CGSize size = [content sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(Main_Screen_Width-XBHelper_Title_Width-39, 10000) lineBreakMode:NSLineBreakByCharWrapping];
            height=height+5+((size.height>size1.height)?size.height:size1.height);
...
+(NSString *)ContractExpTypeContentWithShowModel:(MyProcurementModel *)showModel WithContentModel:(ExpTypeDetail *)contentModel{
    NSString *content=@"";
    if ([showModel.fieldName isEqualToString:@"LocalCyAmount"]) {
        content=[GPUtils transformNsNumber:contentModel.LocalCyAmount];
    }else if ([showModel.fieldName isEqualToString:@"ExpenseCode"]){
        content=[GPUtils getSelectResultWithArray:@[contentModel.ExpenseCat,contentModel.ExpenseType] WithCompare:@"/"];
    }else if ([showModel.fieldName isEqualToString:@"Amount"]){
        content=[GPUtils transformNsNumber:contentModel.Amount];
    }else if ([showModel.fieldName isEqualToString:@"PurchaseNumber"]){
        content=[NSString stringIsExist:contentModel.PurchaseInfo];
    }else if([showModel.fieldName isEqualToString:@"No"]){
        content = [NSString isEqualToNull:contentModel.No]?contentModel.No:@"";
    }
    return content;
}
///////////////////////////**********************************入库单*********************************************///////////////////////
...
-(void)configPurAmCellWithArray:(NSMutableArray *)array withDetailsModel:(DeatilsModel *)deModel withindex:(NSInteger)index withCount:(NSInteger)count{
    ...
    UILabel *titleLabel=[GPUtils createLable:CGRectMake(12, 10, Main_Screen_Width-55, 18) text:nil font:Font_Important_15_20 textColor:Color_CellDark_Same_28 textAlignment:NSTextAlignmentLeft];
    if (count>1) {
        titleLabel.text=[NSString stringWithFormat:@"%@(%ld)",Custing(@"采购项目明细及金额", nil),(long)(index+1)];
    }else{
        titleLabel.text=[NSString stringWithFormat:@"%@",Custing(@"采购项目明细及金额", nil)];
    }
    [self.mainView addSubview:titleLabel];
...
-(void)configPurBuCellWithArray:(NSMutableArray *)array withDetailsModel:(DeatilsModel *)deModel withindex:(NSInteger)index withCount:(NSInteger)count{
    ...
    UILabel *titleLabel=[GPUtils createLable:CGRectMake(12, 10, Main_Screen_Width-55, 18) text:nil font:Font_Important_15_20 textColor:Color_CellDark_Same_28 textAlignment:NSTextAlignmentLeft];
    if (count>1) {
        titleLabel.text=[NSString stringWithFormat:@"%@(%ld)",Custing(@"采购项目", nil),(long)(index+1)];
    }else{
        titleLabel.text=[NSString stringWithFormat:@"%@",Custing(@"采购项目", nil)];
    }
    [self.mainView addSubview:titleLabel];
...
//////////////////////////**********************************金额汇总明细*********************************************///////////////////////
- (void)configSummaryCellWithArray:(NSMutableArray *)array withDetailsModel:(SummaryDetail *)deModel withIndex:(NSInteger)index withCount:(NSInteger)count{
    [self.mainView removeFromSuperview];
    self.mainView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, Main_Screen_Width, [ProcureDetailsCell SummaryCellHeightWithArray:array WithModel:deModel])];
    [self.contentView addSubview:self.mainView];
    if (index == 0 && count > 1) {
        CGSize size = [Custing(@"展开", nil) sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(Main_Screen_Width, 30) lineBreakMode:NSLineBreakByCharWrapping];
        CGFloat titleWidth = size.width;
    ...
    for (MyProcurementModel *model in array) {
        ...
        detailLabel.numberOfLines = 0;
        [self.mainView addSubview:detailLabel];
        NSString *content = [ProcureDetailsCell SummaryContentWithShowModel:model WithContentModel:deModel];
        CGSize size1 = [model.Description sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(XBHelper_Title_Width, 10000) lineBreakMode:NSLineBreakByCharWrapping];
        CGSize size = [content sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(detailLabel.frame.size.width, 10000) lineBreakMode:detailLabel.lineBreakMode];
        NSInteger viewHeight = (size.height>size1.height)?size.height:size1.height;
        detailLabel.frame = CGRectMake(XBHelper_Title_Width+27, Y(nameLabel), size.width, viewHeight);
        detailLabel.text = content;
        [nameLabel sizeToFit];
        [detailLabel sizeToFit];
        height = height + viewHeight + 5;
    }
}
+(CGFloat)SummaryCellHeightWithArray:(NSMutableArray *)arr WithModel:(SummaryDetail *)CellModel{
    NSInteger height=33;
    for (MyProcurementModel *models in arr) {
        NSString *content=[ProcureDetailsCell SummaryContentWithShowModel:models WithContentModel:CellModel];
        CGSize size1 = [models.Description sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(XBHelper_Title_Width, 10000) lineBreakMode:NSLineBreakByCharWrapping];
        if ([NSString isEqualToNull:content]) {
            CGSize size = [content sizeCalculateWithFont:Font_Important_15_20 constrainedToSize:CGSizeMake(Main_Screen_Width-XBHelper_Title_Width-39, 10000) lineBreakMode:NSLineBreakByCharWrapping];
            height=height+5+((size.height>size1.height)?size.height:size1.height);
...
+(NSString *)SummaryContentWithShowModel:(MyProcurementModel *)showModel WithContentModel:(SummaryDetail *)contentModel{
    NSString *content=@"";
//    if ([showModel.fieldName isEqualToString:@"ExpenseDesc"]){
//        content=contentModel.ExpenseDesc;
//
//    }
    return content;
