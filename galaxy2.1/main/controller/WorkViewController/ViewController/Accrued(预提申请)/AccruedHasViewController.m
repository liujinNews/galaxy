...
#import "AccruedHasViewController.h"

@interface AccruedHasViewController ()

@end

@implementation AccruedHasViewController


-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas = [[AccruedFormData alloc]initWithStatus:2];
    }
    return self;
}
...
-(void)createMainView{
    ...
    _AccruedReqDetailView = [[AccruedReqDetailView alloc]init];
    [self.contentView addSubview:_AccruedReqDetailView];
    [_AccruedReqDetailView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_view_line4 makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.AccruedReqDetailView.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    //金额汇总表
    _View_SummaryTable = [[UITableView alloc]initWithFrame:CGRectZero style:UITableViewStylePlain];
    _View_SummaryTable.backgroundColor=Color_WhiteWeak_Same_20;
    _View_SummaryTable.delegate=self;
    _View_SummaryTable.dataSource=self;
    _View_SummaryTable.scrollEnabled=NO;
    _View_SummaryTable.separatorStyle=UITableViewCellSeparatorStyleNone;
    [self.contentView addSubview:_View_SummaryTable];
    [_View_SummaryTable makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_View_Approve makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_SummaryTable.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
//    }
    ...
    if (self.FormDatas.arr_DetailsDataArray && self.FormDatas.arr_DetailsDataArray.count != 0 && self.FormDatas.bool_DetailsShow) {
        [self updateAccruedReqDetailView];
    }
    //预算视图
...
-(void)updateIsPaymentViewWithModel:(MyProcurementModel *)model{
    if ([model.fieldValue isEqualToString:@"0"]) {
        model.fieldValue = Custing(@"未扣款", nil);
    }
    if ([model.fieldValue isEqualToString:@"1"]) {
        model.fieldValue = Custing(@"已扣款", nil);
    }
    [_View_IsPayment addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_IbanADDRESS updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
...
//MARK:更新费用明细视图
-(void)updateAccruedReqDetailView{
    [_AccruedReqDetailView updatePaymentExpMainViewWithData:self.FormDatas.arr_DetailsDataArray WithEditType:2];
    __weak typeof(self) weakSelf = self;
    _AccruedReqDetailView.AccruedReqBackClickedBlock = ^(NSInteger type, NSInteger index, AccruedReqDetail * _Nonnull model) {
        AccruedReqDetailHasController *vc = [[AccruedReqDetailHasController alloc]init];
        vc.AccruedReqDetail = [model copy];
...
//MARK:更新金额汇总明细
- (void)updateSummaryTableViewWithModel:(MyProcurementModel *)model{
    NSInteger height = 10;
...
//    }
    [_View_SummaryTable updateConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@(height));
    }];
    [_View_SummaryTable reloadData];
}
