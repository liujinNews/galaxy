...
#import "AccruedNewViewController.h"
#import "SelectCardViewController.h"
//#import "PayMentDetailLeadController.h"
#import "AccruedDetailLeadController.h"
@interface AccruedNewViewController ()

@end

@implementation AccruedNewViewController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas = [[AccruedFormData alloc]initWithStatus:1];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)createMainView{
    ...
    _accruedRDView = [[AccruedReqDetailView alloc]init];
    [self.contentView addSubview:_accruedRDView];
    [_accruedRDView mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_View_HasInvoice mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.accruedRDView.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
//MARK:更新费用明细视图
-(void)updateAccruedRDView{
    [_accruedRDView updatePaymentExpMainViewWithData:self.FormDatas.arr_DetailsDataArray WithEditType:1];
    __weak typeof(self) weakSelf = self;
    _accruedRDView.AccruedReqBackClickedBlock = ^(NSInteger type, NSInteger index, AccruedReqDetail * _Nonnull model) {
        //type1新增2修改3删除4财务修改
        if (type == 3) {
            ...
            AccruedReqDetailNewController *vc = [[AccruedReqDetailNewController alloc]init];
            vc.dict_parameter = @{
            ...
            vc.AccruedReqDetailAddEditBlock = ^(AccruedReqDetail * _Nonnull model, NSInteger type,NSMutableArray *accruedArr) {
                if (type==1) {
                ...
                [weakSelf.accruedRDView updateTableView];
    ...
    _accruedRDView.AccruedLeadClickedBlock = ^{//导入费用明细
        //type1新增2修改3删除4财务修改
//        static NSInteger maxNo = 0;

        NSMutableArray *array = [NSMutableArray array];

        AccruedDetailLeadController *vc = [[AccruedDetailLeadController alloc]init];
        ...
        vc.importDetailBackBlock = ^(NSMutableArray * _Nonnull array) {
            for (AccruedReqDetail *pay in array) {
...
-(void)ContractClick{
    [self keyClose];
    __weak typeof(self) weakSelf = self;
    ChooseCateFreshController *vc = [[ChooseCateFreshController alloc]initWithType:self.FormDatas.int_isContractPaymentMethod == 1 ? @"ContractsIs":@"Contracts"];//ContractsV3
