...
@interface CancelFlowNewController ()

@end

@implementation CancelFlowNewController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas = [[CancelFlowFormData alloc]initWithStatus:1];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)createMainView{
    ...
    _View_CancelForm = [[UIView alloc]init];
    _View_CancelForm.backgroundColor = Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_CancelForm];
    [_View_CancelForm mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_Project.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    
    _View_FlowName = [[UIView alloc]init];
    _View_FlowName.backgroundColor = Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_FlowName];
    [_View_FlowName mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_CancelForm.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_Reason mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_FlowName.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        if ([model.fieldName isEqualToString:@"FormSerialNo"]) {
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                ...
                [self updateCancelFormViewWithModel:model];
        ...
        }else if ([model.fieldName isEqualToString:@"FlowName"]) {
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateFlowNameViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
...
//MARK:更新撤销申请单
-(void)updateCancelFormViewWithModel:(MyProcurementModel *)model{
    _txf_CancelForm = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_CancelForm WithContent:_txf_CancelForm WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.FormDatas.str_FormReason}];
    __weak typeof(self) weakSelf = self;
    view.FormClickedBlock = ^(MyProcurementModel *model) {
        ChooseCateFreshController *vc = [[ChooseCateFreshController alloc]initWithType:@"FormReason"];
        vc.ChooseCategoryId = weakSelf.FormDatas.str_FormTaskId;
        vc.dict_otherPars=@{@"Type":@"0"};
        vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
            ChooseCateFreModel *model = array[0];
            weakSelf.FormDatas.str_FormReason = [GPUtils getSelectResultWithArray:@[[NSString stringWithFormat:@"%@",model.serialNo],[NSString stringWithFormat:@"%@",model.taskName]] WithCompare:@"/"];
            weakSelf.txf_CancelForm.text = weakSelf.FormDatas.str_FormReason;
            weakSelf.FormDatas.str_FormFlowName = model.flowName;
            weakSelf.txf_FlowName.text = model.flowName;
            weakSelf.FormDatas.str_FormFlowCode = model.flowCode;
            weakSelf.FormDatas.str_FormTaskId = model.taskId;
            weakSelf.FormDatas.str_FormSerialNo = model.serialNo;
        };
        [weakSelf.navigationController pushViewController:vc animated:YES];
    };
    [_View_CancelForm addSubview:view];
}
//MARK:更新流程名称
-(void)updateFlowNameViewWithModel:(MyProcurementModel *)model{
    _txf_FlowName = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_FlowName WithContent:_txf_FlowName WithFormType:formViewShowText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_FlowName addSubview:view];
}
//MARK:更新理由
