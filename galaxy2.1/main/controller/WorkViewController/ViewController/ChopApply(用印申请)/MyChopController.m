...
@interface MyChopController ()

@end

@implementation MyChopController
-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[MyChopFormData alloc]initWithStatus:1];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)createMainView{
    ...
    [_View_ChopDate mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.SubmitPersonalView.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    
    _View_ChopFile_Name=[[UIView alloc]init];
    _View_ChopFile_Name.backgroundColor=Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_ChopFile_Name];
    [_View_ChopFile_Name mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_ChopDate.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
    
    _View_ChopFile_Type=[[UIView alloc]init];
    _View_ChopFile_Type.backgroundColor=Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_ChopFile_Type];
    [_View_ChopFile_Type mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_ChopFile_Name.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_Project mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_ChopFile_Type.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        if ([model.fieldName isEqualToString:@"SealDate"]){
        ...
        }else if ([model.fieldName isEqualToString:@"SealName"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateSealNameViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
        }else if ([model.fieldName isEqualToString:@"FileTypeId"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
    ...
    if (self.FormDatas.bool_DetailsShow) {
        if (self.FormDatas.arr_DetailsDataArray.count==0) {
            MyChopDeatil *model=[[MyChopDeatil alloc]init];
...
-(void)updateSealDateViewWithModel:(MyProcurementModel *)model{
    _txf_ChopDate=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ChopDate WithContent:_txf_ChopDate WithFormType:formViewSelectDate WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_ChopDate addSubview:view];
}
//MARK:更新用印文件名称
-(void)updateSealNameViewWithModel:(MyProcurementModel *)model{
    _txf_ChopFile_Name=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ChopFile_Name WithContent:_txf_ChopFile_Name WithFormType:formViewEnterText  WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_ChopFile_Name addSubview:view];
}

//MARK:更新类型
-(void)updateFileTypeViewWithModel:(MyProcurementModel *)model{
    _txf_ChopFile_Type=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ChopFile_Type WithContent:_txf_ChopFile_Type WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.FormDatas.str_FileType}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
        ChooseCateFreshController *vc=[[ChooseCateFreshController alloc]initWithType:@"ConfigurationItem"];
        vc.ChooseCategoryId=weakSelf.FormDatas.str_FileTypeId;
        vc.ChooseModel=model;
        vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
            ChooseCateFreModel *model = array[0];
            weakSelf.txf_ChopFile_Type.text=model.name;
            weakSelf.FormDatas.str_FileType=model.name;
            weakSelf.FormDatas.str_FileTypeId=model.Id;
        };
        [weakSelf.navigationController pushViewController:vc animated:YES];
    }];
    self.FormDatas.str_FileTypeId=[NSString stringWithFormat:@"%@",model.fieldValue];
    [_View_ChopFile_Type addSubview:view];
}
//MARK:更新项目视图
...
-(void)updateAddDetailsView{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initAddBtbWithBaseView:_View_AddDetails withTitle:Custing(@"增加明细", nil) withTitleAlignment:1 withImageArray:@[@"commom_addDetails_Icon"] withBtnLocation:1 withlineStyle:0];
        __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model) {
        MyChopDeatil *model1=[[MyChopDeatil alloc]init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell configChopCellWithModel:self.FormDatas.arr_DetailsArray[indexPath.row] withDetailsModel:self.FormDatas.arr_DetailsDataArray[indexPath.section] WithCount:self.FormDatas.arr_DetailsArray.count WithIndex:indexPath.row];
    ...
    if (cell.SupplierBtn) {
        ...
        [cell setCellClickedBlock:^(NSIndexPath *index,UITextField *tf){
            ...
            [weakSelf keyClose];
            MyChopDeatil *modelD=[weakSelf.FormDatas.arr_DetailsDataArray objectAtIndex:index.section];
            if (!modelD) {
                MyChopDeatil *modelD=[[MyChopDeatil alloc]init];
                [weakSelf.FormDatas.arr_DetailsDataArray insertObject:modelD atIndex:index.section];
            }
            ChooseCateFreshController *vc=[[ChooseCateFreshController alloc]initWithType:@"ConfigurationItem"];
            vc.ChooseCategoryId=modelD.SealTypeId;
            ...
            vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
                ChooseCateFreModel *model = array[0];
                modelD.SealTypeId = model.Id;
                modelD.SealType=model.name;
                tf.text =modelD.SealType;
            };
            [weakSelf.navigationController pushViewController:vc animated:YES];
        }];
    }
...
{  //string就是此时输入的那个字符textField就是此时正在输入的那个输入框返回YES就是可以改变输入框的值NO相反
    ...
    NSString * toBeString = [textField.text stringByReplacingCharactersInRange:range withString:string]; //得到输入框的内容
     if (textField.tag>=10000&&textField.tag<=10051){
         NSString *pattern;
         pattern = @"^([1-9][0-9]{0,4})?$";
         NSError *error = nil;
...
-(void)createHeadViewWithSection:(NSInteger)section{
    ...
    if (self.FormDatas.arr_DetailsDataArray.count==1) {
        titleLabel.text=Custing(@"公章明细", nil);
    }else{
        titleLabel.text=[NSString stringWithFormat:@"%@(%ld)",Custing(@"公章明细", nil),(long)section+1];
        if (section!=0) {
...
{
    MyChopDeatil *model=[self.FormDatas.arr_DetailsDataArray objectAtIndex:text.tag-10000];
    if (!model) {
        MyChopDeatil *model=[[MyChopDeatil alloc]init];
...
-(void)deleteDetails:(UIButton *)btn{
    ...
    [self keyClose];
    NSLog(@"删除明细");
    _Aler_DeleteDetils=[[UIAlertView alloc]initWithTitle:@"" message:[NSString stringWithFormat:@"%@%ld?",Custing(@"你确定要删除公章明细", nil),(long)(btn.tag-1200+1)] delegate:self cancelButtonTitle:Custing(@"取消",nil) otherButtonTitles:Custing(@"删除",nil), nil];
...
-(void)configModelOtherData{
    self.FormDatas.SubmitData.SealDate=_txf_ChopDate.text;
    self.FormDatas.SubmitData.SealName=_txf_ChopFile_Name.text;
