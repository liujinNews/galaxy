...
@interface MyChopHasController ()

@end

@implementation MyChopHasController
-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[MyChopFormData alloc]initWithStatus:2];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)createMainView{
    ...
    _View_ChopDate=[[UIView alloc]init];
    _View_ChopDate.backgroundColor=Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_ChopDate];
    [_View_ChopDate makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.view_line1.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    
    _View_ChopFile=[[UIView alloc]init];
    _View_ChopFile.backgroundColor=Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_ChopFile];
    [_View_ChopFile makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_ChopDate.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    
    _View_ChopType=[[UIView alloc]init];
    _View_ChopType.backgroundColor=Color_WhiteWeak_Same_20;
    [self.contentView addSubview:_View_ChopType];
    [_View_ChopType makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_ChopFile.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_Project mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_ChopType.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        if ([model.fieldName isEqualToString:@"SealDate"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateSealDateViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"SealName"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateSealNameViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"FileTypeId"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateFileTypeViewWithModel:model];
...
//MARK:更新用印日期
-(void)updateSealDateViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_ChopDate addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_ChopDate updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
//MARK:更新用印文件名称
-(void)updateSealNameViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_ChopFile addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_ChopFile updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
//MARK:更新用印类型
-(void)updateFileTypeViewWithModel:(MyProcurementModel *)model{
    model.fieldValue=[NSString isEqualToNull:self.FormDatas.str_FileType]?[NSString stringWithFormat:@"%@",self.FormDatas.str_FileType]:@"";
    __weak typeof(self) weakSelf = self;
    [_View_ChopType addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_ChopType updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateDetailsTableView{
    if (self.FormDatas.bool_isOpenDetail) {
        NSInteger height=10;
        for (MyChopDeatil *model in self.FormDatas.arr_DetailsDataArray) {
            height=height+[ProcureDetailsCell MyChopCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
        }
        [_View_DetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }else{
        MyChopDeatil *model=self.FormDatas.arr_DetailsDataArray[0];
        NSInteger height=10+[ProcureDetailsCell MyChopCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
        [_View_DetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }
...
{
    MyChopDeatil *model=self.FormDatas.arr_DetailsDataArray[indexPath.row];
    return [ProcureDetailsCell MyChopCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell configMyChopCellWithArray:self.FormDatas.arr_DetailsArray withDetailsModel:self.FormDatas.arr_DetailsDataArray[indexPath.row] withindex:indexPath.row withCount:self.FormDatas.arr_DetailsDataArray.count] ;
    if (cell.LookMore) {
