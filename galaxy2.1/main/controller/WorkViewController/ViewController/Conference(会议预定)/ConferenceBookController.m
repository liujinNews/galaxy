...
@interface ConferenceBookController ()

@end

@implementation ConferenceBookController
-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[ConferenceFormData alloc]initWithStatus:1];
    }
    return self;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(chooseRoomResult:) name:@"GETROOMIDADNTIME" object:nil];
...
-(void)createMainView{
    ...
    [_View_RoomName mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_View_ToDate mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_FromDate.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_Staff mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_ToDate.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_MeetingNum mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_Staff.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_Equipment mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_MeetingNum.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_OpenMethod mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_Equipment.mas_bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        ...
        }else if ([model.fieldName isEqualToString:@"TypeId"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateTypeViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
        }else if ([model.fieldName isEqualToString:@"RoomId"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
        ...
        }else if ([model.fieldName isEqualToString:@"StaffId"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateStaffViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
        }else if ([model.fieldName isEqualToString:@"MeetingNum"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateMeetingNumViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
        }else if ([model.fieldName isEqualToString:@"Equipment"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
                [self updateEquipmentViewWithModel:model];
                [self.FormDatas.arr_UnShowmsArray removeObject:model.fieldName];
            }
        }else if ([model.fieldName isEqualToString:@"OpenMethod"]){
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
...
-(void)updateTypeViewWithModel:(MyProcurementModel *)model{
    ...
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_Type WithContent:_txf_Type WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.FormDatas.str_Type}];
    __weak typeof(self) weakSelf = self;
    ...
    if ([NSString isEqualToNullAndZero:model.fieldValue]) {
        self.FormDatas.str_TypeId=[NSString stringWithFormat:@"%@",model.fieldValue];
    }
    [_View_Type addSubview:view];
}
//MARK:会议室名称
-(void)updateRoomViewWithModel:(MyProcurementModel *)model{
    _txf_RoomName=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_RoomName WithContent:_txf_RoomName WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.FormDatas.str_RoomName}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
        [weakSelf RoomClick];
    }];
    [_View_RoomName addSubview:view];
    
    if ([NSString isEqualToNullAndZero:model.fieldValue]) {
        self.FormDatas.str_RoomId=[NSString stringWithFormat:@"%@",model.fieldValue];
    }
}
//MARK:会议开始时间
-(void)updateFromDateViewWithModel:(MyProcurementModel *)model{
    ...
    if ([NSString isEqualToNullAndZero:model.fieldValue]) {
        self.FormDatas.str_FromeDate=[NSString stringWithFormat:@"%@",model.fieldValue];
    }
}
//MARK:会议结束时间
-(void)updateToDateViewWithModel:(MyProcurementModel *)model{
    _txf_ToDate=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_ToDate WithContent:_txf_ToDate WithFormType:formViewShowText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_ToDate addSubview:view];
    if ([NSString isEqualToNullAndZero:model.fieldValue]) {
        self.FormDatas.str_ToDate=[NSString stringWithFormat:@"%@",model.fieldValue];
    }
}
//MARK:会议人员
-(void)updateStaffViewWithModel:(MyProcurementModel *)model{
    _txf_Staff = [[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_Staff WithContent:_txf_Staff WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.FormDatas.str_StaffName}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
        [weakSelf StaffClick];
    }];
    [_View_Staff addSubview:view];
    if ([NSString isEqualToNullAndZero:model.fieldValue]) {
        self.FormDatas.str_StaffId=[NSString stringWithFormat:@"%@",model.fieldValue];
    }
}
//MARK:会议人数
-(void)updateMeetingNumViewWithModel:(MyProcurementModel *)model{
    _txf_MeetingNum=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_MeetingNum WithContent:_txf_MeetingNum WithFormType:formViewEnterNum WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_MeetingNum addSubview:view];
}
//MARK:会议设备
-(void)updateEquipmentViewWithModel:(MyProcurementModel *)model{
    _txf_Equipment=[[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_Equipment WithContent:_txf_Equipment WithFormType:formViewEnterText WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_Equipment addSubview:view];
}
//MARK:会议公开方式
-(void)updateOpenMethodViewWithModel:(MyProcurementModel *)model{
    _txf_OpenMethod = [[UITextField alloc]init];
    SubmitFormView *view=[[SubmitFormView alloc]initBaseView:_View_OpenMethod WithContent:_txf_OpenMethod WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":[self.FormDatas getOpenMethodId:model.fieldValue]}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
        [weakSelf OpenMethodClick];
    }];
    [_View_OpenMethod addSubview:view];
}
//MARK:更新明细
...
-(void)updateAddDetailsView{
    ...
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model) {
        ConferenceDeatil *model1=[[ConferenceDeatil alloc]init];
...
//MARK:修改会议室选择
-(void)RoomClick{
    MeetingRoomController *vc=[[MeetingRoomController alloc]init];
    [self.navigationController pushViewController:vc animated:YES];
}
//MARK:会议室修改返回值
-(void)chooseRoomResult:(NSNotification *)not{
    NSDictionary *dict = not.userInfo;
    self.FormDatas.str_FromeDate=dict[@"FromTime"];
    self.txf_FromDate.text=self.FormDatas.str_FromeDate;
    self.FormDatas.str_ToDate=dict[@"ToTime"];
    self.txf_ToDate.text=self.FormDatas.str_ToDate;
    self.FormDatas.str_RoomId=dict[@"RoomId"];
    self.FormDatas.str_RoomName=dict[@"RoomName"];
    self.txf_RoomName.text=self.FormDatas.str_RoomName;
...
-(void)StaffClick{
    ...
    NSArray *idarr = [self.FormDatas.str_StaffId componentsSeparatedByString:@","];
    ...
    [contactVC setBlock:^(NSMutableArray *array) {
        ...
        weakSelf.FormDatas.str_StaffId=[GPUtils getSelectResultWithArray:idArr WithCompare:@","];
        weakSelf.FormDatas.str_StaffName=[GPUtils getSelectResultWithArray:nameArr WithCompare:@","];
        weakSelf.txf_Staff.text=weakSelf.FormDatas.str_StaffName;
        weakSelf.txf_MeetingNum.text=[NSString stringWithFormat:@"%lu",(unsigned long)array.count];
    }];
    [self.navigationController pushViewController:contactVC animated:YES];
}
//MARK:公开方式
-(void)OpenMethodClick{
    STOnePickView *picker = [[STOnePickView alloc]init];
    picker.typeTitle=Custing(@"公开方法", nil);
    __weak typeof(self) weakSelf = self;
    [picker setBlock:^(STOnePickModel *Model, NSInteger type) {
        weakSelf.txf_OpenMethod.text=[weakSelf.FormDatas getOpenMethodId:Model.Id];
    }];
    picker.DateSourceArray=[NSMutableArray arrayWithArray:self.FormDatas.arr_OpenMethod];
    STOnePickModel *model=[[STOnePickModel alloc]init];
    model.Id=self.FormDatas.str_OpenMethodId;
    picker.Model=model;
    [picker UpdatePickUI];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell configConferenceCellWithModel:self.FormDatas.arr_DetailsArray[indexPath.row] withDetailsModel:self.FormDatas.arr_DetailsDataArray[indexPath.section] WithCount:self.FormDatas.arr_DetailsArray.count WithIndex:indexPath.row];
    cell.IndexPath=indexPath;
    
    [cell.SubjectTF addTarget:self action:@selector(SubjectChange:) forControlEvents:UIControlEventEditingChanged];
    cell.SubjectTF.tag=100+indexPath.section;
    cell.SubjectTF.delegate = self;
    
    [cell.SpokesmanTF addTarget:self action:@selector(SpokesmanChange:) forControlEvents:UIControlEventEditingChanged];
    cell.SpokesmanTF.tag=100+indexPath.section;
    cell.SpokesmanTF.delegate = self;
...
{  //string就是此时输入的那个字符textField就是此时正在输入的那个输入框返回YES就是可以改变输入框的值NO相反
    ...
    if (textField.tag>=0&&textField.tag<=151){
        if ([toBeString length] >10) { //如果输入框内容大于255
...
-(void)createHeadViewWithSection:(NSInteger)section{
    ...
    if (self.FormDatas.arr_DetailsDataArray.count==1) {
        titleLabel.text=Custing(@"会议议程", nil);
    }else{
        titleLabel.text=[NSString stringWithFormat:@"%@(%ld)",Custing(@"会议议程", nil),(long)section+1];
        if (section!=0) {
...
//MARK:采购明细详情填写
-(void)SubjectChange:(UITextField *)text
{
    ConferenceDeatil *model=[self.FormDatas.arr_DetailsDataArray objectAtIndex:text.tag-100];
    if (!model) {
        ConferenceDeatil *model=[[ConferenceDeatil alloc]init];
        [self.FormDatas.arr_DetailsDataArray insertObject:model atIndex:text.tag-100];
        model.Subject=text.text;
    }else{
        model.Subject=text.text;
    }
}
-(void)SpokesmanChange:(UITextField *)text
{
    ConferenceDeatil *model=[self.FormDatas.arr_DetailsDataArray objectAtIndex:text.tag-100];
    if (!model) {
        ConferenceDeatil *model=[[ConferenceDeatil alloc]init];
        [self.FormDatas.arr_DetailsDataArray insertObject:model atIndex:text.tag-100];
        model.Spokesman=text.text;
    }else{
        model.Spokesman=text.text;
    }
}
...
-(void)deleteDetails:(UIButton *)btn{
    ...
    [self keyClose];
    NSLog(@"删除明细");
    _Aler_DeleteDetils=[[UIAlertView alloc]initWithTitle:@"" message:[NSString stringWithFormat:@"%@%ld?",Custing(@"你确定要删除会议议程", nil),(long)(btn.tag-1200+1)] delegate:self cancelButtonTitle:Custing(@"取消",nil) otherButtonTitles:Custing(@"删除",nil), nil];
...
-(void)configModelOtherData{
    self.FormDatas.SubmitData.Name=_txf_Name.text;
    self.FormDatas.SubmitData.MeetingNum=_txf_MeetingNum.text;
    self.FormDatas.SubmitData.Equipment=_txf_Equipment.text;
...
{
    [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
    [[GPClient shareGPClient]REquestByPostWithPath:[self.FormDatas getSaveUrl] Parameters:[self.FormDatas SaveFormDateWithExpIds:@"" WithComment:@"" WithCommonField:[self.FormDatas getCommonField]] Delegate:self SerialNum:1 IfUserCache:NO];
}
//MARK:提交
-(void)requestAppSubmit{
    [YXSpritesLoadingView showWithText:Custing(@"光速加载中...",nil) andShimmering:NO andBlurEffect:NO];
    [[GPClient shareGPClient]REquestByPostWithPath:[self.FormDatas getSubmitUrl] Parameters:[self.FormDatas SubmitFormDateWithExpIds:@"" WithComment:@"" WithCommonField:[self.FormDatas getCommonField]] Delegate:self SerialNum:3 IfUserCache:NO];
}
//MARK:退单提交
...
- (void)dealloc
{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"GETROOMIDADNTIME" object:nil];
