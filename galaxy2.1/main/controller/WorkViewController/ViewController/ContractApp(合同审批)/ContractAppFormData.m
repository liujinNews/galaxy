...
-(void)DealWithFormBaseData{
    ...
    if (![result isKindOfClass:[NSNull class]]) {
        ...
        self.bool_FouDetailsShow = [[NSString stringWithFormat:@"%@",result[@"contractExpTypeDetail"]] isEqualToString:@"1"] ? YES:NO;
        ...
        if (self.bool_FouDetailsShow) {
            ...
            //明细主表数据
            if ([result[@"formFields"][@"detailFld"]isKindOfClass:[NSArray class]]) {
                for (NSDictionary *dic in result[@"formFields"][@"detailFld"]) {
                    if ([[[dic objectForKey:@"isShow"]stringValue]isEqualToString:@"1"]) {
                        MyProcurementModel *model=[[MyProcurementModel alloc]init];
                        [model setValuesForKeysWithDictionary:dic];
        ...
        if ([result[@"formData"]isKindOfClass:[NSDictionary class]]&&[result[@"formData"][@"detail"] isKindOfClass:[NSDictionary class]]) {
            ...
            if ([detail[@"sa_ContractPayMethodDetail"] isKindOfClass:[NSArray class]]) {
                for (NSDictionary *dict in detail[@"sa_ContractPayMethodDetail"]) {
                    ...
                    if(![dict[@"contAppOrderNo"] isKindOfClass:[NSNull class]]){
                        model.ContAppOrderNo = [NSString stringWithFormat:@"%@",dict[@"contAppOrderNo"]];
                    }
                    if (![dict[@"contPayMethodExpTypDels"] isKindOfClass:[NSNull class]]) {
                        model.contPayMethodExpTypDels = [NSMutableArray array];
                        NSArray *contPayArr = (NSArray *)[NSString transformToObj:[NSString stringWithFormat:@"%@",dict[@"contPayMethodExpTypDels"]]];
                        model.contPayMethodExpTypDels = [self createContPayArrWithArr:contPayArr];
                        [self.arr_FouDetailsDataArray addObjectsFromArray:[self createContPayArrWithArr:contPayArr]];
                    }
...
- (NSMutableArray *)createContPayArrWithArr:(NSArray *)dataArr{
    NSMutableArray *contPayArr = [NSMutableArray array];
    if (dataArr.count>0) {
        for (NSDictionary *dict in dataArr) {
            ExpTypeDetail *model=[[ExpTypeDetail alloc]init];
            ...
            if (![dict[@"purchaseNumber"] isKindOfClass:[NSNull class]]) {
                model.PurchaseNumber=[NSString stringWithFormat:@"%@",dict[@"purchaseNumber"]];
            }
            if (![dict[@"purchaseInfo"] isKindOfClass:[NSNull class]]) {
                model.PurchaseInfo=[NSString stringWithFormat:@"%@",dict[@"purchaseInfo"]];
            }
            if (![dict[@"amount"] isKindOfClass:[NSNull class]]) {
                model.Amount=[NSString stringWithFormat:@"%@",dict[@"amount"]];
            }
            if (![dict[@"payMethodNo"] isKindOfClass:[NSNull class]]) {
                model.PayMethodNo=[NSString stringWithFormat:@"%@",dict[@"payMethodNo"]];
            }
            [contPayArr addObject:model];
        }
    }
    return contPayArr;
}
...
-(NSString *)testModel{
    ...
    for (NSString *str in self.arr_isShowmsArray) {
            ...
            for (NSString *str in self.arr_SecisShowmDetailArray) {
                ...
                if ([i isEqualToString:@"1"]) {
                    for (ContractPayMethodDetail *model in self.arr_SecDetailsDataArray) {
                        ...
                        }else if([str isEqualToString:@"ContAppOrderNo"]){
                            detailStr=model.ContAppOrderNo;
                        }
                        if (![NSString isEqualToNull:detailStr]) {
                            returnTips=[self showerrorDetail:str];
                            goto when_failed;
        ...
        }else if ([key isEqualToString:@"FourDetailList"]){//费用类别明细
            for (NSString *str in self.arr_FouisShowmDetailArray) {
                NSString *i = [NSString stringWithFormat:@"%@",[self.dict_FouisRequiredmsDetaildic objectForKey:str]];
                if ([i isEqualToString:@"1"]) {
                    for (ExpTypeDetail *model in self.arr_FouDetailsDataArray) {
                        NSString *detailStr;
                        if ([str isEqualToString:@"ExpenseCode"]){
                            detailStr = model.ExpenseCode;
                        }else if ([str isEqualToString:@"LocalCyAmount"]){
                            detailStr = model.LocalCyAmount;
                        }else if ([str isEqualToString:@"PurchaseNumber"]){
                            detailStr = model.PurchaseNumber;
                        }else if ([str isEqualToString:@"PayMethodNo"]){
                            detailStr = model.PayMethodNo;
                        }
                        if (![NSString isEqualToNull:detailStr]) {
                            returnTips = [self showerrorDetail:str];
                            goto when_failed;
...
-(void)contectData{
    ...
    if (self.arr_SecDetailsDataArray.count!=0) {
        ...
        if ([[modelsDic allKeys] containsObject:@"contPayMethodExpTypDels"]) {
            [modelsDic removeObjectForKey:@"contPayMethodExpTypDels"];
        }
    ...
    //费用类别明细
    NSMutableDictionary *Sa_ContractPayMethodExpTypDetail = [[NSMutableDictionary alloc]init];
    NSArray *fieldNames4 = [NSArray array];
    NSMutableArray *Values4 = [NSMutableArray array];
    if (self.arr_FouDetailsDataArray.count != 0) {
        NSMutableDictionary *modelsDic = [FeeAppDeatil initDicByModel:self.arr_FouDetailsDataArray[0]];
        fieldNames4 = [modelsDic allKeys];
        for (NSString *key in fieldNames4) {
            NSMutableArray  *array = [NSMutableArray array];
            for (ExpTypeDetail *model in self.arr_FouDetailsDataArray) {
                if ([NSString isEqualToNull:[model valueForKey:key]]) {
                    [array addObject:[model valueForKey:key]];
            ...
            [Values4 addObject:array];
        }
        [Sa_ContractPayMethodExpTypDetail setObject:@"Sa_ContractPayMethodExpTypDetail" forKey:@"tableName"];
        [Sa_ContractPayMethodExpTypDetail setObject:fieldNames4 forKey:@"fieldNames"];
        [Sa_ContractPayMethodExpTypDetail setObject:Values4 forKey:@"fieldBigValues"];
    }else{
        [Sa_ContractPayMethodExpTypDetail setObject:@"Sa_ContractPayMethodExpTypDetail" forKey:@"tableName"];
        [Sa_ContractPayMethodExpTypDetail setObject:fieldNames4 forKey:@"fieldNames"];
        [Sa_ContractPayMethodExpTypDetail setObject:Values4 forKey:@"fieldBigValues"];
    }
    [detailedArray addObject:Sa_ContractPayMethodExpTypDetail];
