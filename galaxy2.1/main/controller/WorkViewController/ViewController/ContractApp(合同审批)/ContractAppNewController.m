...
-(void)updateMainView{
    ...
    if (self.FormDatas.bool_SecDetailsShow) {
        if (self.FormDatas.arr_SecDetailsDataArray.count == 0) {
            ...
            model.ContAppOrderNo = @"1";
            [self.FormDatas.arr_SecDetailsDataArray addObject:model];
            if (self.FormDatas.arr_FouDetailsDataArray.count == 0) {
                ExpTypeDetail *expModel = [[ExpTypeDetail alloc] init];
                expModel.PayMethodNo = @"1";
                [self.FormDatas.arr_FouDetailsDataArray addObject:expModel];
            }
        }
...
-(void)updatePayModeTableView{
    ...
    for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
        if (![singleArr containsObject:expModel.PayMethodNo]) {
            [singleArr addObject:expModel.PayMethodNo];
        }
    }
    contactCount = singleArr.count;
    NSInteger payModelCount = self.FormDatas.arr_SecDetailsDataArray.count;
    [_View_PayModeTable updateConstraints:^(MASConstraintMaker *make) {
        make.height.equalTo(@((self.FormDatas.arr_SecDetailsArray.count*42+27)*self.FormDatas.arr_SecDetailsDataArray.count+(self.FormDatas.arr_FouDetailsArray.count*42+42)*self.FormDatas.arr_FouDetailsDataArray.count + (payModelCount - contactCount)*42));
    }];
...
-(void)updatePayModeAddView{
    ...
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model) {
        NSInteger secArrCount = weakSelf.FormDatas.arr_SecDetailsDataArray.count;
        ContractPayMethodDetail *model1 = [[ContractPayMethodDetail alloc]init];
        model1.ContAppOrderNo = [NSString stringWithFormat:@"%ld",(long)secArrCount+1];
        [weakSelf.FormDatas.arr_SecDetailsDataArray addObject:model1];
        
        ExpTypeDetail *expModel = [[ExpTypeDetail alloc] init];
        expModel.PayMethodNo = [NSString stringWithFormat:@"%ld",(long)secArrCount+1];
        [weakSelf.FormDatas.arr_FouDetailsDataArray addObject:expModel];
...
//MARK:修改采购申请单 View_ExpTypeTable
-(void)PurchaseExpTypeFormClick:(NSInteger)section WithDataArray:(NSMutableArray *)dataArray  WithBlock:(void(^)(NSString *purStr))purchaseETBlock{
    [self keyClose];
    if (dataArray.count <= 0) {
        return;
    }
    ExpTypeDetail *expModel = [dataArray objectAtIndex:section];
    ChooseCateFreshController *vc = [[ChooseCateFreshController alloc]initWithType:@"PurchaseNumber"];
    vc.ChooseCategoryId=expModel.PurchaseNumber;
    vc.isMultiSelect = NO;
...
//    __weak typeof(self) weakSelf = self;
    vc.ChooseFreshCateBackBlock = ^(NSMutableArray *array, NSString *type) {
        ...
        expModel.PurchaseInfo = [GPUtils getSelectResultWithArray:name WithCompare:@"⊕"];
        expModel.PurchaseNumber = [GPUtils getSelectResultWithArray:Id WithCompare:@","];
        if (purchaseETBlock) {
            purchaseETBlock([NSString stringWithFormat:@"%@",expModel.PurchaseInfo]);
        }
    };
...
//        return self.FormDatas.arr_FouDetailsDataArray.count;
        ...
        if (self.FormDatas.arr_FouDetailsDataArray.count > 0) {
            for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
                if ([expModel.PayMethodNo integerValue] == _View_ExpTypeTable.tag - 100) {
                    [dataArr addObject:expModel];
                }
            }
        }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (tableView == _View_TermTable) {
        ...
        NSInteger count = dataArr.count == 0?0:self.FormDatas.arr_FouDetailsArray.count;
        return count;
    }
    return 0;
}
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == _View_TermTable||tableView == _View_PayModeTable||tableView == _View_ConYearExpTable||tableView == _View_ExpTypeTable) {
        return 42.0f;
    }else{
        BOOL isExpTable = NO;
        //由于不知名的原因（断点实验多次），全局变量的_View_ExpTypeTable会执行多次刷新，会多次执行当前方法（但是不会多次执行- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath这个方法），但是如果self.arr_ExpTypeTable有超过两个表时，刷新时会造成此时刷新的表tableView和当前全局变量指向_View_ExpTypeTable的表不是同一个（此时上面的那个判断就没用了），所以要循环数组arr_ExpTypeTable来判断当前刷新的tableView是否是arr_ExpTypeTable中所拥有的表，否则会造成row的高度变为0.0f；
        for (UITableView *expTable in self.arr_ExpTypeTable) {
            if (tableView == expTable) {
                isExpTable = YES;
            }
           }
        if (isExpTable) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView == _View_TermTable) {
        ...
        cell.CellBackDataBlock = ^(NSIndexPath * _Nonnull index, UITextField * _Nonnull tf, MyProcurementModel * _Nonnull model, id  _Nonnull dModel) {
            ...
            if ([model.fieldName isEqualToString:@"Year"]){
                ...
                for (NSInteger i = thisYearString; i <= thisYearString + 40; i++) {
                    STOnePickModel *model=[[STOnePickModel alloc]init];
                    model.Type = [NSString stringWithFormat:@"%ld",(long)i];
        ...
        return cell;
    }else if(tableView == _View_ExpTypeTable){//费用类别明细

        FormDetailBaseCell *cell=[tableView dequeueReusableCellWithIdentifier:@"ExpTypeBaseCell"];
        if (cell==nil) {
            cell=[[FormDetailBaseCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ExpTypeBaseCell"];
        }
        cell.IndexPath=indexPath;
        NSMutableArray *dataArr = [NSMutableArray array];
        if (self.FormDatas.arr_FouDetailsDataArray.count > 0) {
            for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
                if ([expModel.PayMethodNo integerValue] == _View_ExpTypeTable.tag - 100) {
                    expModel.No = [NSString stringWithFormat:@"%ld",indexPath.section+1];
                    [dataArr addObject:expModel];
                }
            }
        }
//        [cell configContractExpTypeCellWithModel:self.FormDatas.arr_FouDetailsArray[indexPath.row]  withDetailsModel:self.FormDatas.arr_FouDetailsDataArray[indexPath.section] WithCount:self.FormDatas.arr_FouDetailsArray.count WithIndex:indexPath.row];
        [cell configContractExpTypeCellWithModel:self.FormDatas.arr_FouDetailsArray[indexPath.row]  withDetailsModel:dataArr[indexPath.section] WithCount:self.FormDatas.arr_FouDetailsArray.count WithIndex:indexPath.row];
                    __weak typeof(self) weakSelf = self;
        cell.CellBackDataBlock = ^(NSIndexPath * _Nonnull index, UITextField * _Nonnull tf, MyProcurementModel * _Nonnull model, id  _Nonnull dModel) {
            
            ExpTypeDetail *method = (ExpTypeDetail*)dModel;
            if ([model.fieldName isEqualToString:@"LocalCyAmount"]){
                method.LocalCyAmount = tf.text;
            }else if([model.fieldName isEqualToString:@"No"]){
//                method.No = tf.text;
            }else if ([model.fieldName isEqualToString:@"Amount"]){
                method.Amount = tf.text;
                method.LocalCyAmount = [GPUtils transformNsNumber:[GPUtils decimalNumberMultipWithString:method.Amount with:[NSString isEqualToNull:self.FormDatas.str_ExchangeRate]?self.FormDatas.str_ExchangeRate:@"1.0000"]];
                NSIndexPath *indexPath = [NSIndexPath indexPathForRow:3 inSection:index.section];
                [weakSelf.View_ExpTypeTable reloadRowsAtIndexPaths:[NSArray arrayWithObjects:indexPath, nil] withRowAnimation:UITableViewRowAnimationNone];
                ...
                CostCateNewSubModel *model=[[CostCateNewSubModel alloc]init];
                model.expenseCode=method.ExpenseCode;
                pickerArea.CateModel=model;
                [pickerArea UpdatePickUI];
                [pickerArea setContentMode:STPickerContentModeBottom];
                pickerArea.str_flowCode=@"F0006";
                                    
                [pickerArea setChooseCateBlock:^(CostCateNewModel *firstModel, CostCateNewSubModel *secondModel) {
                    if (![secondModel.expenseCode isEqualToString:method.ExpenseCode]) {
                                  
                        method.ExpenseType=[NSString isEqualToNull:secondModel.expenseType]?secondModel.expenseType:@"";
                        method.ExpenseCode=secondModel.expenseCode;
                        method.ExpenseIcon=secondModel.expenseIcon;
                        method.ExpenseCat=secondModel.expenseCat;
                        method.ExpenseCatCode=secondModel.expenseCatCode;
                        tf.text=[GPUtils getSelectResultWithArray:@[secondModel.expenseCat,secondModel.expenseType]];
                        [weakSelf.View_ExpTypeTable reloadData];
                ...
                [weakSelf PurchaseExpTypeFormClick:indexPath.section WithDataArray:dataArr WithBlock:^(NSString *purStr) {
                    tf.text = purStr;
                }];
            }
        };
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (tableView == _View_TermTable) {
        ...
        NSMutableArray *expDataArr = [NSMutableArray array];
        ContractPayMethodDetail *payMModel = [self.FormDatas.arr_SecDetailsDataArray objectAtIndex:_View_ExpTypeTable.tag - 101];
        for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
            if ([payMModel.ContAppOrderNo isEqualToString:expModel.PayMethodNo]) {
                [expDataArr addObject:expModel];
            }
        }
        [self createContractExpTypeHeadViewWithSection:section WithDataArr:expDataArr WithIndexNo:[payMModel.ContAppOrderNo integerValue]];
        return _View_ExpTypeHead;
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section{
    if (tableView == _View_PayModeTable) {
        float footHeight = 0.0;
        if (self.FormDatas.arr_FouDetailsDataArray.count > 0) {
            for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
                if ([expModel.PayMethodNo integerValue] == section+1) {
                    footHeight += (self.FormDatas.arr_FouDetailsArray.count*42+42);
                }
            }
        }
        return footHeight == 0.0 ? 42.0:footHeight;
    }else{
    return 0.01;
    }
}
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (tableView == _View_PayModeTable) {
        [self updatePayModeFootViewWithSection:section];
        return _View_PayModeFoot;
    }else{
    UIView *view = [[UIView alloc] init];
...
- (void)createContractExpTypeHeadViewWithSection:(NSInteger)section WithDataArr:(NSMutableArray *)expDataArr WithIndexNo:(NSInteger)indexNo{
    //indexNo的值与ContAppOrderNo，PayMethodNo的值保持一致
    UIView *View_ExpTypeHead=[[UIView alloc]initWithFrame:CGRectMake(0, 0, Main_Screen_Width, 42)];
    View_ExpTypeHead.tag = 1000+indexNo;
    
    UILabel *titleLabel=[[UILabel alloc]initWithFrame:CGRectMake(0, 0, 180, 18)];
    titleLabel.center=CGPointMake(0+2+8+90, 21);
    titleLabel.font=Font_Important_15_20;
    titleLabel.textAlignment=NSTextAlignmentLeft;
    titleLabel.textColor=Color_Unsel_TitleColor;
    if (expDataArr.count == 0) {
        titleLabel.text=Custing(@"费用类别明细", nil);
    }else{
        titleLabel.text=[NSString stringWithFormat:@"%@(%ld)",Custing(@"费用类别明细", nil),(long)section+1];
    }
    [View_ExpTypeHead addSubview:titleLabel];

    UIButton *addBtn = [GPUtils createButton:CGRectMake(0, 0, 20, 20) action:@selector(addExpTypeTable:) delegate:self normalImage:[UIImage imageNamed:@"addExpType"] highlightedImage:nil];
    addBtn.center = CGPointMake(Main_Screen_Width - 40 - 15- 13.5, 21);
    addBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    addBtn.tag = 100+section;
    [View_ExpTypeHead addSubview:addBtn];
    
    UIButton *deleteBtn=[GPUtils createButton:CGRectMake(0, 0, 20, 20) action:@selector(deleteExpTypeTable:) delegate:self normalImage:[UIImage imageNamed:@"deleteExpType"] highlightedImage:nil];
    deleteBtn.center=CGPointMake(Main_Screen_Width-25, 21);
    deleteBtn.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
    deleteBtn.tag=100+section;
    [View_ExpTypeHead addSubview:deleteBtn];
    _View_ExpTypeHead = View_ExpTypeHead;
    UIView *view_Line = [[UIView alloc]initWithFrame:CGRectZero];
    view_Line.backgroundColor = Color_White_Same_20;
    view_Line.frame = CGRectMake(X(titleLabel), 0, Main_Screen_Width - X(titleLabel), 1);
    [View_ExpTypeHead addSubview:view_Line];
//    _View_ExpTypeHead.backgroundColor=Color_White_Same_20;
}
...
//MARK:增加费用类别明细
- (void)addExpTypeTable:(UIButton *)btn{
    [self keyClose];
    
    UIView *view = btn.superview;
    ExpTypeDetail *expModel = [[ExpTypeDetail alloc] init];
    expModel.PayMethodNo = [NSString stringWithFormat:@"%ld",(long)(view.tag-1000)];
    [self.FormDatas.arr_FouDetailsDataArray addObject:expModel];
    [self updatePayModeTableView];

}
//MARK:删除费用类别明细
- (void)deleteExpTypeTable:(UIButton *)btn{
    [self keyClose];
    UIView *view = btn.superview;
    NSMutableArray *expArr = [NSMutableArray array];
    for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
           if ([expModel.PayMethodNo integerValue] == view.tag - 1000) {
               [expArr addObject:expModel];
           }
       }
    __weak typeof(self) weakSelf = self;
    if (expArr.count > 0) {
        NSString *message = [NSString stringWithFormat:@"%@%ld?",Custing(@"你确定要删除费用明细", nil),(long)(btn.tag - 100+1)];
        ...
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:Custing(@"确定", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            if (expArr.count > btn.tag - 100) {
                ExpTypeDetail *expModel = [expArr objectAtIndex:btn.tag - 100];
                [weakSelf.FormDatas.arr_FouDetailsDataArray removeObject:expModel];
            }
            [weakSelf updatePayModeTableView];
        }];
        ...
        UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"" message:Custing(@"已无可删费用类别明细", nil) preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *okAction = [UIAlertAction actionWithTitle:Custing(@"确定", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        }];
        [alertC addAction:okAction];
        [self presentViewController:alertC animated:YES completion:nil];
...
-(void)deleteTable:(UIButton *)btn{
    ...
    NSString *title;
    if (btn.tag >= 4200) {
        title=[NSString stringWithFormat:@"%@%ld?",Custing(@"你确定要删除费用明细", nil),(long)(btn.tag - 4200 + 1)];
    }else if (btn.tag >= 3200){
    ...
    [UIAlertView bk_showAlertViewWithTitle:@"" message:title cancelButtonTitle:Custing(@"取消",nil) otherButtonTitles:@[Custing(@"删除",nil)] handler:^(UIAlertView *alertView, NSInteger buttonIndex) {
        if (buttonIndex == 1) {
            if (btn.tag >= 4200) {
//                [weakSelf.FormDatas.arr_FouDetailsDataArray removeObjectAtIndex:btn.tag-4200];
//                [weakSelf updateExpTypeTableView];
            }else if (btn.tag >= 3200) {
                ...
                NSMutableArray *deleteExpArr = [NSMutableArray array];
                for (ExpTypeDetail *expModel in weakSelf.FormDatas.arr_FouDetailsDataArray) {
                    if ([expModel.PayMethodNo integerValue] == btn.tag - 2200+1) {
                        [deleteExpArr addObject:expModel];
                    }
                }
                if (deleteExpArr.count > 0) {
                    for (ExpTypeDetail *expModel in deleteExpArr) {
                        [weakSelf.FormDatas.arr_FouDetailsDataArray removeObject:expModel];
                    }
                }
                for (ExpTypeDetail *expModel in weakSelf.FormDatas.arr_FouDetailsDataArray) {
                    if ([expModel.PayMethodNo integerValue]> btn.tag - 2200+1) {
                        expModel.PayMethodNo = [NSString stringWithFormat:@"%ld",(long)([expModel.PayMethodNo integerValue]-1)];
                    }
                }
                for (ContractPayMethodDetail *payModel in self.FormDatas.arr_SecDetailsDataArray) {
                    if ([payModel.ContAppOrderNo integerValue] > btn.tag - 2200+1) {
                        payModel.ContAppOrderNo = [NSString stringWithFormat:@"%ld",(long)([payModel.ContAppOrderNo integerValue]-1)];
                    }
                }
                [weakSelf updatePayModeTableView];
            }else if (btn.tag >= 1200){
...
//存储费用类别明细表数组
- (NSMutableArray *)arr_ExpTypeTable{
    if (!_arr_ExpTypeTable) {
        _arr_ExpTypeTable = [NSMutableArray array];
    }
    return _arr_ExpTypeTable;
}

//MARK:更新付款方式表足-费用类别明细表
- (void)updatePayModeFootViewWithSection:(NSInteger)section{
    float footHeight = 0.0;

    if (self.FormDatas.arr_FouDetailsDataArray.count > 0) {
        for (ExpTypeDetail *exModel in self.FormDatas.arr_FouDetailsDataArray) {
            if ([exModel.PayMethodNo integerValue] == section+1) {
                footHeight += (self.FormDatas.arr_FouDetailsArray.count*42+42);
            }
        }
    }
    footHeight = footHeight == 0.0 ? 42.0:footHeight;
    _View_PayModeFoot = [[UIView alloc] initWithFrame:CGRectMake(0, 0, Main_Screen_Width, footHeight)];
    UITableView *View_ExpTypeTable;
    if (self.arr_ExpTypeTable.count > section) {
        View_ExpTypeTable = [self.arr_ExpTypeTable objectAtIndex:section];
    }else{
        View_ExpTypeTable =[[UITableView alloc]initWithFrame:CGRectZero style:UITableViewStyleGrouped];
        View_ExpTypeTable.backgroundColor=Color_WhiteWeak_Same_20;
        View_ExpTypeTable.delegate=self;
        View_ExpTypeTable.dataSource=self;
        View_ExpTypeTable.scrollEnabled=NO;
        View_ExpTypeTable.separatorStyle=UITableViewCellSeparatorStyleNone;
        View_ExpTypeTable.tag = 101+section;
        if (self.arr_ExpTypeTable.count <= section) {
            [self.arr_ExpTypeTable addObject:View_ExpTypeTable];
        }
    }
    View_ExpTypeTable.frame = CGRectMake(0, 0, Main_Screen_Width, footHeight);
    [_View_PayModeFoot addSubview:View_ExpTypeTable];
    _View_ExpTypeTable = View_ExpTypeTable;
...
- (void)testAmount{
    //判断付款明细的付款金额和底下费用类别明细的本位币之和相等
    BOOL allowSubmit = YES;
    for (ContractPayMethodDetail *payModel in self.FormDatas.arr_SecDetailsDataArray) {
        float payAmount = [payModel.Amount floatValue];
        float expAmount = 0.0;
        for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
            if ([expModel.PayMethodNo integerValue] == [payModel.ContAppOrderNo integerValue]) {
                expAmount = expAmount + [expModel.Amount floatValue];
            }
        }
        if (expAmount != payAmount) {
            allowSubmit = NO;
            NSString *message = [NSString stringWithFormat:@"%@(%ld):%@",Custing(@"付款方式", nil),[payModel.ContAppOrderNo integerValue]+1,Custing(@"付款方式金额与其下费用类别明细的本位币金额总和不相等", nil)];
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"" message:message preferredStyle:UIAlertControllerStyleAlert];
            UIAlertAction *okAction = [UIAlertAction actionWithTitle:Custing(@"确认", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                self.dockView.userInteractionEnabled=YES;
            }];
            [alertC addAction:okAction];
            [self presentViewController:alertC animated:YES completion:nil];
...
//    [GPUtils transformNsNumber:[GPUtils decimalNumberMultipWithString:amount with:([NSString isEqualToNull:weakSelf.FormDatas.str_ExchangeRate]?weakSelf.FormDatas.str_ExchangeRate:@"1.0000")]];
    if (allowSubmit) {
        for (ContractPayMethodDetail *payModel in self.FormDatas.arr_SecDetailsDataArray) {
            if (self.FormDatas.str_ExchangeRate) {
                payModel.LocalCyAmount = [GPUtils transformNsNumber:[GPUtils decimalNumberMultipWithString:payModel.Amount with:[NSString isEqualToNull:self.FormDatas.str_ExchangeRate]?self.FormDatas.str_ExchangeRate:@"1.0000"]];
            }
        }
        for (ExpTypeDetail *expModel in self.FormDatas.arr_FouDetailsDataArray) {
            if (self.FormDatas.str_ExchangeRate) {
                expModel.LocalCyAmount = [GPUtils transformNsNumber:[GPUtils decimalNumberMultipWithString:expModel.Amount with:[NSString isEqualToNull:self.FormDatas.str_ExchangeRate]?self.FormDatas.str_ExchangeRate:@"1.0000"]];
            }
        }
        [self.FormDatas contectData];
        [self dealWithImagesArray];
