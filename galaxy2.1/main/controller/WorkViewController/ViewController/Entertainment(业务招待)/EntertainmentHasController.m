...
@interface EntertainmentHasController ()

@end

@implementation EntertainmentHasController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas=[[EntertainmentFormData alloc]initWithStatus:2];
    }
    return self;
}
...
-(void)createMainView{
    ...
    [_View_Object makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_View_StartTime makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_Object.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_EndTime makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_StartTime.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    [_View_Visitor makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_EndTime.bottom);
        make.left.right.equalTo(self.contentView);
    }];
    ...
    _View_VistorDetailsTable=[[UITableView alloc]initWithFrame:CGRectZero style:UITableViewStylePlain];
    _View_VistorDetailsTable.backgroundColor=Color_WhiteWeak_Same_20;
    _View_VistorDetailsTable.delegate=self;
    _View_VistorDetailsTable.dataSource=self;
    _View_VistorDetailsTable.scrollEnabled=NO;
    _View_VistorDetailsTable.separatorStyle=UITableViewCellSeparatorStyleNone;
    [self.contentView addSubview:_View_VistorDetailsTable];
    [_View_VistorDetailsTable makeConstraints:^(MASConstraintMaker *make) {
    ...
    [_View_SecDetailsTable makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.View_VistorDetailsTable.bottom);
        make.left.right.equalTo(self.contentView);
    }];
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        ...
        }else if ([model.fieldName isEqualToString:@"ReceptionObj"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateObjectViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"VisitDate"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateStartTimeViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"LeaveDate"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
            [self updateEndTimeViewWithModel:model];
        }else if ([model.fieldName isEqualToString:@"Visitor"]&&[[model.isShow stringValue]isEqualToString:@"1"]) {
            _int_line1=1;
        ...
        }else if ([model.fieldName isEqualToString:@"IsUseCar"]&&[[model.isShow stringValue]isEqualToString:@"1"]){
            _int_line1=1;
            [self updateIsUseCarView:model];
        }else if ([model.fieldName isEqualToString:@"RentCarFee"]&&[[model.isShow stringValue]isEqualToString:@"1"]){
            _int_line1=1;
        ...
        }else if ([model.fieldName isEqualToString:@"HotelFee"]&&[[model.isShow stringValue]isEqualToString:@"1"]){
            _int_line1=1;
    ...
    if (self.FormDatas.bool_ThirDetailsShow&&self.FormDatas.arr_ThirDetailsDataArray.count!=0) {
        [self updateVistorDetailsTable];
        [_View_VistorDetailsTable reloadData];
    }
...
-(void)updateObjectViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_Object addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_Object updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateStartTimeViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_StartTime addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_StartTime updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateEndTimeViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_EndTime addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_EndTime updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateIsUseCarView:(MyProcurementModel *)model{
    model.fieldValue=[[NSString stringWithFormat:@"%@",model.fieldValue]isEqualToString:@"1"]?Custing(@"是", nil):Custing(@"否", nil);
    [_View_IsUseCar addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_IsUseCar updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateRentCarFeeView:(MyProcurementModel *)model{
    [_View_RentCarFee addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 IsAmount:1 block:^(NSInteger height) {
        [self.View_RentCarFee updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateHotelFeeView:(MyProcurementModel *)model{
    [_View_HotelFee addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 IsAmount:1 block:^(NSInteger height) {
        [self.View_HotelFee updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateDetailsTableView{
    if (self.FormDatas.bool_isOpenDetail) {
        NSInteger height=10;
        for (EntertainmentDeatil *model in self.FormDatas.arr_DetailsDataArray) {
            height=height+[ProcureDetailsCell EntertainmentCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
        }
        [_View_DetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }else{
        EntertainmentDeatil *model=self.FormDatas.arr_DetailsDataArray[0];
        NSInteger height=10+[ProcureDetailsCell EntertainmentCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
        [_View_DetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }
}
-(void)updateVistorDetailsTable{
    if (self.FormDatas.bool_ThirisOpenDetail) {
        NSInteger height=10;
        for (EntertainmentVisitorDeatil *model in self.FormDatas.arr_ThirDetailsDataArray) {
            height=height+[ProcureDetailsCell EntertainmentVisitorCellHeightWithArray:self.FormDatas.arr_ThirDetailsArray WithModel:model];
        }
        [_View_VistorDetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }else{
        EntertainmentVisitorDeatil *model=self.FormDatas.arr_ThirDetailsDataArray[0];
        NSInteger height=10+[ProcureDetailsCell EntertainmentVisitorCellHeightWithArray:self.FormDatas.arr_ThirDetailsArray WithModel:model];
        [_View_VistorDetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }
...
-(void)updateSecDetailsTableView{
    if (self.FormDatas.bool_SecisOpenDetail) {
        NSInteger height=10;
        for (EntertainmentSchDeatil *model in self.FormDatas.arr_SecDetailsDataArray) {
            height=height+[ProcureDetailsCell EntertainmentCellHeightWithArray:self.FormDatas.arr_SecDetailsArray WithModel:model];
        }
        [_View_SecDetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }else{
        EntertainmentSchDeatil *model=self.FormDatas.arr_SecDetailsDataArray[0];
        NSInteger height=10+[ProcureDetailsCell EntertainmentCellHeightWithArray:self.FormDatas.arr_SecDetailsArray WithModel:model];
        [_View_SecDetailsTable updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(@(height));
        }];
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    }else if (tableView==_View_VistorDetailsTable){
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView==_View_DetailsTable) {
        EntertainmentDeatil *model=self.FormDatas.arr_DetailsDataArray[indexPath.row];
        return [ProcureDetailsCell EntertainmentCellHeightWithArray:self.FormDatas.arr_DetailsArray WithModel:model];
    }else if (tableView==_View_SecDetailsTable){
        EntertainmentSchDeatil *model=self.FormDatas.arr_SecDetailsDataArray[indexPath.row];
        return [ProcureDetailsCell EntertainmentCellHeightWithArray:self.FormDatas.arr_SecDetailsArray WithModel:model];
    }else if (tableView==_View_VistorDetailsTable){
        EntertainmentVisitorDeatil *model=self.FormDatas.arr_ThirDetailsDataArray[indexPath.row];
        return [ProcureDetailsCell EntertainmentVisitorCellHeightWithArray:self.FormDatas.arr_ThirDetailsArray WithModel:model];
    }else{
        return 0;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView==_View_DetailsTable) {
        ...
        [cell configEntertainmentCellWithArray:self.FormDatas.arr_DetailsArray withDetailsModel:self.FormDatas.arr_DetailsDataArray[indexPath.row] withindex:indexPath.row withCount:self.FormDatas.arr_DetailsDataArray.count] ;
        if (cell.LookMore) {
        ...
        [cell configEntertainmentCellWithArray:self.FormDatas.arr_SecDetailsArray withDetailsModel:self.FormDatas.arr_SecDetailsDataArray[indexPath.row] withindex:indexPath.row withCount:self.FormDatas.arr_SecDetailsDataArray.count] ;
        if (cell.LookMore) {
        ...
        [cell configEntertainmentVisitorCellWithArray:self.FormDatas.arr_ThirDetailsArray withDetailsModel:self.FormDatas.arr_ThirDetailsDataArray[indexPath.row] withindex:indexPath.row withCount:self.FormDatas.arr_ThirDetailsDataArray.count] ;
        if (cell.LookMore) {
