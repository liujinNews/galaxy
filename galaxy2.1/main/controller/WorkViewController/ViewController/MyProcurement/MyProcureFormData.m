...
- (void)contectPurchaseArr:(NSMutableArray *)purArr withTableName:(NSString *)tableName{
    ...
    if (purArr.count!=0) {
        if ([tableName isEqualToString:@"Sa_PurchaseAmountDetail"]) {
            fieldNames = [NSMutableArray arrayWithObjects:@"No",@"Description",@"CurrencyCode",@"Currency",@"ExchangeRate",@"Amount",@"LocalCyAmount",@"ExpenseCode",@"ExpenseType",@"ExpenseIcon",@"ExpenseCatCode",@"ExpenseCat",@"FeeAppNumber",@"FeeAppInfo",@"Remarks",@"Attachments", nil];
        }else if([tableName isEqualToString:@"Sa_PurchaseBusinessDetail"]){
