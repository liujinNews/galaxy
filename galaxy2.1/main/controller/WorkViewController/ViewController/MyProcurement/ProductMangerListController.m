...
//    [[[[ self.searchbar.subviews objectAtIndex : 0 ] subviews ] objectAtIndex : 0 ] removeFromSuperview ];
    //    判断iOS的版本是否大于13.0
      if (@available(iOS 13.0, *)) {
          ...
          self.searchbar.searchTextField.placeholder = [NSString stringWithFormat:@"%@%@",Custing(@"搜索", nil),Custing(@"产品", nil)];
...
-(void)requestPurchaseItem{
    ...
    NSDictionary * dic =@{@"Type":self.isUser > 0 ? @"1":@"2" ,@"Name":_searchAim,@"PageIndex":[NSString stringWithFormat:@"%ld",(long)self.currPage],@"PageSize":@20,@"OrderBy":@"",@"IsAsc":@""};
