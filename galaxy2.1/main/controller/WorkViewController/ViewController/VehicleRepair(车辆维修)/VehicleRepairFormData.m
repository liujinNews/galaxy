...
@implementation VehicleRepairFormData
...
-(void)initializeNewData{
    ...
    self.str_flowCode=@"F0024";
    self.arr_isShowmsArray=[[NSMutableArray alloc]initWithArray:@[@"RequestorDeptId",@"BranchId",@"CostCenterId",@"RequestorBusDeptId",@"AreaId",@"LocationId",@"Reason",@"CarNo",@"CarSize",@"LoadCapacity",@"Mileage",@"ExpenseCode",@"ExpenseDesc",@"ProjId",@"ClientId",@"SupplierId",@"EstimatedAmount",@"CurrencyCode",@"ExchangeRate",@"DetailList",@"Reserved1",@"Reserved2",@"Reserved3",@"Reserved4",@"Reserved5",@"Reserved6",@"Reserved7",@"Reserved8",@"Reserved9",@"Reserved10",@"Remark",@"Attachments",@"FirstHandlerUserName",@"CcUsersName"]];
...
-(NSString *)OpenFormUrl{
    if (self.int_formStatus==1) {
        return [NSString stringWithFormat:@"%@",NEWVEHICLEREPAIR];
    }else{
        return [NSString stringWithFormat:@"%@",HASVEHICLEREPAIR];
    }
}
...
-(void)DealWithFormBaseData{
    ...
    if (![result isKindOfClass:[NSNull class]]) {
        ...
        self.bool_DetailsShow=[[NSString stringWithFormat:@"%@",result[@"vehicleSvcDetail"]] isEqualToString:@"1"]?YES:NO;
        ...
        if (![formDataDict isKindOfClass:[NSNull class]]) {
            NSArray *sa_ChopAppDetailArray=formDataDict[@"sa_VehicleSvcDetail"];
...
-(void)inModelContent{
    self.SubmitData=[[VehicleRepairData alloc]init];
...
-(NSString *)testModel{
    VehicleRepairData *model=self.SubmitData;
    NSString *returnTips;
    NSMutableDictionary *modeldic = [VehicleRepairData initDicByModel:model];
        ...
        if ([key isEqualToString:@"DetailList"]) {
            for (NSString *str in self.arr_isShowmDetailArray) {
                ...
                if ([i isEqualToString:@"1"]) {
                    for (VehicleRepairDeatil *model in self.arr_DetailsDataArray) {
                        NSString *detailStr;
...
-(NSString *)showerror:(NSString*)info{
    ...
    if ([info isEqualToString:@"RequestorDeptId"]) {
        ...
        showinfo = Custing(@"请选择车牌号码", nil);
    }else if([info isEqualToString:@"CarSize"]) {
        showinfo = Custing(@"请输入型号", nil);
    }else if([info isEqualToString:@"LoadCapacity"]) {
        showinfo = Custing(@"请输入载重量", nil);
    }else if([info isEqualToString:@"Mileage"]) {
        showinfo = Custing(@"请输入现行里程", nil);
...
-(void)contectData{
    ...
    NSMutableDictionary *Sa_VehicleSvcApp = [[NSMutableDictionary alloc]init];
    NSMutableArray *fieldNamesArr = [[NSMutableArray alloc]init];
    NSMutableArray *fieldValuesArr = [[NSMutableArray alloc]init];
    NSMutableDictionary *modelDic=[VehicleRepairData initDicByModel:self.SubmitData];
    for(id key in modelDic)
    {
        [fieldNamesArr addObject:key];
        [fieldValuesArr addObject:[modelDic objectForKey:key]];
    }
    [Sa_VehicleSvcApp setObject:@"Sa_VehicleSvcApp" forKey:@"tableName"];
    [Sa_VehicleSvcApp setObject:fieldNamesArr forKey:@"fieldNames"];
    [Sa_VehicleSvcApp setObject:fieldValuesArr forKey:@"fieldValues"];
    [mainArray addObject:Sa_VehicleSvcApp];
    
    
    NSMutableArray *detailedArray=[[NSMutableArray alloc]init];
    NSMutableDictionary *Sa_VehicleSvcDetail = [[NSMutableDictionary alloc]init];
    NSArray *fieldNames = [NSArray array];
    NSMutableArray *Values=[NSMutableArray array];
    if (self.arr_DetailsDataArray.count!=0) {
        NSMutableDictionary *modelsDic=[VehicleRepairDeatil initDicByModel:self.arr_DetailsDataArray[0]];
        ...
        [Sa_VehicleSvcDetail setObject:@"Sa_VehicleSvcDetail" forKey:@"tableName"];
        [Sa_VehicleSvcDetail setObject:fieldNames forKey:@"fieldNames"];
        [Sa_VehicleSvcDetail setObject:Values forKey:@"fieldBigValues"];
    }else{
        [Sa_VehicleSvcDetail setObject:@"Sa_VehicleSvcDetail" forKey:@"tableName"];
        [Sa_VehicleSvcDetail setObject:fieldNames forKey:@"fieldNames"];
        [Sa_VehicleSvcDetail setObject:Values forKey:@"fieldBigValues"];
    ...
    [detailedArray addObject:Sa_VehicleSvcDetail];
...
-(NSString *)getTableName{
    return [NSString stringWithFormat:@"%@",@"Sa_VehicleSvcApp"];
}
