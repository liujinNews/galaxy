...
@interface WareHouseEntryHasController ()

@end

@implementation WareHouseEntryHasController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas = [[WareHouseEntryFormData alloc]initWithStatus:2];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)updateStoreDateViewWithModel:(MyProcurementModel *)model{
    __weak typeof(self) weakSelf = self;
    [_View_StoreDate addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [weakSelf.View_StoreDate updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateReceivedDateViewWithModel:(MyProcurementModel *)model{
    [_View_ReceivedDate addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_ReceivedDate updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateStoreTypeViewWithModel:(MyProcurementModel *)model{
    [_View_StoreType addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_StoreType updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateInvStorageViewWithModel:(MyProcurementModel *)model{
    [_View_InvStorage addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_InvStorage updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
...
-(void)updateInvStorageAddrViewWithModel:(MyProcurementModel *)model{
    [_View_InvStorageAddr addSubview:[XBHepler creation_Lable:[UILabel new] model:model Y:0 block:^(NSInteger height) {
        [self.View_InvStorageAddr updateConstraints:^(MASConstraintMaker *make) {
            make.height.equalTo(height);
        }];
    }]];
}
