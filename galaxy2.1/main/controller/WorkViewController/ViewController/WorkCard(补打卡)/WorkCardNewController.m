...
@interface WorkCardNewController ()

@end

@implementation WorkCardNewController

-(instancetype)init{
    self = [super init];
    if (self) {
        self.FormDatas = [[WorkCardFormData alloc]initWithStatus:1];
    }
    return self;
}
- (void)viewDidLoad {
...
-(void)updateMainView{
    ...
    for (MyProcurementModel *model in self.FormDatas.arr_FormMainArray) {
        if ([model.fieldName isEqualToString:@"TimeCard"]) {
        ...
        }else if ([model.fieldName isEqualToString:@"WitnessId"]) {
            if ([[model.isShow stringValue]isEqualToString:@"1"]) {
                [self.FormDatas.dict_isRequiredmsdic setValue:model.isRequired forKey:model.fieldName];
...
-(void)updateTimeCardViewWithModel:(MyProcurementModel *)model{
    _txf_TimeCard = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_TimeCard WithContent:_txf_TimeCard WithFormType:formViewSelectDateTime WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:nil];
    [_View_TimeCard addSubview:view];
}
//MARK:更新补卡理由
...
-(void)updateWitnessViewWithModel:(MyProcurementModel *)model{
    _txf_Witness = [[UITextField alloc]init];
    SubmitFormView *view = [[SubmitFormView alloc]initBaseView:_View_Witness WithContent:_txf_Witness WithFormType:formViewSelect WithSegmentType:lineViewNoneLine Withmodel:model WithInfodict:@{@"value1":self.FormDatas.str_WitnessName}];
    __weak typeof(self) weakSelf = self;
    [view setFormClickedBlock:^(MyProcurementModel *model){
        [weakSelf WitnessClick];
    }];
    [_View_Witness addSubview:view];
}
//MARK:更新自定义字段
...
-(void)WitnessClick{
    ...
    NSArray *idarr = [self.FormDatas.str_WitnessId componentsSeparatedByString:@","];
    ...
    [contactVC setBlock:^(NSMutableArray *array) {
        ...
        weakSelf.FormDatas.str_WitnessId = [GPUtils getSelectResultWithArray:idArr WithCompare:@","];
        weakSelf.FormDatas.str_WitnessName = [GPUtils getSelectResultWithArray:nameArr WithCompare:@","];
        weakSelf.txf_Witness.text = weakSelf.FormDatas.str_WitnessName;
    }];
...
-(void)configModelOtherData{
    ...
    self.FormDatas.SubmitData.TimeCard = _txf_TimeCard.text;
