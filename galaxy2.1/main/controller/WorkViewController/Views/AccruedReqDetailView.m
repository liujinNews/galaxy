...
@interface AccruedReqDetailView ()<UITableViewDelegate,UITableViewDataSource>
 ...
 *  子表cell
 */
@property (nonatomic,strong)AccruedReqDetailCell *PaymentExpCell;
/**
 *  增加明细按钮视图
 */
...
@implementation AccruedReqDetailView
- (instancetype)init{
    self = [super init];
    if (self) {
...
-(void)updateTableView{
    if (_isOpenShare == NO) {
        ...
        NSInteger height = 0;
        for (AccruedReqDetail *model in self.formData) {
            height += [AccruedReqDetailCell cellHeightWithModel:model withEditType:self.editType];
        }
...
-(void)AddDetailsClick:(id)obj{
    if (self.AccruedReqBackClickedBlock) {
        self.AccruedReqBackClickedBlock(1, 0, [AccruedReqDetail new]);
    }
}
-(void)leadDetail:(id)sender{
    if (self.AccruedLeadClickedBlock) {
        self.AccruedLeadClickedBlock();
    }
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    AccruedReqDetail *model = self.formData[indexPath.row];
    return [AccruedReqDetailCell cellHeightWithModel:model withEditType:self.editType];
}
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    AccruedReqDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:@"AccruedReqDetailCell"];
    if (cell == nil) {
        cell = [[AccruedReqDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"AccruedReqDetailCell"];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.AccruedReqBackClickedBlock) {
        AccruedReqDetail *model = self.formData[indexPath.row];
        __weak typeof(self) weakSelf = self;
        self.AccruedReqBackClickedBlock(weakSelf.editType == 3 ? 4:2, indexPath.row, model);
    }
}
...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewRowAction *deleteRowAction = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:Custing(@"删除", nil)  handler:^(UITableViewRowAction *action, NSIndexPath *indexPath){
        ...
        self.AccruedReqBackClickedBlock(3, indexPath.row, model);
        [self updateTableView];
    }];
