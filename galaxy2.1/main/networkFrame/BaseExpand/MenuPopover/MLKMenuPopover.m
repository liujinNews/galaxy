...
#define MENU_POINTER_RECT       CGRectMake(frame.origin.x+93, frame.origin.y, 23, 11)

#define CONTAINER_BG_COLOR      RGBA(0, 0, 0, 0.6f)
...
@property (nonatomic,assign) CGRect tableview_rect;
...
//        [self.containerButton addSubview:menuPointerView];
        ...
        _menuItemsTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
...
//        _menuItemsTableView.layer.cornerRadius = 8.f;
...
//        menuItemsTableView.backgroundView = bgView;
        _menuItemsTableView.backgroundColor = Color_form_TextFieldBackgroundColor;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil)
    {
        ...
        [cell.textLabel setFont:Font_cellTitle_14];
...
- (void)showInView:(UIView *)view
{
    ...
    self.containerButton.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.7];
    self.containerButton.frame = CGRectMake(0, 44, Main_Screen_Width, Main_Screen_Height);
    [view addSubview:self.containerButton];
    
    CGFloat height = _menuItemsTableView.frame.origin.y;
//    _menuItemsTableView.hidden = NO;
    _menuItemsTableView.frame = CGRectMake(X(_menuItemsTableView), Y(_menuItemsTableView), WIDTH(_menuItemsTableView), NavigationbarHeight-height);
    
    [UIView animateWithDuration:ANIMATION_DURATION
                     animations:^{
                         self.containerButton.alpha = ONE;
                         _menuItemsTableView.frame =CGRectMake(0, 0, WIDTH(_menuItemsTableView), _tableview_rect.size.height);
                     }
...
- (void)hide
{
    [UIView animateWithDuration:ANIMATION_DURATION
                     animations:^{
                         self.containerButton.alpha = ZERO;
                         _menuItemsTableView.frame = CGRectMake(X(_menuItemsTableView), Y(_menuItemsTableView), WIDTH(_menuItemsTableView), NavigationbarHeight-_tableview_rect.size.height);
                     }
                     completion:^(BOOL finished) {
