...
-(id)initWithFrame:(CGRect)frame items:(NSArray *)items{
    ...
    if(self){
        ...
        _descriptionTextColor = Color_form_TextFieldBackgroundColor;
        _descriptionTextFont  = [UIFont fontWithName:@"Avenir-Medium" size:18.0];
        _descriptionTextShadowColor  = [Color_Unsel_TitleColor colorWithAlphaComponent:0.4];
...
- (void)loadDefault{
    ...
    [_items enumerateObjectsUsingBlock:^(PNPieChartDataItem *item, NSUInteger idx, BOOL *stop) {
        if (total == 0){
            [endPercentages addObject:@(1.0 / weakSelf.items.count * (idx + 1))];
        }else{
...
- (UILabel *)descriptionLabelForItemAtIndex:(NSUInteger)index{
    ...
    if (self.showAbsoluteValues) {
        ...
        titleValue = [NSString stringWithFormat:@"%.2f%%",[self ratioForItemAtIndex:index] * 100];
    }
...
- (void)maskChart{
    ...
    CAShapeLayer *maskLayer = [self newCircleLayerWithRadius:radius
                                                 borderWidth:borderWidth
                                                   fillColor:[UIColor clearColor]
                                                 borderColor:Color_Unsel_TitleColor
                                             startPercentage:0
                                               endPercentage:1];
...
- (UIView*) getLegendWithMaxWidth:(CGFloat)mWidth{
    ...
    for (PNPieChartDataItem *pdata in self.items) {
        ...
        label.textColor = self.legendFontColor ? self.legendFontColor : Color_Unsel_TitleColor;
