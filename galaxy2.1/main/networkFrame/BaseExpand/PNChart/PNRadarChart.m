...
@property (nonatomic) CGFloat pncenterX;
@property (nonatomic) CGFloat pncenterY;
...
//            NSLog( @"At least three items!");
        ...
        _fontColor = Color_Unsel_TitleColor;
        ...
        //Private iVar
        _pncenterX = frame.size.width/2;
        _pncenterY = frame.size.height/2;
...
- (void)calculateChartPoints {
    ...
    if (_labelStyle==PNRadarChartLabelStyleCircle) {
        margin = MIN(_pncenterX , _pncenterY)*3/10;
    ...
    CGFloat maxLength = ceil(MIN(_pncenterX, _pncenterY) - margin);
...
//        NSLog(@"Circle number is higher than max");
    ...
    for (id value in values) {
        ...
        CGFloat x = _pncenterX +length*cos(angle);
        CGFloat y = _pncenterY +length*sin(angle);
...
- (void)drawRect:(CGRect)rect {
    ...
    for (NSValue *pointValue in largestPointArray){
        ...
        CGContextRef graphContext = UIGraphicsGetCurrentContext();
        CGContextBeginPath(graphContext);
        CGContextMoveToPoint(graphContext, _pncenterX, _pncenterY);
        CGPoint point = [pointValue CGPointValue];
        CGContextAddLineToPoint(graphContext, point.x, point.y);
        CGContextSetStrokeColorWithColor(graphContext, _webColor.CGColor);
        CGContextStrokePath(graphContext);
...
- (void)strokeChart {
    ...
    animateMove.fromValue = [NSValue valueWithCGPoint:CGPointMake(_pncenterX, _pncenterY)];
...
- (void)drawLabelWithMaxLength:(CGFloat)maxLength labelArray:(NSArray *)labelArray angleArray:(NSArray *)angleArray {
    ...
    for (NSString *labelString in labelArray) {
        ...
        CGFloat x = _pncenterX + labelLength *cos(angle);
        CGFloat y = _pncenterY + labelLength *sin(angle);
