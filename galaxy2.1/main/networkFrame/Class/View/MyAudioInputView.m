...
@property (nonatomic, strong)YXSpritesLoadingView *showviwe;
...
//        _pttTipsLable.textAlignment = UITextAlignmentCenter;
...
//        _timeLable.textAlignment = UITextAlignmentCenter;
...
//        _operationTipsLabel.textAlignment = UITextAlignmentCenter;
...
//        _recordBtn.touchDelegate = self;
        ...
        //重写
        _recordBtn = [[MyPttRecordBtn alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height)];
        [_recordBtn.titleLabel setFont:Font_Important_15_20];
        
        [_recordBtn setTitleColor:Color_GrayDark_Same_20 forState:UIControlStateNormal];
        [_recordBtn setTitleColor:Color_GrayDark_Same_20 forState:UIControlStateHighlighted];
        
        [_recordBtn setTitle:@"按住说话" forState:UIControlStateNormal];
        [_recordBtn setTitle:@"松开发送" forState:UIControlStateHighlighted];
        [_recordBtn setBackgroundImage:[UIImage imageNamed:@"aio_voice_button_nor"] forState:UIControlStateNormal];
        [_recordBtn setBackgroundImage:[UIImage imageNamed:@"aio_voice_button_press"] forState:UIControlStateHighlighted];
        [_recordBtn addTarget:self action:@selector(onCancel) forControlEvents:UIControlEventTouchUpOutside];
        
        _recordBtn.touchDelegate = self;
        [self addSubview:_recordBtn];
        
        _showviwe = [[YXSpritesLoadingView alloc]init];
...
- (void)touchBegin:(NSSet*)touches
{
    ...
    [YXSpritesLoadingView showWithText:@"手指上滑，取消发送" andShimmering:NO andBlurEffect:NO];
...
- (void)touchMove:(NSSet *)touches
{
    ...
    if (!_operationViewContainer.hidden) {
        ...
        NSLog(@"XXX%f YYY%f",point.x,point.y);
        
        if (point.y<Main_Screen_Height-46) {
//            [YXSpritesLoadingView dismiss];
            [YXSpritesLoadingView showWithText:@"松开手指，取消发送" andShimmering:NO andBlurEffect:NO];
        }
        else
        {
            [YXSpritesLoadingView showWithText:@"手指上滑，取消发送" andShimmering:NO andBlurEffect:NO];
...
- (void)touchEnd:(NSSet *)touches
{
    ...
    CGPoint point = [touch locationInView:_operationViewContainer];
    if (point.y<Main_Screen_Height-46) {
        NSLog(@"%f   ```` 当前屏幕的 值 %f，无效区域 %f",point.y,Main_Screen_Height,Main_Screen_Height-46);
        [YXSpritesLoadingView dismiss];
//        [self touchCancel:touches];
        return;
    }
    else
    {
