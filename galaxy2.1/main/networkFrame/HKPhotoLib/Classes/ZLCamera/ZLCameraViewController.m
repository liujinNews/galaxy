...
- (void) setup{
    ...
    topView.backgroundColor = Color_Unsel_TitleColor;
    ...
    contentView.backgroundColor = Color_Unsel_TitleColor;
    ...
    if (self.int_place == 1) {
        ...
        cancalBtn.frame = CGRectMake(12, 0, 60, 40);
        cancalBtn.titleLabel.font = Font_Same_14_20;
        [cancalBtn setTitle:Custing(@"取消", nil) forState:UIControlStateNormal];
        ...
        flashBtn.frame = CGRectMake(72, 0, 50, 40);
        closeBtn.frame = CGRectMake(122, 0, 50, 40);

        //打开相册
        UIButton *openImg = [UIButton buttonWithType:UIButtonTypeCustom];
        openImg.frame = CGRectMake(margin, 0, x, controlView.zl_height);
        [openImg setTitle:Custing(@"相册", nil) forState:UIControlStateNormal];
        [openImg addTarget:self action:@selector(opneImg:) forControlEvents:UIControlEventTouchUpInside];
        [controlView addSubview:openImg];
...
-(void)opneImg:(id)sender{
    if (self.images.count>=1) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:Custing(@"提醒", nil) message:[NSString stringWithFormat:@"%@%ld",Custing(@"图片的个数不能超过",nil),(long)self.maxCount]delegate:nil cancelButtonTitle:Custing(@"好的", nil) otherButtonTitles:nil, nil];
        [alertView show];
    }else{
        ...
        pickerVc.callBack = ^(NSArray *assets){
            [weakSelf.images addObjectsFromArray:assets];
            [weakSelf doneAction];
        };
...
- (void)stillImage:(id)sender
{
    ...
    if (self.maxCount > 0 && self.images.count >= self.maxCount) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:Custing(@"提醒", nil) message:[NSString stringWithFormat:@"%@%ld",(self.int_place == 1 ? Custing(@"图片的个数不能超过",nil):Custing(@"拍照的个数不能超过",nil)),(long)self.maxCount]delegate:nil cancelButtonTitle:Custing(@"好的", nil) otherButtonTitles:nil, nil];
        [alertView show];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.01 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        maskView.backgroundColor = Color_form_TextFieldBackgroundColor;
