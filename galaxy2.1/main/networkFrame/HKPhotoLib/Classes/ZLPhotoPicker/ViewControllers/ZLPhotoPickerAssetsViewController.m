...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [rightBtn setTitle:Custing(@"完成Camera", nil) forState:UIControlStateNormal];
...
- (UILabel *)makeView{
    if (!_makeView) {
        ...
        makeView.textColor = Color_form_TextFieldBackgroundColor;
...
#pragma mark 初始化按钮
- (void) setupButtons{
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:Custing(@"取消", nil) style:UIBarButtonItemStyleDone target:self action:@selector(back)];
    self.navigationItem.rightBarButtonItem.tintColor=[GPUtils colorHString:@"#797979"];
}
