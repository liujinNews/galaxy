...
- (instancetype)initWithTitle:(NSString *)title message:(NSString *)message canDismis:(BOOL)candimis{
    self = [super init];
    if (self) {
...
-(void)buildViews{
    ...
    if (self.canDismis) {
        self.backgroundView=[[UIView alloc]initWithFrame:[self screenBounds]];
    ...
    _coverView.backgroundColor = Color_Unsel_TitleColor;
    _coverView.alpha = 0;
    ...
    _alertView.backgroundColor = Color_form_TextFieldBackgroundColor;
    ...
    _labelTitle.font = Font_cellContent_16;
    _labelTitle.textColor = Color_cellContent;
    ...
    _labelmessage.textColor = Color_Unsel_TitleColor;
...
- (void)buttonTouched:(UIButton*)button{
    ...
    if (![button.titleLabel.text isEqualToString:@"前往APP Store"]) {
        [self dismiss];
    }
...
-(UIView*)topView{
    UIViewController *con = [[AppDelegate appDelegate]topViewController];
    return  con.view;
}
- (void)show {
