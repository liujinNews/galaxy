...
#import "JQScanNative.h"
#import "JQZXingWrapper.h"
...
//ZXing封装
@property(nonatomic,strong)JQZXingWrapper *scanZXingObj;
              ...
              success:(void(^)(NSArray<JQScanResult*> *array))blockScanResult
{
    if (self = [super init])
    {
        ...
        if ( [JQScanWrapper isSysIos7Later] )
        {
            _scanNativeObj = [[JQScanNative alloc]initWithPreView:preView ObjectType:arrayBarCodeType cropRect:cropRect success:^(NSArray<JQScanResult*> *array) {
        ...
        {
            _scanZXingObj = [[JQZXingWrapper alloc]initWithPreView:preView block:^(ZXBarcodeFormat barcodeFormat, NSString *str, UIImage *scanImg) {
                
                //TODO:ZXing统一识别所有的码
                if ( blockScanResult )
                {
                    NSString *barCodeType = [JQScanWrapper convertZXBarcodeFormat:barcodeFormat];
                    
                    JQScanResult *result = [[JQScanResult alloc]initWithScanString:str imgScan:scanImg barCodeType:barCodeType];
...
- (void)startScan
{
    ...
    if ( [JQScanWrapper isSysIos7Later] && !_isUseZXingLib )
...
{    
    ...
    [JQZXingWrapper recognizeImage:image block:^(ZXBarcodeFormat barCodeFormat,NSString* str)
     {
         NSString *barCodeType = [JQScanWrapper convertZXBarcodeFormat:barCodeFormat];
         
         if (block) {
             
             JQScanResult *result = [[JQScanResult alloc]initWithScanString:str imgScan:tmpImg barCodeType:barCodeType];
             block(@[result]);
         }
...
+ (UIImage*)createQRWithString:(NSString*)str size:(CGSize)size
{
    if ([JQScanWrapper isSysIos7Later]) {
        ...
        return [JQScanWrapper createNonInterpolatedUIImageFormCIImage:[JQScanWrapper createQRForString:str] withSize:size.width];
    }
    else
