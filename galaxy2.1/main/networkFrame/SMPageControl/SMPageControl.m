...
- (void)_renderPages:(CGContextRef)context rect:(CGRect)rect
{
	...
	for (NSInteger i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			fillColor = _currentPageIndicatorTintColor ? _currentPageIndicatorTintColor : Color_form_TextFieldBackgroundColor;
			image = _currentPageImages[indexNumber];
			if (nil == image) {
				image = _currentPageIndicatorImage;
			}
		} else {
			fillColor = _pageIndicatorTintColor ? _pageIndicatorTintColor : [Color_form_TextFieldBackgroundColor colorWithAlphaComponent:0.3f];
...
- (void)setStyleWithDefaults:(SMPageControlStyleDefaults)defaultStyle
{
	switch (defaultStyle) {
		case SMPageControlDefaultStyleModern:
			...
			self.pageIndicatorTintColor = [Color_form_TextFieldBackgroundColor colorWithAlphaComponent:0.2f];
		...
		default:
			...
			self.pageIndicatorTintColor = [Color_form_TextFieldBackgroundColor colorWithAlphaComponent:0.3f];
